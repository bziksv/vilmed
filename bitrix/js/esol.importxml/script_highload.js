...
var EIXPreview = {
	...
	ShowFieldSettings: function(e, btn)
	{
		...
		var dialogParams = {
			...
			'content_url':'/bitrix/admin/'+esolIXModuleFilePrefix+'_field_settings_hl.php?lang='+BX.message('LANGUAGE_ID')+'&field='+val+'&field_name='+name+'&index='+index+'&PROFILE_ID='+form.PROFILE_ID.value,
			'width':'900',
			'height':'400',
			'resizable':true,
...
var EProfile = {
	...
	ShowCron: function()
	{
		var cronWindowPath = '/bitrix/admin/'+esolIXModuleFilePrefix+'_cron_settings.php?lang='+BX.message('LANGUAGE_ID')+'&suffix=highload';
		...
		BX.addCustomEvent(dialog, 'onWindowRegister', function(){
			...
			$.post(cronWindowPath, 'action=getphpversion', function(data){
				if(data.length > 0)
				{
					$('#esol-ix-cron-form input[name="agent_php_path"]').val(data);
				}
			});
		});
	...
	OpenMissignElementFields: function(link)
	{
		...
		var dialogParams = {
			...
			'content_url':'/bitrix/admin/'+esolIXModuleFilePrefix+'_missignelem_fields_hl.php?lang='+BX.message('LANGUAGE_ID')+'&HLBL_ID='+hlblId+'&INPUT_ID='+input.attr('id'),
	...
	ShowFileAuthForm: function()
	{
		...
		var dialog = new BX.CAdminDialog({
			...
			'content_url':'/bitrix/admin/'+esolIXModuleFilePrefix+'_source_linkauth.php?lang='+BX.message('LANGUAGE_ID')+'&PROFILE_ID='+pid+'&TYPE=HL',
			'content_post': post,
			'width':'900',
			'height':'450',
			'resizable':true});
	...
	OpenMissignElementFilter: function(link)
	{
		...
		var dialogParams = {
			...
			'content_url':'/bitrix/admin/'+esolIXModuleFilePrefix+'_missignelem_filter_hl.php?lang='+BX.message('LANGUAGE_ID')+'&HIGHLOADBLOCK_ID='+hlblockId+'&PROFILE_ID='+$('#PROFILE_ID').val(),
		...
		dialog.SetButtons([
			...
			new BX.CWindowButton(
			{
				...
				action: function () {
					...
					$.post('/bitrix/admin/'+esolIXModuleFilePrefix+'_missignelem_filter_hl.php?lang='+BX.message('LANGUAGE_ID'), $('#esol-ix-filter').serialize(), function(data){
...
var EProfileList = {	
	...
	{
		var windowUrl = '/bitrix/admin/'+esolIXModuleFilePrefix+'_profile_list_highload.php?lang='+BX.message('LANGUAGE_ID')+'&action=shownewgroupform';
		var dialogParams = {
			'title':BX.message("ESOL_IX_POPUP_NEW_GROUP_TITLE"),
	...
	NewGroupDialogButtonsSet: function(fireEvents)
	{
		...
		dialog.SetButtons([
			...
			new BX.CWindowButton(
			{
				title: BX.message('ESOL_IX_POPUP_NEW_GROUP_SAVE_BTN'),
				id: 'savebtn',
				name: 'savebtn',
				className: BX.browser.IsIE() && BX.browser.IsDoctype() && !BX.browser.IsIE10() ? '' : 'adm-btn-save',
				action: function () {
					...
					$.ajax({
						url: '/bitrix/admin/'+esolIXModuleFilePrefix+'_profile_list_highload.php?lang='+BX.message('LANGUAGE_ID'),
						type: 'POST',
...
var EImport = {
	...
	GetStatus: function()
	{
		...
		$.ajax({
			type: "GET",
			url: '/upload/tmp/'+esolIXModuleName+'/'+this.pid+'_highload.txt?hash='+(new Date()).getTime(),
			success: function(data){
				var finish = false;
				if(data && data.substr(0, 1)=='{' && data.substr(data.length-1)=='}')
				{
...
var ESettings = {
	...
	BindConversionEvents: function()
	{
		$('.esol-ix-settings-conversion').each(function(){
			...
			if($(parent).closest('form').attr('data-notadmin'))
			{			
				$('select.field_then, span.field_then input[type="hidden"]', parent).bind('change', function(){
					var readony = (this.value=='EXPRESSION');
					$('.field_to textarea, input.field_to', parent).prop('readonly', readony);
					//$('.field_to .choose_val', parent).prop('disabled', readony);
					$('.choose_val', parent).prop('disabled', readony);
