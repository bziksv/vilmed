(function(BX, $) {
	...
	var constructor = Input.Formula = Plugin.Base.extend({
		...
		setValue: function(value) {
			this.getElement('function').find('option').each((index, option) => { option.selected = (value['FUNCTION'] === option.value); });
			this.syncFunctionAndPartsMultiple();
			this.setSelectedFields(value['PARTS']);
		},
		...
		setSelectedFields: function(field) {
			const partsElement = this.getElement('parts');
