<?php

$MESS['YANDEX_MARKET_MENU_CONNECT'] = 'Подключение к Маркету';
$MESS['YANDEX_MARKET_MENU_ORIGIN_ROOT'] = 'Рекламное размещение';
$MESS['YANDEX_MARKET_MENU_SETUP'] = 'Прайс-листы';
$MESS['YANDEX_MARKET_MENU_COLLECTION'] = 'Коллекции';
$MESS['YANDEX_MARKET_MENU_PROMO'] = 'Акции (бета-версия)';
$MESS['YANDEX_MARKET_MENU_ORDER_ADMIN'] = 'Заказы';
$MESS['YANDEX_MARKET_MENU_ORDER_ADMIN_FBS'] = 'Заказы (FBS)';
$MESS['YANDEX_MARKET_MENU_ORDER_ADMIN_DBS'] = 'Заказы (DBS)';
$MESS['YANDEX_MARKET_MENU_ORDER_LIST'] = 'Документы';
$MESS['YANDEX_MARKET_MENU_SHIPMENT_LIST'] = 'Отгрузки';
$MESS['YANDEX_MARKET_MENU_SETTINGS'] = 'Настройки';
$MESS['YANDEX_MARKET_MENU_SALES_BOOST_SETUP'] = 'Буст продаж';
$MESS['YANDEX_MARKET_MENU_EVENT'] = 'Журнал событий';
$MESS['YANDEX_MARKET_MENU_LOG'] = 'Журнал ошибок';
$MESS['YANDEX_MARKET_MENU_CONFIRMATION'] = 'Подтверждение прав Яндекс.Вебмастер';
$MESS['YANDEX_MARKET_MENU_HELP'] = 'Справка';
$MESS['YANDEX_MARKET_MENU_FEEDBACK'] = 'Обратная связь';
$MESS['YANDEX_MARKET_MENU_CATALOG'] = 'Каталог товаров';
