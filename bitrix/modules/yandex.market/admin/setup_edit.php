<?php

use Bitrix\Main;
use Bitrix\Main\Localization\Loc;
use Yandex\Market;

const BX_SESSION_ID_CHANGE = false;

require $_SERVER['DOCUMENT_ROOT'] . '/bitrix/modules/main/include/prolog_admin.php';

Loc::loadMessages(__FILE__);

global $APPLICATION;

if (!Main\Loader::includeModule('yandex.market'))
{
	\CAdminMessage::ShowMessage([
        'TYPE' => 'ERROR',
        'MESSAGE' => Loc::getMessage('YANDEX_MARKET_SETUP_EDIT_REQUIRE_MODULE')
    ]);
}
else if (!Market\Ui\Access::isProcessExportAllowed())
{
	\CAdminMessage::ShowMessage([
		'TYPE' => 'ERROR',
		'MESSAGE' => Loc::getMessage('YANDEX_MARKET_SETUP_EDIT_ACCESS_DENIED')
	]);
}
else
{
	$request = Main\Context::getCurrent()->getRequest();
	$parent = (int)$request->get('parent');
	$baseQuery = [
		'lang' => LANGUAGE_ID,
	];

	$APPLICATION->IncludeComponent('yandex.market:admin.form.edit', '', [
		'TITLE' => Market\Config::getLang('SETUP_EDIT_TITLE_EDIT'),
		'TITLE_ADD' => Market\Config::getLang('SETUP_EDIT_TITLE_ADD'),
		'BTN_SAVE' => Market\Config::getLang('SETUP_EDIT_BTN_SAVE'),
		'FORM_ID'   => 'YANDEX_MARKET_ADMIN_SETUP_EDIT',
		'FORM_BEHAVIOR' => 'steps',
		'ALLOW_SAVE' => Market\Ui\Access::isWriteAllowed(),
		'PRIMARY'   => !empty($_GET['id']) ? $_GET['id'] : null,
		'COPY' => isset($_GET['copy']) && $_GET['copy'] === 'Y',
		'LIST_URL'  => Market\Ui\Admin\Path::getModuleUrl('setup_list', $baseQuery),
		'SAVE_URL'  => Market\Ui\Admin\Path::getModuleUrl('setup_run', $baseQuery) . '&id=#ID#',
		'PROVIDER' => Market\Component\Setup\EditForm::class,
		'MODEL_CLASS_NAME' => Market\Export\Setup\Model::class,
		'GROUP' => $parent,
		'USE_METRIKA' => 'Y',
		'CONTEXT_MENU' => [
			[
                'ICON' => 'btn_list',
	            'LINK' => Market\Ui\Admin\Path::getModuleUrl('setup_list', $baseQuery),
	            'TEXT' => Market\Config::getLang('SETUP_EDIT_CONTEXT_MENU_LIST')
            ]
        ],
		'TABS' => [
			[
				'name' => Market\Config::getLang('SETUP_EDIT_TAB_COMMON'),
				'data' => [
					'METRIKA_GOAL' => 'select_infoblocks'
				],
				'fields' => [
					'NAME',
					'DOMAIN',
					'HTTPS',
					'FILE_NAME',
					'EXPORT_SERVICE',
					'EXPORT_FORMAT',
					'SHOP_DATA',
					'IBLOCK',
					'ENABLE_AUTO_DISCOUNTS',
					'ENABLE_CPA',
					'AUTOUPDATE',
					'REFRESH_PERIOD',
					'REFRESH_TIME',
				]
			],
			[
				'name' => Market\Config::getLang('SETUP_EDIT_TAB_PARAM'),
				'layout' => 'setup-param',
				'data' => [
					'METRIKA_GOAL' => 'infoblock_matching',
					'WARNING' => Market\Config::getLang('SETUP_EDIT_TAB_PARAM_WARNING'),
				],
				'fields' => [
					'IBLOCK_LINK.PARAM',
				]
			],
			[
				'name' => Market\Config::getLang('SETUP_EDIT_TAB_FILTER'),
				'layout' => 'setup-filter',
				'final' => true,
				'data' => [
					'METRIKA_GOAL' => 'delivery_options'
				],
				'fields' => [
					'DELIVERY',
					'SALES_NOTES',
					'IBLOCK_LINK.DELIVERY',
					'IBLOCK_LINK.SALES_NOTES',
					'IBLOCK_LINK.FILTER',
					'IBLOCK_LINK.EXPORT_ALL',
				]
			]
		],
		'PRODUCT_PARAM_FIELDS' => [
			'IBLOCK_LINK.PARAM',
		],
	]);
}

require $_SERVER['DOCUMENT_ROOT'] . '/bitrix/modules/main/include/epilog_admin.php';
