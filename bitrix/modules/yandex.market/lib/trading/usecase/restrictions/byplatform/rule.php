...
class Rule
{
	...
	{
		if (!static::isFeatureSupported($entityType)) { return []; }
		...
		{
			if ($setup->isDeprecated()) { continue; }

			$business = $setup->getBusiness();
			$platformId = $business->getPlatformId();

			if ($platformId <= 0) { continue; }

			$result[$business->getPlatformId()] = $business->getName();
		}

		return $result;
	}

	protected static function isFeatureSupported($entityType)
	{
		if ($entityType === static::ENTITY_TYPE_DELIVERY || $entityType === static::ENTITY_TYPE_PAY_SYSTEM)
		{
			return true;
		}
