...
class TradeBindingPreserve extends Market\Reference\Event\Regular
{
	...
	protected static $tradingPlatformIds = [];
	...
	public static function restore()
	{
		...
		{
			$xmlData = self::parseXmlId($row['XML_ID']);
			...
			$fields = [
				...
				'PARAMS' => array_intersect_key($xmlData, [
					'CAMPAIGN_ID' => true,
					'SETUP_ID' => true,
				]),
			];
	...
	protected static function getTradeBindingOrderXmlData(Sale\TradeBindingEntity $tradeBindingEntity)
	{
		...
		return self::parseXmlId($order->getField('XML_ID'));
	}
	...
	protected static function parseXmlId($xmlId)
	{
		$xmlData = TradingEntity\Sale\Platform::parseOrderXmlId($xmlId);

		if ($xmlData === null || isset($xmlData['PLATFORM_ID']))
		{
			return $xmlData;
		}

		$platformId = self::tradingPlatformId($xmlData['SETUP_ID']);
	...
	protected static function tradingPlatformId($setupId)
	{
		$setupId = (int)$setupId;

		if ($setupId <= 0) { return null; }

		if (!isset(self::$tradingPlatformIds[$setupId]) && !array_key_exists($setupId, self::$tradingPlatformIds))
		{
			$tradingRow = Market\Trading\Setup\Table::getRow([
				'filter' => [ '=ID' => $setupId ],
				'select' => [ 'EXTERNAL_ID' ],
			]);

			self::$tradingPlatformIds[$setupId] = !empty($tradingRow['EXTERNAL_ID']) ? (int)$tradingRow['EXTERNAL_ID'] : null;
		}

		return self::$tradingPlatformIds[$setupId];
	}
}