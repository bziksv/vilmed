	...
	implements TradingService\Reference\Action\HasActivity
{
	...
	protected function formatDeliveryServiceIdValue(Market\Api\Model\Order\Delivery $delivery, $serviceId)
	{
		try
		{
			...
			$deliveryService = $this->provider->getRequestFactory()
                   ->create(Market\Api\Delivery\Services\Request::class)
                   ->execute()
                   ->getDeliveryServices()
    ...
    {
        return parent::getDeliveryActivities() + [
            'dates' => 'send/delivery/date',
            'outletStorageLimitDate' => 'send/delivery/storageLimit',
        ];
    }
