...
class Action extends TradingService\Marketplace\Action\OrderStatus\Action
{
	...
	protected function fillBuyerProperties()
	{
        ...
        $isProcessing = $statusService->isProcessing($status);
		...
		else if ($isProcessing && !isset($values['PHONE']) && $this->hasBuyerPhoneProperty() && $this->isBuyerPhoneExpired())
		{
	...
	protected function clearBuyerPhoneTask()
	{
		...
		$task->clear($campaignId, 'fill/phone');
	}
	...
	protected function createBuyerPhoneTask()
	{
		...
		$task->clear($campaignId, 'fill/phone');
		$task->schedule($campaignId, 'fill/phone', $payload);
	}
