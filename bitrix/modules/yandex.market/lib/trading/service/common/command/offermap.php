...
class OfferMap
{
	...
	protected function resolveFeedDuplicates(array $offerMap = null)
	{
		...
		if ($catalog !== null && $catalog->wasSubmitted())
		{
			...
			$exported = $catalogPrimary->exported(array_keys($offerMap), $catalog->getId());

			list($newMap, $replaced) = $this->unsetSkuWithOtherElement($offerMap, $exported);

			if (empty($replaced)) { return $offerMap; }
		}
		else if ($this->provider instanceof TradingService\Marketplace\Provider)
		{
			$feedPrimary = new FeedPrimary();
			$feeds = $this->provider->getOptions()->getProductFeeds();
			$exported = $feedPrimary->exported(array_keys($offerMap), $feeds);

			list($newMap, $replaced) = $this->unsetSkuWithOtherElement($offerMap, $exported);

			if (empty($replaced) || !$feedPrimary->canUsePrimaryAsSku($feeds)) { return $offerMap; }
		}
		else
		{
			return $offerMap;
