...
class FeedPrimary
{
	...
	protected function feedsIblocksWithShopSkuTag(array $feeds)
	{
		...
		$query = Export\Param\Table::getList([
			'filter' => [
				'=ENTITY_TYPE' => Export\Param\Table::ENTITY_TYPE_IBLOCK_LINK,
				'=XML_TAG' => 'shop-sku',
		...
		return array_column($query->fetchAll(), 'ENTITY_ID');
	}
	...
	protected function feedPrimaryTagsSources($iblockLinkId)
	{
		...
		$query = Export\ParamValue\Table::getList([
			'filter' => [
				'=PARAM.ENTITY_TYPE' => Export\Param\Table::ENTITY_TYPE_IBLOCK_LINK,
				'=PARAM.ENTITY_ID' => $iblockLinkId,
				[
					'LOGIC' => 'OR',
					[
