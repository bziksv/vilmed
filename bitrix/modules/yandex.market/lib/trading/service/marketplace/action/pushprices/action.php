...
class Action extends TradingService\Reference\Action\DataAction
{
	...
	{
		if ($this->provider->getOptions()->priceConfigMigrated())
		{
			throw new Main\SystemException('conflict old push/prices with new catalog');
		}
	...
	protected function buildOffers($prices)
	{
		...
		foreach ($prices as $price)
		{
			...
			{
				$basePrice = round($price['BASE_PRICE']);
	...
	{
		$this->createOffersRequest($offers)->execute();
	}
	...
	protected function createOffersRequest($offers)
	{
		...
		{
			$request = $this->provider->getRequestFactory()->create(TradingService\Marketplace\Api\SendPrices\Business\Request::class);
            $request->setOffers($this->convertOfferForBusiness($offers));
		}
		else
		{
			$request = $this->provider->getRequestFactory()->create(TradingService\Marketplace\Api\SendPrices\Campaign\Request::class);
