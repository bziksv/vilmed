		...
		TradingService\Reference\Document\HasRenderFile
{
		...
		return [
			'FORMAT' => [
				...
				'VALUES' => [
					...
					[
						'ID' => 'A9_HORIZONTALLY',
						'VALUE' => static::getLang('TRADING_SERVICE_MARKETPLACE_DOCUMENT_BOX_LABEL_FORMAT_A9_HORIZONTALLY', null, 'A9_HORIZONTALLY'),
					],
					[
						'ID' => 'A9',
						'VALUE' => static::getLang('TRADING_SERVICE_MARKETPLACE_DOCUMENT_BOX_LABEL_FORMAT_A9', null, 'A9'),
					],
					[
						'ID' => 'A6',
	...
	public function render(array $items, array $settings = [])
	{
		...
		if ($settings['FORMAT'] === 'A4')
		{
			$generated = $this->fetchReport($items, $settings);

			return $this->renderReport($generated);
		}

		if ($this->useBundle($settings))
		{
			return $this->renderDocument($items, $settings, 'pdfbundle');
		}

		return $this->renderFileWindow($items);
	}
	...
	protected function renderFileWindow(array $items)
	{
	...
	protected function fetchReport(array $items, array $settings = [])
	{
		$generate = $this->provider->getRequestFactory()->create(Market\Api\Reports\Documents\Labels\Generate\Request::class);
		$generate->setOrderIds(array_column($items, 'ORDER_ID'));
		$generate->setFormat($settings['FORMAT']);

		return $generate->execute();
	}

	protected function renderReport(Market\Api\Reports\GenerateResponse $generated)
	{
		return $this->renderComponent('report', [
			'REPORT_ID' => $generated->getReportId(),
			'REPORT_WAIT' => $generated->getEstimatedGenerationTime(),
			'SETUP_ID' => $this->provider->getOptions()->getSetupId(),
		]);
	}
}