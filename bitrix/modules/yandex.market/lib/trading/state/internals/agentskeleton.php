...
{
	const CAMPAIGN_ID_MIGRATION_LIMIT = 10000;
	...
	protected static function getCampaignSetup($campaignId)
	{
		if ($campaignId < self::CAMPAIGN_ID_MIGRATION_LIMIT)
		{
			return self::getSetup($campaignId);
		}

		$setup = Market\Trading\Campaign\ModelPool::getById($campaignId)->getTrading();

		if (!$setup->isActive())
		{
			throw new Main\ObjectPropertyException(sprintf('campaign %s is inactive', $campaignId));
		}
	...
	{
		$setup = Market\Trading\Setup\ModelPool::getById($setupId);
