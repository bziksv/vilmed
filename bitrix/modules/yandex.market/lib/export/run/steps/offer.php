...
class Offer extends Base
{
			...
			do
			{
				try
				{
					...
					foreach ($iblockConfig['FILTER_LIST'] as $sourceFilter)
					{
						...
						if ($offsetObject->get('filter') !== null && $offsetObject->tick('filter')) { break; }
					}
	...
	public function run($action, $offset = null)
	{
		...
		foreach ($this->getIblockConfigList() as $iblockConfig)
		{
			...
			$sourceSelect = (new Market\Export\Param\TagMap($tagDescriptionList))->getSourceSelect();
	...
	protected function getTagResultHash($xmlContent, Market\Result\XmlValue $tagValues = null)
	{
		...
		return $this->calculateXmlContentHash(
			$xmlContent,
			$tagValues !== null ? $tagValues->getType() : null
		);
	}
	...
	{
		return Market\Export\Run\Storage\OfferTable::class;
	}
			...
			if (isset($rule['TAG']))
			{
				...
				foreach ($elementIds as $elementId)
				{
					...
					if (!$this->isEmptyXmlValue($ruleValue))
					{
						...
						{
                            $ruleValue = $ruleNode->compareValue($ruleValue, $context, $tagValues);
