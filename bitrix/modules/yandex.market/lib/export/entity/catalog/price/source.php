...
class Source extends Market\Export\Entity\Reference\Source
{
	...
	{
		list($priceType, $fieldName) = explode('.', $selectField, 2);
	...
	protected function getElementCatalogPrice($element, $fields, $priceType, $context, $catalogPriceIds, $elementPrices)
	{
		...
		if (!empty($catalogPriceIds))
		{
			...
			$percentVat = isset($element['CATALOG_VAT']) ? $this->castVatRate($element['CATALOG_VAT']) : 0;
	...
	protected function castVatRate($rate)
	{
		if ($rate === 'NO_VAT')
		{
			return 0;
		...
		if (preg_match('/^VAT_(\d+)$/', $rate, $matches))
		{
			$ratio = ltrim($matches[1], '0');

			return (float)$ratio * 0.01;
		}

		if (is_numeric($rate))
		{
			return (float)$rate * 0.01;
		}
