...
class Source extends Market\Export\Entity\Reference\Source
{
	...
	protected function getDisplayValue($element, $fieldName, $context = null)
	{
		...
		else
		{
			...
			switch ($fieldName)
			{
				...
				case 'VAT':
					$vatRate = is_numeric($originalValue) ? (int)$originalValue : null;

					if ($vatRate === null)
					{
						$result = isset($element['VAT_EXCLUDE_VAT']) && $element['VAT_EXCLUDE_VAT'] === 'Y' ? 'NO_VAT' : null;
					}
					else if ($vatRate === 0)
					{
						$result = isset($element['VAT_NAME']) && mb_strpos($element['VAT_NAME'], '0%') !== false ? 'VAT_0' : 'NO_VAT';
					}
					else if ($vatRate > 0)
					{
						if ($vatRate < 10)
	...
	protected function loadInternalData($elementList, $select)
	{
		...
		$needVat = in_array('VAT', $select, true);
		$loadedCatalogVat = [];
		$internalSelect[] = 'ID';
		...
		while ($row = $query->fetch())
		{
			...
			if ($needVat && empty($row['VAT_ID']) && isset($elementList[$row['ID']]))
			{
				$iblockId = (int)$elementList[$row['ID']]['IBLOCK_ID'];
				$catalogVat = null;

				if (isset($loadedCatalogVat[$iblockId]) || array_key_exists($iblockId, $loadedCatalogVat))
				{
					$catalogVat = $loadedCatalogVat[$iblockId];
				}
				else
				{
					$catalog = \CCatalogSku::GetInfoByIBlock($elementList[$row['ID']]['IBLOCK_ID']);

					if (!empty($catalog['VAT_ID']))
					{
						$catalogVat = Catalog\VatTable::getRowById($catalog['VAT_ID']);
						$loadedCatalogVat[$iblockId] = $catalogVat;
					}
				}

				if ($catalogVat !== null)
				{
					$row['VAT'] = $catalogVat['RATE'];
					$row['VAT_NAME'] = $catalogVat['NAME'];
					$row['VAT_EXCLUDE_VAT'] = $catalogVat['EXCLUDE_VAT'];
					$row['VAT_ID'] = $catalogVat['ID'];
				}
			}
	...
	protected function convertSelectToInternalFields($select, Main\Entity\Base $entity)
	{
		...
		foreach ($select as $key => $field)
		{
			...
			else if (isset($referenceFields[$field]))
			{
				...
				for ($i = count($reference) - 1; $i >= 2; --$i)
				{
					$querySelect[$selectName . '_' . $reference[$i]] = $reference[0] . '.' . $reference[$i];
					$referenceMap[$selectName . '_' . $reference[$i]] = $field . '_' . $reference[$i];
				}
			}
	...
	{
		$vatAdditional = [ 'ID', 'NAME' ];

		if (Catalog\VatTable::getEntity()->hasField('EXCLUDE_VAT'))
		{
			$vatAdditional[] = 'EXCLUDE_VAT';
		}

		return [
			'MEASURE_RATIO' => ['YM_MEASURE_RATIO', 'RATIO'],
			'VAT' => array_merge([ 'YM_VAT', 'RATE' ], $vatAdditional),
		];
	}
