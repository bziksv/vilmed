...
class Provider
{
	protected static $catalogIblockIdCache = [];
	...
	public static function getCatalogIblockId($offerIblockId)
	{
		...
		if ($offerIblockId <= 0 || !Main\Loader::includeModule('catalog')) { return null; }

		if (isset(self::$catalogIblockIdCache[$offerIblockId]) || array_key_exists($offerIblockId, self::$catalogIblockIdCache))
		{
			return self::$catalogIblockIdCache[$offerIblockId];
		}
		...
		if (!empty($catalogData) && $catalogData['CATALOG_TYPE'] === \CCatalogSku::TYPE_OFFERS)
		{
			...
			$result = $productIblockId > 0 ? $productIblockId : null;
		}

		self::$catalogIblockIdCache[$offerIblockId] = $result;
