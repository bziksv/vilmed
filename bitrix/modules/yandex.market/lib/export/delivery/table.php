...
class Table extends Market\Reference\Storage\Table
{
	...
	public static function migrate(Main\DB\Connection $connection)
	{
		...
		if (!isset($tableFields['DELIVERY_TYPE']))
		{
			$connection->queryExecute(
				'ALTER TABLE ' . $sqlHelper->quote($tableName)
				. ' ADD COLUMN ' . $sqlHelper->quote('DELIVERY_TYPE') . ' varchar(8) NOT NULL'
			);
			$connection->queryExecute(
				'UPDATE ' . $sqlHelper->quote($tableName)
				. ' SET ' . $sqlHelper->quote('DELIVERY_TYPE') . ' = \'' . static::DELIVERY_TYPE_DELIVERY .'\''
			);
		}
	}
