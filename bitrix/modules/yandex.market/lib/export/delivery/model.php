...
class Model extends Market\Reference\Storage\Model
{
	...
	{
		$periodFromValue = trim($this->getField('PERIOD_FROM'));
		$periodFrom = (int)$periodFromValue;
		$hasPeriodFrom = ($periodFromValue !== '' && $periodFrom >= 0);
		$periodToValue = trim($this->getField('PERIOD_TO'));
		$periodTo = (int)$periodToValue;
		$hasPeriodTo = ($periodToValue !== '' && $periodTo >= 0);
		$result = null;
	...
	{
		$value = trim($this->getField('ORDER_BEFORE'));
		$valueInt = (int)$value;
		$result = null;

		if ($valueInt >= 0 && $valueInt <= 24 && $value !== '')
		{
			$result = $valueInt;
		}
