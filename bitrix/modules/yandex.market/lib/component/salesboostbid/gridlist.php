...
class GridList extends Component\Base\GridList
{
	...
	protected function fetchBids(array $fetchParameters)
	{
		...
		$options = $business->getOptions();

		$request = new Bids\Info\Request($business->getId(), $options->getApiAuth(), $business->createLogger());
	...
	protected function fetchRecommendations(array $skus)
	{
		...
		$request = new Bids\Recommendations\Request($business->getId(), $options->getApiAuth(), $business->createLogger());
	...
	{
		return $this->business()->createLogger();
	}
