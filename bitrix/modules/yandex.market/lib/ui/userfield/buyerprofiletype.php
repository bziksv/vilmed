...
class BuyerProfileType extends EnumerationType
{
	...
	protected static function extractUserFieldPersonType($userField)
	{
		...
		if ($fieldPersonType !== null && isset($userField['ROW']) && is_array($userField['ROW']))
		{
			$value = Market\Utils\Field::getChainValue($userField['ROW'], $fieldPersonType, Market\Utils\Field::GLUE_BRACKET);
		...
		if (isset($userField['SETTINGS']['PERSON_TYPE_DEFAULT']))
		{
			return $userField['SETTINGS']['PERSON_TYPE_DEFAULT'];
		}
	...
	{
		if (!isset($userField['SETTINGS']['PERSON_TYPE_FIELD'])) { return null; }

		$field = $userField['SETTINGS']['PERSON_TYPE_FIELD'];

		if (isset($userField['COMPOUND_KEY']))
		{
			$field = "{$userField['COMPOUND_KEY']}[{$field}]";
		}
