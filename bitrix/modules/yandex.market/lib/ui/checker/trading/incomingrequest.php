...
class IncomingRequest extends Checker\Reference\AbstractTest
{
	...
	public function test()
	{
		...
		{
			if ($this->isPullSetup($setup) || !$this->hasDomain($setup)) { continue; }
	...
	protected function isPullSetup(TradingSetup\Model $setup)
	{
		try
		{
			if ($setup->getBehaviorCode() === Market\Trading\Service\Manager::BEHAVIOR_BUSINESS) { return true; }

			$options = $setup->wakeupService()->getOptions();

			if (!($options instanceof Market\Trading\Service\Marketplace\Options)) { return false; }

			return ($options->getYandexMode() === Market\Trading\Service\Marketplace\Options::YANDEX_MODE_PULL);
		}
		catch (Main\NotImplementedException $exception)
		{
			return false;
		}
	}

	protected function hasDomain(TradingSetup\Model $setup)
	{
		...
		else if ($request instanceof Main\HttpRequest)
		{
			...
			$result = (
				$requestHost !== ''
				&& mb_strtolower($requestHost) !== 'localhost'
