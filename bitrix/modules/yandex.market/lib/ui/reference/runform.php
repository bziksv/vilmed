...
abstract class RunForm extends Form
{
	...
	protected function showFormProlog()
	{
		...
		echo '<form class="js-plugin" method="post" action="' . htmlspecialcharsbx($postUrl) . '" data-plugin="Ui.Admin.ExportForm" id="yamarket_run_form">';
...
EOL
	...
	private function showAutostart()
	{
		if ($this->request->get('autostart') !== 'Y') { return; }

		echo <<<SCRIPT
			<script>
				BX.ready(function() {
					const Plugin = BX.namespace('YandexMarket.Plugin');
					const form = $('#yamarket_run_form');
					
					Plugin.manager.getInstance(form).run();
				});
			</script>
SCRIPT;
	}
