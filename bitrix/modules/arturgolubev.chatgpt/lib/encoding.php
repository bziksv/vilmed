...
class Encoding {
	...
	static function exTruncateText($text, $len, $dots = false){
		if(self::exStrlen($text) > $len){
			$text = self::exSubstr($text, 0, $len);
			$text = trim(self::exSubstr($text, 0, self::exStrrpos($text, ' ')));			
			if($dots)
				$text .= '...';
		}
		return $text;
