...
class Profile {
	...
	public function CheckStorage()
	{
		...
		/*profile_group*/
		$tEntity = new \Bitrix\EsolImportxml\ProfileGroupTable();
		$tblName = $tEntity->getTableName();
		$conn = $tEntity->getEntity()->getConnection();
		if(!$conn->isTableExists($tblName))
		{
	...
	public function GetList($arFilter=array(), $groups = false)
	{
		...
		if($groups)
		{
			...
			while($arr = $dbRes->Fetch())
			{
				...
				{
					$arr['GGROUP_ID'] = 0;
					if(!empty($arProfiles)) $arr['GGROUP_NAME'] = Loc::getMessage("ESOL_IX_PROFILES_WITHOUT_GROUP");
				}
	...
	public function Update($ID, $settigs_default, $settings, $extrasettings=null)
	{
		...
		if(($oldIblockId != $arProfile['SETTINGS_DEFAULT']['IBLOCK_ID'] || isset($arProfile['OLD_IBLOCK_DATA'])) && Loader::includeModule('iblock') && class_exists('\Bitrix\Iblock\PropertyTable'))
		{
			...
			$arSections = array();
			$dbRes = \Bitrix\Iblock\SectionTable::getList(array('filter'=>array('=IBLOCK_ID'=>$iblockId), 'select'=>array('ID', 'IBLOCK_SECTION_ID', 'NAME'), 'order'=>array('DEPTH_LEVEL'=>'ASC')));
			while($arr = $dbRes->Fetch())
			{
			...
			if(isset($arProfile['OLD_IBLOCK_DATA']))
			{
				...
				if(isset($arProfile['OLD_IBLOCK_DATA']['SECTIONS']) && isset($arProfile['OLD_IBLOCK_DATA']['SECTIONS'][$oldIblockId]) && is_array($arProfile['OLD_IBLOCK_DATA']['SECTIONS'][$oldIblockId]))
				{
					foreach($arProfile['OLD_IBLOCK_DATA']['SECTIONS'][$oldIblockId] as $k=>$v)
					{
			...
			else
			{
				...
				$dbRes = \Bitrix\Iblock\SectionTable::getList(array('filter'=>array('=IBLOCK_ID'=>$oldIblockId), 'select'=>array('ID', 'IBLOCK_SECTION_ID', 'NAME'), 'order'=>array('DEPTH_LEVEL'=>'ASC')));
				while($arr = $dbRes->Fetch())
				{
			...
			if(count($arPropRels) > 0)
			{
				...
				{
					$arMap = \Bitrix\EsolImportxml\Utils::Unserialize(base64_decode($arProfile['SETTINGS']['PROPERTY_MAP']));
			...
			{
				if($arProfile['SETTINGS']['SECTION_ID'] && isset($arSectionRels[$arProfile['SETTINGS']['SECTION_ID']]))
				{
					$arProfile['SETTINGS']['SECTION_ID'] = $arSectionRels[$arProfile['SETTINGS']['SECTION_ID']];
				}
				if(isset($arProfile['SETTINGS']['SECTION_MAP']) && strlen($arProfile['SETTINGS']['SECTION_MAP']) > 0)
				{
					$arMap = \Bitrix\EsolImportxml\Utils::Unserialize(base64_decode($arProfile['SETTINGS']['SECTION_MAP']));
					if(is_array($arMap) && isset($arMap['MAP']) && is_array($arMap['MAP']))
					{
						foreach($arMap['MAP'] as $k=>$v)
						{
							if(isset($arSectionRels[$v['ID']]))
							{
								$arMap['MAP'][$k]['ID'] = $arSectionRels[$v['ID']];
							}
						}
						$arProfile['SETTINGS']['SECTION_MAP'] = base64_encode(serialize($arMap));
					}
				}
			}
		}
	...
	public function UpdateFileHash($ID, $file, $updateDateFinish=false)
	{
		$hash = md5_file($file);
		$arFields = array('FILE_HASH'=>$hash);
		if($updateDateFinish) $arFields['DATE_FINISH'] = new \Bitrix\Main\Type\DateTime();
		$this->UpdateFields($ID, $arFields);
	}
	...
	{
		$tmpfile = $this->tmpdir.$id.($this->suffix ? '_'.$this->suffix : '').'.txt';
		if(!file_exists($tmpfile) || filesize($tmpfile)>200*1024) return '';		
		$arParams = \Bitrix\EsolImportxml\Utils::JsObjectToPhp(file_get_contents($tmpfile));
		$percent = round(((int)$arParams['total_read_line'] / max((int)$arParams['total_file_line'], 1)) * 100);
		$percent = min($percent, 99);
	...
	public function GetProfileParamsByFile($tmpfile)
	{
		...
		$maxLength1 = 10*1024;
		$maxLength2 = 500*1024;
		if(strlen($content) > $maxLength1 && (!preg_match('/"total_read_line"\s*:\s*\d+/', $content) || strlen($content) > $maxLength2))
		{
			$arParams = array();
			...
			if(preg_match_all('/"([^"]*)":(\d+(\.\d+)?)/', $content, $m))
			{
				foreach($m[1] as $k2=>$v2)
				{
					$arParams[$v2] = $m[2][$k2];
	...
	public function GetProcessedProfiles()
	{
		...
		foreach($arProfiles as $k=>$v)
		{
			...
			if(!file_exists($tmpfile) || filesize($tmpfile)>200*1024 || (time() - filemtime($tmpfile) < 4*60) || filemtime($tmpfile) < mktime(0, 0, 0, 12, 24, 2015))
			{
				unset($arProfiles[$k]);
				continue;
			}
	...
	public function GetProccessParams($id)
	{
		...
		if(file_exists($tmpfile))
		{
			...
			if(file_exists($paramFile)) $arParams = \Bitrix\EsolImportxml\Utils::Unserialize(file_get_contents($paramFile));
			return $arParams;
		}
		return false;
	}
	...
	{
		$arProfiles = $this->GetList(is_numeric($PROFILE_ID) && strlen($PROFILE_ID) > 0 ? array('LOGIC'=>'OR', array('ACTIVE'=>'Y'), array('ID'=>$PROFILE_ID+1)) : array(), true);
	...
	public static function EncodeProfileParams($arParams)
	{
		...
		$arParams['ENCODING'] = \Bitrix\EsolImportxml\Utils::getSiteEncoding();
	...
	public static function DecodeProfileParams($paramStr, $checkEncoding=true)
	{
		...
		$arParams = \Bitrix\EsolImportxml\Utils::Unserialize($paramStr);
		if(!is_array($arParams)) $arParams = array();
		if($checkEncoding && $arParams['ENCODING'] && $arParams['ENCODING'] != \Bitrix\EsolImportxml\Utils::getSiteEncoding())
		{
			$arParams = \Bitrix\Main\Text\Encoding::convertEncodingArray($arParams, $arParams['ENCODING'], \Bitrix\EsolImportxml\Utils::getSiteEncoding());
		}
		return $arParams;
	}
		...
		if(is_array($settings))
		{
			...
			if($settings['ADDITIONAL_SETTINGS'])
			{
				foreach($settings['ADDITIONAL_SETTINGS'] as $k=>$v)
				{
					if($v && !is_array($v))
					{
						$v = \Bitrix\EsolImportxml\Utils::JsObjectToPhp($v);
					}
	...
	public function CheckFileSettings(&$fileParams, $PROFILE_ID)
	{
		if(!$GLOBALS['USER']->IsAdmin())
		{
			if(preg_match('/^\{.*\}$/s', $fileParams))
			{
				$arParams = \Bitrix\EsolImportxml\Utils::JsObjectToPhp($fileParams);
				if((isset($arParams['HANDLER_FOR_LINK']) && strlen($arParams['HANDLER_FOR_LINK']) > 0)
					 || (isset($arParams['HANDLER_FOR_LINK_BASE64']) && strlen($arParams['HANDLER_FOR_LINK_BASE64']) > 0))
				{
					...
					if(strlen($PROFILE_ID) > 0 && is_numeric($PROFILE_ID))
					{
						...
						{
							$arOldParams = \Bitrix\EsolImportxml\Utils::JsObjectToPhp($arProfile['SETTINGS_DEFAULT']['EXT_DATA_FILE']);
					...
					$fileParams = \Bitrix\EsolImportxml\Utils::PhpToJSObject($arParams);
				}
			}
		}
	}
			...
			if(is_array($extrasettings))
			{
				...
				if(isset($arProfile['EXTRASETTINGS']) && is_array($arProfile['EXTRASETTINGS']))
				{
					foreach($arProfile['EXTRASETTINGS'] as $k=>$v)
					{
						...
						{
							if(!array_key_exists($convKey, $v) || !is_array($v[$convKey])) continue;
							foreach($v[$convKey] as $k2=>$v2)
							{
								if($v2['THEN']=='EXPRESSION' && strlen($v2['TO']) > 0 && !in_array($v2['TO'], $arExpressions))
								{
									$arExpressions[] = $v2['TO'];
								}
							}
						...
						if(array_key_exists('FILTER_EXPRESSION', $v) && strlen($v['FILTER_EXPRESSION']) > 0 && !in_array($v['FILTER_EXPRESSION'], $arFilterExpressions))
						{
							$arFilterExpressions[] = $v['FILTER_EXPRESSION'];
						}
					}
				}
				...
				{
					if(!is_array($v)) continue;
					$arOldExtra = (isset($arProfile['EXTRASETTINGS'][$k]) ? $arProfile['EXTRASETTINGS'][$k] : array());
					$arKeys = array('CONVERSION', 'EXTRA_CONVERSION');
					foreach($arKeys as $convKey)
					{
						if(!array_key_exists($convKey, $v) || !is_array($v[$convKey])) continue;
						foreach($v[$convKey] as $k2=>$v2)
						{
							if($v2['THEN']=='EXPRESSION' && !in_array($v2['TO'], $arExpressions))
							{
								$extrasettings[$k][$convKey][$k2]['TO'] = '';
								$index = (array_key_exists('INDEX', $v2) && strlen($v3['INDEX']) > 0 ? $v2['INDEX'] : $k2);
								if(isset($arOldExtra[$convKey][$index]['TO']))
								{
									$extrasettings[$k][$convKey][$k2]['TO'] = $arOldExtra[$convKey][$index]['TO'];
								}
							}
							if(array_key_exists('INDEX', $v2)) unset($extrasettings[$k][$convKey][$k2]['INDEX']);
						}
					}
					
					if(array_key_exists('FILTER_EXPRESSION', $v) && !in_array($v['FILTER_EXPRESSION'], $arFilterExpressions))
					{
						unset($extrasettings[$k]['FILTER_EXPRESSION']);
						if(isset($arOldExtra['FILTER_EXPRESSION']))
						{
							$extrasettings[$k]['FILTER_EXPRESSION'] = $arOldExtra['FILTER_EXPRESSION'];
						}
					}
				}
			}
		...
		if(file_exists($fn))
		{
			...
			foreach($arFileData as $line)
			{
				if(mb_strpos($line, ':') > 0)
				{
					...
					$arData[trim($arLine[0])] = \Bitrix\EsolImportxml\Utils::Unserialize(base64_decode(trim($arLine[1])));
				}
			}
	...
	public function OutputBackup()
	{
		...
		while($arProfile = $dbRes->Fetch())
		{
			...
			if(Loader::includeModule('iblock') && class_exists('\Bitrix\Iblock\PropertyTable') && isset($arProfile['PARAMS']) && strlen($arProfile['PARAMS']) > 0 && ($arProfileParams = self::DecodeProfileParams($arProfile['PARAMS'])) && is_array($arProfileParams))
			{
				...
				$arPropIds = array();
				$arSectionIds = array();
				if($arProfileParams['SETTINGS']['SECTION_ID']) $arSectionIds[] = $arProfileParams['SETTINGS']['SECTION_ID'];
				...
				{
					$arMap = \Bitrix\EsolImportxml\Utils::Unserialize(base64_decode($arProfileParams['SETTINGS']['PROPERTY_MAP']));
					if(is_array($arMap) && isset($arMap['MAP']) && is_array($arMap['MAP']))
					{
						foreach($arMap['MAP'] as $k=>$v)
						{
				...
				if(isset($arProfileParams['SETTINGS']['SECTION_MAP']) && strlen($arProfileParams['SETTINGS']['SECTION_MAP']) > 0)
				{
					$arMap = \Bitrix\EsolImportxml\Utils::Unserialize(base64_decode($arProfileParams['SETTINGS']['SECTION_MAP']));
					if(is_array($arMap) && isset($arMap['MAP']) && is_array($arMap['MAP']))
					{
						foreach($arMap['MAP'] as $k=>$v)
						{
							if($v['ID']) $arSectionIds[$v['ID']] = $v['ID'];
						}
					}
				}
				$arProps = array();
	...
	public function GetProfilesFromBackup($arPFile)
	{
		...
		$arProfiles = array();
		$arConfig = \Bitrix\EsolImportxml\Utils::Unserialize(base64_decode(file_get_contents($arFiles['config'])));
		$handle = fopen($arFiles['data'], "r");
		while(!feof($handle))
		{
			$buffer = trim(fgets($handle, 16777216));
			if(strlen($buffer) == 0) continue;			
			$arProfile = \Bitrix\EsolImportxml\Utils::Unserialize(base64_decode($buffer));
			...
			foreach($arProfile as $k=>$v)
			{
				...
				if($encoding != $arConfig['encoding'])
				{
					if($k=='PARAMS')
					{
						...
						$v = \Bitrix\Main\Text\Encoding::convertEncoding($v, $arConfig['encoding'], $encoding);
						if($v['SETTINGS']['PROPERTY_MAP']) $v['SETTINGS']['PROPERTY_MAP'] = base64_encode(serialize(\Bitrix\Main\Text\Encoding::convertEncoding(\Bitrix\EsolImportxml\Utils::Unserialize(base64_decode($v['SETTINGS']['PROPERTY_MAP'])), $arConfig['encoding'], $encoding)));
