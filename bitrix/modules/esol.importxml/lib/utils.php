...
class Utils {
	...
	public static function GetOfferIblock($IBLOCK_ID, $retarray=false)
	{
		...
		if(!isset(self::$offerIblocks[$IBLOCK_ID]))
		{
			...
			{
				$arRels = self::Unserialize(\COption::GetOptionString(static::$moduleId, 'CATALOG_RELS'));
	...
	public static function GetFileName($fn)
	{
		...
		if(self::IsUtfMode()) $tmpfile = $APPLICATION->ConvertCharsetArray($fn, LANG_CHARSET, 'CP1251');
				...
				if($updateFile)
				{
					...
					if($oProfile->GetParam('AUTO_FIX_XML_ERRORS')=='Y')
					{
						...
						while(!feof($handle)) 
						{
							...
							/*
							if($pos!==false && !feof($handle) && count($arTags) > 0  && function_exists('mb_strripos'))
							{
								foreach($arTags as $tag)
								{
									$rpos1 = false;
									if(($rpos11 = mb_strripos($buffer2, '<'.$tag.' '))!==false) $rpos1 = $rpos11;
									if(($rpos12 = mb_strripos($buffer2, '<'.$tag.'>'))!==false) $rpos1 = ($rpos1!=false ? max($rpos1, $rpos12) : $rpos12);
									if($rpos1!==false)
									{	
										$rpos2 = mb_strripos($buffer2, '</'.$tag.'>');
										if($rpos2===false || $rpos1 > $rpos2)
										{
											$pos = $rpos1;
										}
										else
										{
											$pos = max($rpos2+mb_strlen('</'.$tag.'>'), mb_strripos($buffer2, '<'));
										}
										$bufferEnd = mb_substr($buffer2, $pos);
										$buffer2 = mb_substr($buffer2, 0, $pos);
										$pos = false;
									}
								}
							...
							*/
							...
							foreach($arTags as $tag)
							{
								//$buffer2 = preg_replace_callback('/(<'.$tag.'(\s[^>]*>|>))(.*)(<\/'.$tag.'>)/Uis', array('\Bitrix\EsolImportxml\Utils', 'ModifyTagWithCdata'), $buffer2);
	...
	public static function ModifyTagWithCdata($m)
	{
		return $m[1].'<![CDATA['.trim(str_ireplace(array('<![CDATA[', ']]>'), '', $m[3])).']]>'.$m[4];
	}
	
	public static function ReplaceNS(&$buffer)
	{
		$subPattern = '([^\s>\[\]"\/]+):([^>\[\]]*)';
		$buffer = preg_replace('/<'.$subPattern.'>/is', '<$2 ns="$1">', $buffer);
		$buffer = preg_replace('/<\/'.$subPattern.'>/is', '</$2>', $buffer);
	...
	public static function MergeCookie(&$arCookies, $arNewCookies)
	{
		...
		if(isset($arCookies['PHPSESSID']) && isset($arNewCookies['PHPSESSID']) && $arNewCookies['PHPSESSID']=='deleted') unset($arNewCookies['PHPSESSID']);
	...
	public static function MakeFileArray($path, $maxTime = 0, $arCookies = array())
	{
		...
		{
			$arTmpFile = $path;
			$temp_path = \Bitrix\Main\IO\Path::convertLogicalToPhysical(\CFile::GetTempName('', $arTmpFile["name"]));
			CheckDirPath($temp_path);
			if(!copy($arTmpFile["tmp_name"], $temp_path)
				&& !move_uploaded_file($arTmpFile["tmp_name"], $temp_path))
			{
				return false;
			}
		...
		else
		{
			...
			{
				$arParams = self::JsObjectToPhp($path);
				...
				if(isset($arParams['FILELINK']))
				{
					...
					{						
						...
						if(in_array($status, array(400, 405)) && array_key_exists('client_secret', $arParams['VARS']))
						{
							...
							if(strpos($res, '{')===0 && ($arAnswer = self::JsObjectToPhp($res)) && is_array($arAnswer) && 
								...
								|| (array_key_exists('message', $arAnswer) && !empty($arAnswer['message']))))
							{
								...
								$client = new \Bitrix\Main\Web\HttpClient(array('disableSslVerification'=>true, 'redirect'=>false));
								$arAnswer = self::JsObjectToPhp(trim($client->post($location, $arParams['VARS'])));
					...
					elseif(!empty($arParams['VARS']) && is_array($arParams['VARS']))
					{
						foreach($arParams['VARS'] as $k=>$v)
						{
							$arPostParams[$k] = $v;
							if(strlen(trim($v)) > 0 && $needEncoding!=$siteEncoding)
							{
								$arPostParams[$k] = \Bitrix\Main\Text\Encoding::convertEncoding($v, $siteEncoding, $needEncoding);
							}
						}
					}
					...
					if(strlen($handler) > 0)
					{
						...
						if(is_array($res))
						{
							...
							if(isset($res['HEADERS']) && is_array($res['HEADERS'])) $arHeaders = array_merge($arHeaders, $res['HEADERS']);
							if(isset($res['POSTPARAMS']) && is_array($res['POSTPARAMS'])) $arPostParams = array_merge($arPostParams, $res['POSTPARAMS']);
						}
						else
						{
							$path = $res;
						...
						))
			{
				...
				//not encode string ?pass=8%5Bs
				//$path = rawurldecode($path);
				$urlencode = !(bool)preg_match('/%[\dA-F]{2}/', $path);
				...
				if(preg_match('/[^A-Za-z0-9\-\.]/', $arUrl['host']))
				{
					...
					if(class_exists('idna_convert'))
					{
						...
						$oldHost = $arUrl['host'];
						if(!self::detectUtf8($oldHost)) $oldHost = \Bitrix\EsolImportxml\Utils::Win1251Utf8($oldHost);
				...
				if(!$bExternalStorage)
				{
					...
					elseif(!empty($arPostParams)) $postBody = $arPostParams;
					...
					if($urlencode) $path = preg_replace_callback('/[^:@\/?=&#%!$,\-\.\+\{\}\[\]]+/', array(__CLASS__, 'UrlEncodeCallback'), $path);
					...
					if(is_array($postBody) || strlen($postBody) > 0)
					{
						if(!is_array($postBody) && strpos($postBody, '<?xml')!==false) $ob->setHeader('content-type', 'application/xml');
					...
					{
						$dRes = $ob->cdownload($path, $temp_path2);
					}
		...
		if(in_array($arFile['type'], array('application/zip', 'application/x-zip-compressed', 'application/gzip', 'application/x-gzip', 'application/x-tar', 'application/rar', 'application/x-rar', 'application/x-rar-compressed', 'application/x-compressed', 'application/octet-stream')) && !in_array($ext, $arExt))
		{
			...
			elseif($ext=='rar')
			{
				...
				{
					$command = 'unrar x "'.$archiveFn.'" '.$tmpsubdir;
					@exec($command);
				}
			}
			...
			else
			{
				...
				{
					self::SysUnzip($archiveFn, $tmpsubdir);
				}
		...
		if(is_array($arFile) && $arFile['size']==0 && !$arFile['ERROR_MESSAGE'])
		{
			return array('ERROR_MESSAGE'=>GetMessage("ESOL_IX_FILE_SIZE_0"));
		}
	...
	public static function SysUnzip($archiveFn, $tmpsubdir)
	{
		$command = 'unzip "'.$archiveFn.'" -d '.$tmpsubdir;
		@exec($command);
		//$command = 'zip test.txt';
		//@exec($command);
	}
	...
	public static function GetNextImportFile($path, $page, $oldFile='', $pid='')
	{
		...
		if(self::PathContianApiPages($path))
		{
			...
			if(is_array($arParams) && isset($arParams['FILELINK']))
			{
				...
				$path = self::PHPToJSObject($arParams);
			}
			$loop = 0;
	...
	public static function IsApiService($path)
	{
		...
		$className = self::GetVendorClassName($arUrl['host']);
		if(is_callable(array($className, 'IsApiService')) && call_user_func(array($className, 'IsApiService'))===true) return true;
		return false;
	}
	...
	public static function PathReplaceApiPages($path, $page=1, $oldFile='')
	{
			...
			if(is_callable(array($client, $method)))
			{
				...
				while(!$isLoad && --$loop>=0)
				{
					try{				
						...
						//file_put_contents(dirname(__FILE__).'/test.txt', print_r($ex, true), FILE_APPEND);
						if(strpos($ex->getMessage(), 'Could not connect to host')!==false)
						{							
							sleep(2);
						}
						else
		...
		elseif(is_callable(array($className, 'IsApiService')) && call_user_func(array($className, 'IsApiService'))===true)
		{
			if(is_callable(array($className, 'GetApiServicePath')))
			{
				$path = call_user_func(array($className, 'GetApiServicePath'), $path, $page);
			}
		}
		return $path;
	}
	...
	public static function GetShowFileBySettings($SETTINGS_DEFAULT)
	{
		...
		if($SETTINGS_DEFAULT["EXT_DATA_FILE"])
		{
			...
			{
				$arParams = self::JsObjectToPhp($SETTINGS_DEFAULT["EXT_DATA_FILE"]);
				if(isset($arParams['FILELINK']))
				{
					$path = $arParams['FILELINK'];
				}
		...
		elseif($SETTINGS_DEFAULT["EMAIL_DATA_FILE"])
		{
			...
			$arParams = self::JsObjectToPhp($json);
			if(!is_array($arParams)) $arParams = self::Unserialize($json);
	...
	public static function AddFileInputActionsHandler(&$content)
	{
		...
		if($pos1!==false && $pos2!==false)
		{
			...
			{
				$json = $m[1];
				$arConfig = self::JsObjectToPhp($json);
				...
				$newJson = self::PHPToJSObject($arConfig);
			...
			<td>
				...
				elseif($arProp["PROPERTY_TYPE"]=='S'):?>
					<select class="esol-ix-filter-chval" name="<?echo $sf;?>[find_el_vtype_property_<?=$arProp["ID"]?>]">
						<option value=""><?echo Loc::getMessage("ESOL_IX_IS_VALUE")?></option>
						<option value="contain"<?if($arFields["find_el_vtype_property_".$arProp["ID"]]=='contain'){echo ' selected';}?>><?echo Loc::getMessage("ESOL_IX_VTYPE_CONTAIN")?></option>
						<option value="not_contain"<?if($arFields["find_el_vtype_property_".$arProp["ID"]]=='not_contain'){echo ' selected';}?>><?echo Loc::getMessage("ESOL_IX_VTYPE_NOT_CONTAIN")?></option>
						<option value="begin_with"<?if($arFields["find_el_vtype_property_".$arProp["ID"]]=='begin_with'){echo ' selected';}?>><?echo Loc::getMessage("ESOL_IX_VTYPE_BEGIN_WITH")?></option>
						<option value="end_on"<?if($arFields["find_el_vtype_property_".$arProp["ID"]]=='end_on'){echo ' selected';}?>><?echo Loc::getMessage("ESOL_IX_VTYPE_END_ON")?></option>
						<option value="empty"<?if($arFields["find_el_vtype_property_".$arProp["ID"]]=='empty'){echo ' selected';}?>><?echo Loc::getMessage("ESOL_IX_IS_EMPTY")?></option>
						<option value="not_empty"<?if($arFields["find_el_vtype_property_".$arProp["ID"]]=='not_empty'){echo ' selected';}?>><?echo Loc::getMessage("ESOL_IX_IS_NOT_EMPTY")?></option>
					</select>
	...
	public static function CorrectCalculatedValue($val)
	{
		...
		if(!self::IsUtfMode() && self::detectUtf8($val))
		{
	...
	{
		$client = new \Bitrix\EsolImportxml\HttpClient(array('socketTimeout'=>10, 'disableSslVerification'=>true));
	...
	public static function DownloadImagesFromText($val, $domain='')
	{
		if(is_array($val))
		{
			foreach($val as $k=>$v)
			{
				$val[$k] = self::DownloadImagesFromText($v, $domain);
			}
			return $val;
		}
		...
		foreach($arPatterns as $k0=>$pattern)
		{
			...
			{
				foreach($m[1] as $k=>$origImg)
				{
					if(strpos($pattern, 'srcset')!==false)
					{
						$arImgs = array();
						foreach(array_map('trim', explode(',', $origImg)) as $subimg)
						{
							$arImgs[] = preg_replace('/\s+\S+$/', '', $subimg);
						}
					}
					else $arImgs = array($origImg);

					$newImg = $origImg;
					foreach($arImgs as $subimg)
					{
						$img = $subimg;
						...
						$newImg = str_replace($subimg, $imgPath1, $newImg);
					}
					//$imgHtml = str_replace($m[1][$k], $imgPath1, $m[0][$k]);
					$imgHtml = str_replace($origImg, $newImg, $m[0][$k]);
	...
	public static function SetConvType0($c)
	{
		...
		$c["CONV_TYPE"]=0;
		if(array_key_exists("INDEX", $c)) unset($c["INDEX"]);
		return $c;
	}
