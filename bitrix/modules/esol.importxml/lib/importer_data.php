...
class ImporterData extends ImporterBase {
	...
	{
		$arElemKeys = $this->baseElemFields;
	...
	public function SaveRecordUpdate(&$arRelProfiles, $IBLOCK_ID, $SECTION_ID, $arElement, $arFields, $arData=array(), $duplicate=false)
	{
		...
		$skipCheckSection = (bool)(!empty($arFieldsElement2['IBLOCK_SECTION']));
			...
			&& $this->conv->SetElementId(0))
		{
			...
			if($this->params['ONLY_CREATE_MODE_PRODUCT']!='Y')
			{
				...
				if($this->params['ELEMENT_ADD_NEW_SECTIONS']=='Y')
				{
					...
					//save main section
					$arFieldsElement2['IBLOCK_SECTION'] = array_unique(array_merge($arElementSections, $arFieldsElement2['IBLOCK_SECTION']));
				}
				if($this->params['ELEMENT_NOT_CHANGE_SECTIONS']!='Y')
				{
					$this->GetSections($arFieldsElement2, $IBLOCK_ID, $SECTION_ID, $arFieldsSections2);
					if(!$skipCheckSection && $this->params['NOT_LOAD_ELEMENTS_WO_SECTION']=='Y' 
						&& (!isset($arFieldsElement2['IBLOCK_SECTION']) || empty($arFieldsElement2['IBLOCK_SECTION'])))
					{
						//$this->logger->SaveElementSkipped('WO_SECTION');
						return true;
					}
				}
	...
	public function SaveRecordAdd($IBLOCK_ID, $SECTION_ID, $arFields, $arItem, $arFilter)
	{
		...
		if($this->params['ONLY_UPDATE_MODE_PRODUCT']!='Y')
		{
			...
			$this->GetDefaultElementFields($arFieldsElement, $iblockFields, $IBLOCK_ID, $arFieldsProps);
			...
			{
				$this->stepparams['error_line']++;
				$this->errors[] = sprintf(Loc::getMessage("ESOL_IX_ADD_ELEMENT_ERROR"), $this->GetLastError(), (strlen($arFieldsElement['NAME']) > 0 ? $arFieldsElement['NAME'] : $arFieldsElement['XML_ID']));
				return false;
			}
		}
		else
	...
	public function SaveRecordOfferUpdate(&$elemName, $ID, $IBLOCK_ID, $OFFERS_IBLOCK_ID, $OFFERS_PROPERTY_ID, $arElement, $arFields, $arData=array(), $duplicate=false)
	{
			...
			&& $this->conv->SetElementId(0))
		{
			...
			if($this->params['ONLY_CREATE_MODE_OFFER']!='Y')
			{
				...
				{
					$this->stepparams['error_line']++;
					$this->errors[] = sprintf(Loc::getMessage("ESOL_IX_UPDATE_OFFER_ERROR"), $this->GetLastError(), $OFFER_ID);
				}
	...
	public function SaveRecordOfferAdd($ID, $NAME, $IBLOCK_ID, $OFFERS_IBLOCK_ID, $OFFERS_PROPERTY_ID, $arFields, $arFilter)
	{
		if($ID && ($this->params['SEARCH_OFFERS_WO_PRODUCTS']!='Y' || $this->params['CREATE_NEW_OFFERS']=='Y'))
		{
			...
			if($this->params['ONLY_UPDATE_MODE_OFFER']!='Y' || ($this->params['SEARCH_OFFERS_WO_PRODUCTS']=='Y' && $this->params['CREATE_NEW_OFFERS']=='Y'))
			{
				...
				$this->GetDefaultElementFields($arFieldsElement, $iblockFields, $OFFERS_IBLOCK_ID, $arFieldsProps);
				...
				{
					$this->stepparams['error_line']++;
					$this->errors[] = sprintf(Loc::getMessage("ESOL_IX_ADD_OFFER_ERROR"), $this->GetLastError(), '');
					return false;
				}
			}
			else
