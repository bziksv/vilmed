			...
			if(is_array($arTrace))
			{
				...
				foreach($arTrace as $traceItem)
				{
					...
					if($traceItem['function']==='ExecuteModuleEventEx' && isset($traceItem['args'][0]['FROM_MODULE_ID']) && isset($traceItem['args'][0]['MESSAGE_ID']))
					{
						...
						$toName = $traceItem['args'][0]['TO_NAME'];
						$iteration = false;
						if($mod=='iblock' && in_array($eventType, $unregEvents))
						{
							...
							{
								if((isset($arEvent['CALLBACK']) && is_string($arEvent['CALLBACK']) && strlen($arEvent['CALLBACK']) > 0 && $arEvent['CALLBACK']==$callback)
									|| (isset($arEvent['TO_NAME']) && is_string($arEvent['TO_NAME']) && strlen($arEvent['TO_NAME']) > 0 && $arEvent['TO_NAME']==$toName))
								{
		...
		if(!isset(self::$elemListHash[$hash]))
		{
			...
			if(class_exists('\Bitrix\Iblock\ElementTable'))
			{
				...
				if($arIblock['WORKFLOW']!='Y')
				{
					...
					elseif(\Bitrix\EsolImportxml\DataManager\ElementPropertyTable::issetValueIndex() && count(preg_grep('/^(IBLOCK_ID|CHECK_PERMISSIONS|[=%]PROPERTY_\d+|[=%]PROPERTY_\d+_VALUE)$/', $arFilterKeys))==count($arFilterKeys) && is_array($arIblock) && $limit===false)
					{
						if($arIblock['VERSION']==1)
						{
							...
							{
								$command = 'namespace Bitrix\EsolImportxml\DataManager;'."\r\n".
			...
			if(class_exists('\Bitrix\Iblock\PropertyEnumerationTable'))
			{
				...
				foreach($arEnumVals as $propKey=>$propVals)
				{
					if(count($propVals) > 0)
					{
						...
						while($arr = $dbRes->Fetch())
						{
							$arEnumValIds[$propKey][ToLower(trim($arr['VALUE']))] = $arr['ID'];
						}
					}
				}
			...
			if(preg_match('/PROPERTY_(\d+)_VALUE$/', $k, $m))
			{
				...
				if(is_array($v)) 
				{
					foreach($v as $k2=>$v2)
					{
						...
						{
							$newVal[$k2] = (isset($arEnumValIds[$propId][ToLower(trim($v2))]) ? $arEnumValIds[$propId][ToLower(trim($v2))] : -1);
						}
					}
				}
				elseif($v!==false && strlen($v) > 0)
				{
					$newVal = (isset($arEnumValIds[$propId][ToLower(trim($v))]) ? $arEnumValIds[$propId][ToLower(trim($v))] : -1);
				}
				unset($arFilter[$k]);
