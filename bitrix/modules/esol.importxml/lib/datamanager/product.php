...
class Product
{
	...
	public function SaveProduct($ID, $IBLOCK_ID, $arProduct, $arPrices, $arStores, $parentID=false, $arOldData=array())
	{
		...
		foreach($arOldProducts as $arCProduct)
		{
			...
			if(array_key_exists('UF_PRODUCT_GROUP', $arProduct) && ToLower($this->GetProductGroupName($arCProduct['ID']))==ToLower(trim($arProduct['UF_PRODUCT_GROUP'])))
			{
				unset($arProduct['UF_PRODUCT_GROUP']);
			}
			if(defined('\Bitrix\Catalog\ProductTable::TYPE_SKU') && $arCProduct['TYPE']==\Bitrix\Catalog\ProductTable::TYPE_SKU && $this->saveProductWithOffers===false)
			{
				...
				if(isset($arProduct['UF_PRODUCT_GROUP'])) ProductD7::SetProductGroup($arCProduct['ID'], $arProduct['UF_PRODUCT_GROUP']);
				continue;
			}
	...
	{
		if($productGroup = $GLOBALS["USER_FIELD_MANAGER"]->GetUserFieldValue("PRODUCT", 'UF_PRODUCT_GROUP', $ID, LANGUAGE_ID))
		{
			...
			return $this->arProductGroups[$productGroup];
		}
		return '';
	}
		...
		foreach($arStores as $sid=>$arFieldsStore)
		{
			...
			foreach($arStoreData as $arPrice)
			{
				...
				if(!empty($arFieldsStore))
				{
					...
					{
						if($this->storeProductD7)
						{
							$update = true;
							foreach(GetModuleEvents("catalog", "OnBeforeStoreProductUpdate", true) as $arEvent)
							{
								if(ExecuteModuleEventEx($arEvent, [$arPrice["ID"], &$arFieldsStore])===false) $update = false;
							}
							if($update)
							{
								$dbRes2 = \Bitrix\Catalog\StoreProductTable::Update($arPrice["ID"], $arFieldsStore);
								if($dbRes2->isSuccess())
								{
									foreach(GetModuleEvents("catalog", "OnStoreProductUpdate", true) as $arEvent)
									{
										ExecuteModuleEventEx($arEvent, [$arPrice["ID"], $arFieldsStore]);
									}
								}
							}
						}
			...
			if(count($arStoreData)==0 && $arFieldsStore['AMOUNT']!=='-')
			{
				...
				if($this->storeProductD7)
				{
					$dbRes2 = \Bitrix\Catalog\StoreProductTable::Add($arFieldsStore);
					if($dbRes2->isSuccess())
					{
						$recId = $dbRes2->GetID();
						foreach(GetModuleEvents("catalog", "OnStoreProductAdd", true) as $arEvent)
						{
							ExecuteModuleEventEx($arEvent, [$recId, $arFieldsStore]);
						}
					}
				}
	...
	public function SetProductQuantity($ID, $IBLOCK_ID=0)
	{
		...
		if($calcPrice)
		{
			$arCalcParams = \Bitrix\EsolImportxml\Utils::Unserialize(\Bitrix\Main\Config\Option::get('iblock', 'KDA_IBLOCK'.$IBLOCK_ID.'_PRICEQNT_CONFORMITY'));
		...
		$arCProduct = $this->GetList(array(), array('ID'=>$ID), false, false, array('ID', 'QUANTITY', 'PURCHASING_PRICE', 'PURCHASING_CURRENCY', 'TYPE', 'SUBSCRIBE', 'SUBSCRIBE_ORIG', 'QUANTITY_TRACE', 'QUANTITY_TRACE_ORIG', 'CAN_BUY_ZERO', 'CAN_BUY_ZERO_ORIG', 'QUANTITY_RESERVED'))->Fetch();
		...
		if($calcPrice)
		{
			...
			foreach($arCalcParams2 as $arCalcParams)
			{
				...
				{
					$dbRes = \Bitrix\Catalog\PriceTable::GetList(array('filter'=>array('PRODUCT_ID'=>$ID, 'CATALOG_GROUP_ID'=>$arPriceKeys), 'select'=>array('CATALOG_GROUP_ID', 'PRICE', 'CURRENCY')));
					while($arr = $dbRes->Fetch())
					{
						$arFields['ICAT_PRICE'.$arr['CATALOG_GROUP_ID']] = $this->GetFloatVal($arr['PRICE']);
						$arFields['ICAT_PRICE'.$arr['CATALOG_GROUP_ID'].'_CURRENCY'] = $arr['CURRENCY'];
					}
				}
				
				$price = $priceNA = $compPrice = $compPriceNA = '';
				$priceField = $priceNAField = '';
				$currency = $currencyNA = false;
				$discount = $discountNA = false;
				foreach($arCalcParams['MAP'] as $arMap)
				{
					$curPrice = false;
					$curCurrency = false;
					if(isset($arFields[$arMap['price']]))
					{
						...
						$compPrice = $price;
						$compPriceNA = $priceNA;
						if(isset($arFields[$arMap['price'].'_CURRENCY']))
						{
							$curCurrency = $arFields[$arMap['price'].'_CURRENCY'];
							if($currency && $curCurrency!=$currency)
							{
								$compPrice = \CCurrencyRates::ConvertCurrency($compPrice, $currency, $curCurrency);
								$compPriceNA = \CCurrencyRates::ConvertCurrency($compPriceNA, $currency, $curCurrency);
							}
						}
					}
					if($curPrice > 0
						&& ($arCalcParams['PARAMS']['ONLY_AVAILABLE']=='N' || (isset($arFields[$arMap['quantity']]) && $arFields[$arMap['quantity']] > 0))
						&& ((float)$compPrice<=0 || ($arCalcParams['PARAMS']['PRICE_CALC']=='MIN' && $curPrice <= $compPrice) || ($arCalcParams['PARAMS']['PRICE_CALC']=='MAX' && $curPrice >= (float)$compPrice)))
					{
						$price = $curPrice;
						$currency = $curCurrency;
						$priceField = $arMap['price'];
						if($arCalcParams['PARAMS']['QUANTITY_CALC']=='FROM_PRICE')
						{
							if($curPrice==$compPrice) $quantity = max($quantity, $arFields[$arMap['quantity']]);
							else $quantity = $arFields[$arMap['quantity']];
						}
					...
					if($curPrice > 0
						&& $arCalcParams['PARAMS']['ONLY_AVAILABLE']=='Y'
						&& ((float)$compPriceNA<=0 || ($arCalcParams['PARAMS']['PRICE_CALC']=='MIN' && $curPrice <= $compPriceNA) || ($arCalcParams['PARAMS']['PRICE_CALC']=='MAX' && $curPrice >= (float)$compPriceNA)))
					{
						$priceNA = $curPrice;
						$currencyNA = $curCurrency;
						$priceNAField = $arMap['price'];
						if(array_key_exists($arMap['price'].'|DISCOUNT', $arFields))
						{
							$discountNA = $arFields[$arMap['price'].'|DISCOUNT'];
						}
					}
				...
				if(strlen($price)==0 && strlen($priceNA) > 0 && $arCalcParams['PARAMS']['PRICE_EMPTY_ACTION']=='SALE_OLD')
				{
					$price = $priceNA;
					$currency = $currencyNA;
					$priceField = $priceNAField;
				...
				if(strlen($price) > 0 || $arCalcParams['PARAMS']['PRICE_EMPTY_ACTION']=='DELETE')
				{
					...
					{
						$arUpdateFields = $arOldFields = array();
						if($this->params['ELEMENT_IMAGES_FORCE_UPDATE']=='Y' || $arCProduct['PURCHASING_PRICE']!=$price)
						{
							$arUpdateFields['PURCHASING_PRICE'] = $price;
							$arOldFields['PURCHASING_PRICE'] = $arCProduct['PURCHASING_PRICE'];
						}
						if($currency && ($this->params['ELEMENT_IMAGES_FORCE_UPDATE']=='Y' || $currency!=$arCProduct['PURCHASING_CURRENCY']))
						{
							$arUpdateFields['PURCHASING_CURRENCY'] = $currency;
							$arOldFields = array('PURCHASING_CURRENCY'=>$arCProduct['PURCHASING_CURRENCY']);
						}
						if(count($arUpdateFields) > 0)
						{
							$this->Update($ID, $IBLOCK_ID, $arUpdateFields);
							$this->logger->AddElementChanges('ICAT_', $arUpdateFields, $arOldFields);
						}
					}
					elseif($arCalcParams['PARAMS']['PRICE_TYPE'] > 0)
					{
						$arUpdateFields = array('PRICE'=>$price);
						if($currency) $arUpdateFields['CURRENCY'] = $currency;
						$this->pricer->SavePrice($ID, array($arCalcParams['PARAMS']['PRICE_TYPE']=>$arUpdateFields));
		...
		if($arCProduct)
		{
			if(!$this->useStoreReserved && isset($arCProduct['QUANTITY_RESERVED']) && $arCProduct['QUANTITY_RESERVED'] > 0) $quantity -= (int)$arCProduct['QUANTITY_RESERVED'];
	...
	public function SaveFieldCalcPrice($ID, $IBLOCK_ID, $priceType, $priceField)
	{
		...
		if(!array_key_exists($priceType, $this->priceCalcProps))
		{
			...
			if($arProp = \Bitrix\Iblock\PropertyTable::getList(array('filter'=>array('=IBLOCK_ID'=>$IBLOCK_ID, '=CODE'=>'ESOL_IMPORT_CALC_PRICE_TYPE_'.$priceType), 'select'=>array('ID')))->Fetch())
			{
