...
class Price
{
	...
	public function SavePrice($ID, $arPrices, $isOffer = false, $arOldPrices=false)
	{
		...
		foreach($arPrices as $gid=>$arFieldsPrice)
		{
			...
			foreach($arSubPrices as $arSubPrice)
			{
				...
				if(isset($arFieldsPrice['PRICE']))
				{
					while(is_array($arFieldsPrice['PRICE'])) $arFieldsPrice['PRICE'] = current($arFieldsPrice['PRICE']);
				...
				{
					$bDeleteOld = false;
					if($arFieldsPrice['PRICE']!=='-' && (array_key_exists('PRICE', $arFieldsPrice) || array_key_exists('EXTRA_ID', $arFieldsPrice)))
					{
						$arFieldsPrice['PRODUCT_ID'] = $ID;
						$arFieldsPrice['CATALOG_GROUP_ID'] = $gid;
	...
	public function GetCurrencyVal($val)
	{
		...
		{
			if(((strpos($val, Loc::getMessage('ESOL_IX_PRICE_CURRENCY_RUB'))!==false && $this->params['DEFAULT_CURRENCY']!='BYN') || strpos($val, '₽')!==false || strpos($val, 'RUR')!==false) && isset($this->arCurrencies['RUB'])) $val = 'RUB';
