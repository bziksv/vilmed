...
class ImporterHl extends ImporterHlData {	
	...
	public function GetNextImportFile()
	{
		...
		if(array_key_exists('EXT_DATA_FILE', $this->params) && ($fid = \Bitrix\EsolImportxml\Utils::GetNextImportFile($this->params['EXT_DATA_FILE'], $page, $this->params['URL_DATA_FILE'], $this->pid)))
		{
			...
			$oProfile = \Bitrix\EsolImportxml\Profile::getInstance('highload')->UpdatePartSettings($this->pid, array('DATA_FILE'=>$fid, 'URL_DATA_FILE'=>$filename, 'OLD_FILE_SIZE'=>(int)filesize($this->filename)));
	...
	{
		$bElemDeactivate = (bool)($this->params['CELEMENT_MISSING_REMOVE_ELEMENT']=='Y');
		...
		$oProfile = \Bitrix\EsolImportxml\Profile::getInstance('highload');
		$oProfile->UpdateFileHash($this->pid, $this->filename, true);
	...
	public function GetMissingFilter($HLBL_ID = 0)
	{
		...
		if($this->params['CELEMENT_MISSING_REMOVE_ELEMENT']=='Y') return array();
	...
	public function SaveRecord($arItem)
	{
		...
		{
			if($this->SaveRecordAdd($entityDataClass, $arFieldsElement)===false)
			{
				return false;
			}
		}
	...
	public function GetCurrencyRates()
	{
		if(!isset($this->currencyRates))
		{
			...
			{
				$arRates = \Bitrix\EsolImportxml\Utils::Unserialize(file_get_contents($currFile));
			}
			else
			{
