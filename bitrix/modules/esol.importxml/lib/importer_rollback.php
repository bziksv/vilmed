...
class ImporterRollback extends Importer {
	...
	function __construct($arParams)
	{
		...
		$this->params = array();
		$this->logger = new \Bitrix\EsolImportxml\Logger();
	...
	public function Proccess()
	{
		...
		while(!$break)
		{
			...
			$dbRes = \Bitrix\EsolImportxml\ProfileExecStatTable::getList($this->getListParams);
	...
	public function RestoreRecord($arRecord)
	{
		...
		$arFields = \Bitrix\EsolImportxml\Utils::Unserialize($arRecord['FIELDS'], true);
	...
	public function RestoreSectionUpdate($ID, $arFields)
	{
		...
		if(isset($arFields['IPROPERTY_TEMPLATES']) && empty($arFields['IPROPERTY_TEMPLATES'])) $arFields['IPROPERTY_TEMPLATES'] = array();
	...
	public function RestoreElementUpdate($ID, $arFields)
	{
		...
		{
			$this->PrepareProps($arFieldsProps);
	...
	public function PrepareProps(&$arProps)
	{
		$arKeys = array_diff(array_keys($arProps), array_keys($this->arPropTypes));
		if(count($arKeys) > 0)
		{
			$dbRes = \Bitrix\Iblock\PropertyTable::getList(array('filter'=>array('ID'=>$arKeys), 'select'=>array('ID', 'PROPERTY_TYPE')));
			while($arr = $dbRes->Fetch())
			{
				$this->arPropTypes[$arr['ID']] = $arr['PROPERTY_TYPE'];
			}
		}
		foreach($arProps as $propId=>$propVal)
		{
			if($this->arPropTypes[$propId]=='F') unset($arProps[$propId]);
		}
	}
}	
?>