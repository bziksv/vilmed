...
class Logger {
	...
	private $saveNotFoundElements = false;
	private $saveSkippedElements = false;
	...
	function __construct($saveLog = false, $profileId = 0)
	{
		if(is_array($saveLog))
		{
			...
			$this->saveNotFoundElements = (bool)($saveLog['STAT_SAVE_NOT_FOUND_ELEMENTS']=='Y');
			$this->saveSkippedElements = (bool)($saveLog['STAT_SAVE_SKIPPED_ELEMENTS']=='Y');
	...
	public function SaveElementChangesMass()
	{
		...
		foreach($this->arElements as $arElem)
		{
			...
			$arVals[] = "(".(int)$arElem['PROFILE_ID'].", ".(int)$arElem['PROFILE_EXEC_ID'].", ".$date.", '".$helper->forSql($arElem['TYPE'])."', ".(int)$arElem['ENTITY_ID'].", '".$helper->forSql($arElem['FIELDS'])."')";
		}
	...
	{
		if(!$this->saveLog || !$this->execId || !$this->saveNotFoundElements) return false;
	...
	public function NeedSaveElementSkipped()
	{
		if($this->saveLog && $this->execId && $this->saveSkippedElements) return true;
		return false;
	}
	
	public function SaveElementSkipped($reason, $fieldName='', $arItem=array(), $IBLOCK_ID=0)
	{
		if(!$this->NeedSaveElementSkipped()) return false;
		
		if(!array_key_exists('IE_IBLOCK_ID', $arItem)) $arItem['IE_IBLOCK_ID'] = $IBLOCK_ID;
		$arFields = array(
			'PROFILE_ID' => $this->profileId,
			'PROFILE_EXEC_ID' => $this->execId,
			'DATE_EXEC' => new \Bitrix\Main\Type\DateTime(),
			...
			'ENTITY_ID' => 0,
			'FIELDS' => serialize(array('REASON'=>$reason, 'FIELD_NAME'=>$fieldName, 'FIELDS'=>$arItem))
		);
	...
	public function GetFieldLangName($name, $IBLOCK_ID=0)
	{
		$this->PrepareFieldList();
		$arFieldNames = $this->fl->GetIblockElementFieldsForStat($IBLOCK_ID);
		return (isset($arFieldNames[$name]) ? $arFieldNames[$name]['name'] : $name);
	}
	
	public function GetElementDescriptionArray($description)
	{
		if(!$description) return '';
		$arFields = (!is_array($description) ? \Bitrix\EsolImportxml\Utils::Unserialize(htmlspecialcharsback($description)) : $description);
		$val = '<pre>'.print_r($arFields, true).'</pre>';
		//$val = str_replace("\t", '<span style="display: inline-block; width: 15px;"></span>', $val);
		return $val;
	}
	
	public function GetElementDescription($description, $showOriginal=false)
	{
		if(!$description) return '';
		$this->PrepareFieldList();
		
		$arFields = (!is_array($description) ? \Bitrix\EsolImportxml\Utils::Unserialize(htmlspecialcharsback($description), true) : $description);
	...
	public function GetSectionDescription($description, $IBLOCK_ID = false)
	{
		...
		$arFields = \Bitrix\EsolImportxml\Utils::Unserialize(htmlspecialcharsback($description));
