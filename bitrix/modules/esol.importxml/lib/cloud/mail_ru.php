...
class MailRu
{
    ...
    /*function getDownloadToken()
    {
        ...
        if ($result !== 'error') {
			$result = \Bitrix\EsolImportxml\Utils::JsObjectToPhp($result);
	...
	public function download(&$tmpPath, $path, $fragment='')
	{
		...
		else
		{
			...
			$tmpPath = \Bitrix\EsolImportxml\Cloud::GetTmpFilePath('folder.zip');
	...
	public function DownloadFile(&$tmpPath, $link)
	{
		...
		if($client && $client->getStatus()==200)
		{
			...
			if($hcd = $headers->get('content-disposition'))
			{
				...
				{
					$fn = urldecode($fn);
					if(!\Bitrix\EsolImportxml\Utils::IsUtfMode() && \CUtil::DetectUTF8($fn))
					{
						$fn = \Bitrix\Main\Text\Encoding::convertEncoding($fn, 'UTF-8', 'CP1251');
					}
					$tmpPath = \Bitrix\EsolImportxml\Cloud::GetTmpFilePath($fn);
				}
			}
