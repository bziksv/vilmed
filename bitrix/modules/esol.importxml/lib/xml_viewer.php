...
class XMLViewer 
{
	...
	public function GetXpathVals($xpath, $parentXpath='', $arFieldParams=array(), $arProfileParams=array())
	{
		...
		if(strlen($parentXpath) > 0)
		{
			...
			foreach($arConv as $k=>$v)
			{
				...
				if(!$isExpressions && $v['THEN']=='EXPRESSION' && strlen(trim($v['TO'])) > 0) $isExpressions = true;
			}
			
			if($isExpressions && !$GLOBALS['USER']->IsAdmin())
			{
				$arExpressions = array();
				if(is_numeric($this->profileId))
				{
					$oProfile = new \Bitrix\EsolImportxml\Profile();
					$arProfile = $oProfile->GetByID($this->profileId);
				...
				foreach($arConv as $k=>$v)
				{
					if($v['THEN']=='EXPRESSION' && !in_array($v['TO'], $arExpressions))
					{
						$arConv[$k]['TO'] = '';
					}
				}
			...
			$this->StartGetXpathRowsPart();
			$loop = true;
			while($loop)
			{
				$loop = $this->GetXpathRowsPart($rows, $parentXpath, $arXpaths);
				if(is_array($rows))
				{
					foreach($rows as $row)
					{
		...
		else
		{
			...
			while($loop)
			{
				$loop = $this->GetXpathRowsPart($rows, $xpath);
				if(is_array($rows))
				{
					$attr = false;
					$arPath = explode('/', $xpath);
	...
	public function StartGetXpathRowsPart()
	{
		if(class_exists($this->xmlReader))
		{
			$this->currentXml = Utils::GetXmlReaderObject($this->filename);
			$this->currentObjectNames = array();
			$this->currentRead = false;
		}
	}
	
	public function EndGetXpathRowsPart()
	{
		if(class_exists($this->xmlReader))
		{
			if($this->currentXml) $this->currentXml->close();
			$this->currentXml = null;
			$this->currentObjectNames = null;
			$this->currentRead = null;
		}
	}
	
	public function GetXpathRowsPart(&$rows, $xpath, $wChild=false, $xpathsMulti=array(), $uniqueXPath='')
	{
		$cnt = 0;
		$maxCnt = 1000;
		$rows = array();
		...
		{
			if(strlen($xpathsMulti) > 0) $xpathsMulti = \Bitrix\EsolImportxml\Utils::Unserialize(base64_decode($xpathsMulti));
			else $xpathsMulti = array();
		}
		if(!is_array($xpathsMulti)) $xpathsMulti = array();
		
		$xpath = trim(trim($xpath), '/');
		if(strlen($xpath) == 0) return false;
		$xpathOrig = $xpath;
		...
		if(!$xml) return false;
		$arObjects = array();
		$arObjectNames = $this->currentObjectNames;
		
		foreach(array_slice($this->currentObjectNames, 0, -1) as $curDepth=>$curName)
		{
			if($curDepth==0)
			{
				$rootNS = '';
		...
		while(($isRead || $xml->read()) && !$break) 
		{
			...
			if($xml->nodeType == $this->xmlReader::ELEMENT) 
			{
				...
				elseif($curXPath==$xpath && ++$cnt>$maxCnt)
				{
					$break = true;
					$end = false;
					$isRead = $this->currentRead = true;
					continue;
				}
		...
		$this->currentObjectNames = $arObjectNames;

		if(is_object($xmlObj))
		{
			//return $xmlObj->xpath('/'.$xpathOrig);
			$rows = $this->Xpath($xmlObj, '/'.$xpathOrig);
	...
	public function GetPropertyList($xpath, $arFields, $isOffers=false, $arPost=array())
	{
		...
		if(!empty($arPost))
		{
			...
			$arSectionMap = \Bitrix\EsolImportxml\Utils::Unserialize(base64_decode($arPost['SECTION_MAP']));
			$arCatIds = array();
	...
	public function GetArrByXpath($simpleXmlObj, $xpath)
	{
		...
		foreach($val as $k=>$v)
		{
			...
			if($this->params['HTML_ENTITY_DECODE']=='Y')
			{
				$val[$k] = html_entity_decode($val[$k], ENT_QUOTES | ENT_HTML5, $this->siteEncoding);
			}
		}
		return $val;
	}
