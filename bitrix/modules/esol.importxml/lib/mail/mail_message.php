...
class MailMessage
{
	...
	private static function decodeMessageBody($header, $body, $charset)
	{
		...
		{
			if($header->charset) $body = \Bitrix\EsolImportxml\MailUtil::convertCharset($body, $header->charset, $charset);
		}
	...
	{
		$headerP = Utils::BinStrpos($message, "\r\n\r\n");

		$rawHeader = Utils::BinSubstr($message, 0, $headerP);
		$body      = Utils::BinSubstr($message, $headerP+4);
		...
		if ($header->IsMultipart())
		{
			...
			$startP = Utils::BinStrpos($message, $startB)+Utils::BinStrlen($startB);
			$endP   = Utils::BinStrpos($message, $endB);

			$data = Utils::BinSubstr($message, $startP, $endP-$startP);
