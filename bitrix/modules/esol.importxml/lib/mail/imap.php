...
class Imap
{
	...
	public function SupportExtCriteria()
	{
		...
		if(ToLower($this->options['host'])=='imap.mail.ru')
		{
			...
			//imap search working
			//slow loading of large numbers of emails
		}
		return $support;
	}
	...
	public function listMailboxes($mailbox, &$error)
	{
		...
		foreach ($this->getUntagged($regex, true) as $item)
		{
			...
			if (preg_match('/^ \{ ( \d+ ) \} $/ix', $sname, $literal))
				$sname = Utils::BinSubstr($item, Utils::BinStrlen($matches[0]), $literal[1]);
	...
	public function getMessage($mailbox, $id, $section, &$error)
	{
		...
		foreach ($this->getUntagged($regex, true) as $item)
		{
			...
			{
				$sbody = Utils::BinSubstr($data, $literal[0][1]+Utils::BinStrlen($literal[0][0]), $literal['size'][0]);
				continue;
			}
		}
		...
		if($section=='HEADER')
		{
			list($header, $htmlBody, $textBody, $parts) = \Bitrix\EsolImportxml\MailMessage::parseMessage($sbody, $this->options['encoding']);
			return $header;
		}
		...
		if($fullData)
		{
			...
			if(strlen(trim($htmlBody))==0) $htmlBody = \Bitrix\Main\Text\Encoding::convertEncoding(base64_decode($sbody), $this->options['encoding'], \Bitrix\EsolImportxml\Utils::getSiteEncoding());
			return array(
				'TEXT' => $htmlBody,
...
//echo '< ', $line;
	...
	protected static function prepareString($data)
	{
		if (preg_match('/^[^\x00\x0a\x0d\x80-\xff]*$/', $data))
			...
			return sprintf("{%u}\x00%s", Utils::BinStrlen($data), $data);
	}
	...
	public static function decodeError($code)
	{
		switch ($code)
		{
			case self::ERR_CONNECT:
				return Loc::getMessage('ESOL_IX_IMAP_ERR_CONNECT');
			case self::ERR_REJECTED:
				return Loc::getMessage('ESOL_IX_IMAP_ERR_REJECTED');
			case self::ERR_COMMUNICATE:
				return Loc::getMessage('ESOL_IX_IMAP_ERR_COMMUNICATE');
			case self::ERR_EMPTY_RESPONSE:
				return Loc::getMessage('ESOL_IX_IMAP_ERR_EMPTY_RESPONSE');
			case self::ERR_BAD_SERVER:
				return Loc::getMessage('ESOL_IX_IMAP_ERR_BAD_SERVER');
			case self::ERR_STARTTLS:
				return Loc::getMessage('ESOL_IX_IMAP_ERR_STARTTLS');
			case self::ERR_COMMAND_REJECTED:
				return Loc::getMessage('ESOL_IX_IMAP_ERR_COMMAND_REJECTED');
			case self::ERR_CAPABILITY:
				return Loc::getMessage('ESOL_IX_IMAP_ERR_CAPABILITY');
			case self::ERR_AUTH:
				return Loc::getMessage('ESOL_IX_IMAP_ERR_AUTH');
			case self::ERR_AUTH_MECH:
				return Loc::getMessage('ESOL_IX_IMAP_ERR_AUTH_MECH');
			case self::ERR_LIST:
				return Loc::getMessage('ESOL_IX_IMAP_ERR_LIST');
			case self::ERR_SELECT:
				return Loc::getMessage('ESOL_IX_IMAP_ERR_SELECT');
			case self::ERR_SEARCH:
				return Loc::getMessage('ESOL_IX_IMAP_ERR_SEARCH');
			case self::ERR_FETCH:
				return Loc::getMessage('ESOL_IX_IMAP_ERR_FETCH');
			case self::ERR_APPEND:
				return Loc::getMessage('ESOL_IX_IMAP_ERR_APPEND');
			case self::ERR_STORE:
				return Loc::getMessage('ESOL_IX_IMAP_ERR_STORE');

			default:
				return Loc::getMessage('ESOL_IX_IMAP_ERR_DEFAULT');
		}
	}
}