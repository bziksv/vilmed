...
class SMail
{
	...
	public function __construct($params=array())
	{
		...
		if(preg_match('/[^A-Za-z0-9\-\.]/', $arEmailParts[1]))
		{
			...
			if(class_exists('\idna_convert'))
			{
				...
				if(!Utils::DetectUTF8($oldHost)) $oldHost = \Bitrix\EsolImportxml\Utils::Win1251Utf8($oldHost);
	...
	public function CheckParams()
	{
		if(!$this->paramsChecked)
		{
			...
			$charset = (\Bitrix\EsolImportxml\Utils::IsUtfMode() ? 'UTF-8' : 'CP1251');
			...
			while(!empty($arFolders) && !$fid)
			{
				...
				if(!empty($mailsIds))
				{
					...
					while($i>=0 && !$break)
					{
						...
						{
							if($arMailFile['DATE']) $arMailFile['DATE'] = trim(preg_replace('/\([^\)]*\)/', '', $arMailFile['DATE']));
	...
	public static function GetNewFile(&$json, $maxTime = 0, $extId = '')
	{
		...
		$arParams = \Bitrix\EsolImportxml\Utils::JsObjectToPhp($json);
		if(!is_array($arParams)) $arParams = \Bitrix\EsolImportxml\Utils::Unserialize($json);
