...
class Cloud
{
	...
	public static function YadiskGetLinksByMask($path)
	{
		...
		if(strlen($fragment) > 0)
		{
			...
			$arRes = \Bitrix\EsolImportxml\Utils::JsObjectToPhp($res);
	...
	{
		$token = \Bitrix\Main\Config\Option::get(static::$moduleId, 'YANDEX_APIKEY', '');
		if(!$token) return array('ERROR_MESSAGE'=>sprintf(Loc::getMessage("ESOL_IX_YANDEX_APIKEY_NOT_DEFINED"), '/bitrix/admin/settings.php?lang=ru&mid_menu=1&mid='.self::$moduleId.'#yandex_token'));
	...
	public function YadiskGetFileByYaLink(&$tmpPath, $fileLink)
	{
		...
		if(strlen($fileLink) > 0)
		{
			...
			if(count($filename)==1)
			{
				...
				if(!\Bitrix\EsolImportxml\Utils::IsUtfMode()) $filename = $GLOBALS['APPLICATION']->ConvertCharset($filename, 'UTF-8', 'CP1251');
		...
		if($res && $client->getStatus()!=404)
		{
			...
			if($hcd && stripos($hcd, 'filename='))
			{
				...
				if(count($hcdParts1) > 0)
				{
					...
					if(!\Bitrix\EsolImportxml\Utils::IsUtfMode()) $fn = $GLOBALS['APPLICATION']->ConvertCharset($fn, 'UTF-8', 'CP1251');
					$fn = preg_replace('/[?]/', '', $fn);
	...
	public function GdriveGetAccessToken(&$arFile, $id, $type='file')
	{
		...
		if($accessToken)
		{
			...
			$res = $ob->get($apiPath);
			$arFile = \Bitrix\EsolImportxml\Utils::JsObjectToPhp($res);
			if($arFile['error'])
			{
				$accessToken = '';
	...
	public function PostimgGetFile(&$tmpPath, $path, $fromFile=false)
	{
		$fileTypes = self::GetFileTypes($fromFile);
		$client = new \Bitrix\Main\Web\HttpClient(array('socketTimeout'=>10, 'disableSslVerification'=>true));
		$client->setHeader('User-Agent', \Bitrix\EsolImportxml\Utils::GetUserAgent());
