...
class Extrasettings {
	...
	public static function GetMarginTemplates(&$pfile)
	{
		...
		if(file_exists($pfile)) $arTemplates = \Bitrix\EsolImportxml\Utils::Unserialize(file_get_contents($pfile));
	...
	{
		global $APPLICATION;
		if($checkCharset && !\Bitrix\EsolImportxml\Utils::IsUtfMode())
		{
				...
				if($k2=='CONVERSION' || $k2=='EXTRA_CONVERSION')
				{
					...
					foreach($v2['WHEN'] as $k3=>$v3)
					{
						if(strlen($v2['FROM'][$k3]) > 0 || strlen($v2['TO'][$k3]) > 0 
							|| in_array($v2['CELL'][$k3], array('ELSE', 'LOADED', 'DUPLICATE'))
							|| in_array($v2['WHEN'][$k3], array('ANY', 'EMPTY', 'NOT_EMPTY'))
							|| in_array($v2['THEN'][$k3], array('NOT_LOAD', 'NOT_LOAD_ELEMENT', 'MATH_ROUND', 'TRANSLIT', 'STRIP_TAGS', 'CLEAR_TAGS')))
						{
							$arConversion = array(
								'CELL' => $v2['CELL'][$k3],
								'WHEN' => $v3,
							...
							if(array_key_exists('INDEX', $v2))
							{
								$arConversion['INDEX'] = $v2['INDEX'][$k3];
							}
