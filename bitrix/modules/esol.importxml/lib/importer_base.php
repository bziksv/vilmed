...
class ImporterBase {
	...
	var $baseElemFields = array('ID', 'NAME', 'IBLOCK_SECTION_ID', 'PREVIEW_PICTURE', 'WF_STATUS_ID', 'ACTIVE');
	...
	public function GetFileName()
	{
		if(!file_exists($this->filename))
		{
			...
			if($fid)
			{
				$arFile = \Bitrix\EsolImportxml\Utils::GetFileArray($fid);
	...
	public function GetNextImportFile()
	{
		...
		if(array_key_exists('EXT_DATA_FILE', $this->params) && ($fid = \Bitrix\EsolImportxml\Utils::GetNextImportFile($this->params['EXT_DATA_FILE'], $page, $this->params['URL_DATA_FILE'], $this->pid)))
		{
			...
			$oProfile = \Bitrix\EsolImportxml\Profile::getInstance()->UpdatePartSettings($this->pid, array('DATA_FILE'=>$fid, 'URL_DATA_FILE'=>$filename, 'OLD_FILE_SIZE'=>(int)filesize($this->filename)));
			...
			$this->stepparams['count_rows'] = array();
			return true;
		}
		return false;
	}
	...
	public function CheckRelProfiles($arRelProfiles)
	{
		...
		foreach($arRelProfiles as $p)
		{
			...
			if(isset($pParams['SETTINGS_DEFAULT']['EXT_DATA_FILE']) && preg_match('/^\{.*\}$/s', $pParams['SETTINGS_DEFAULT']['EXT_DATA_FILE']) && ($arFileParams = \Bitrix\EsolImportxml\Utils::JsObjectToPhp($pParams['SETTINGS_DEFAULT']['EXT_DATA_FILE'])) && is_array($arFileParams))
			{
				$arFileParams['FILELINK'] = $p['LINK'];
				$p['LINK'] = \Bitrix\EsolImportxml\Utils::PHPToJSObject($arFileParams);
			}
	...
	public function GetXmlObject(&$countRows, $beginRow, $xpath, $nolimit = false)
	{
		...
		if($close) $xml->close();
		if(function_exists('libxml_get_last_error') && ($xmlError = libxml_get_last_error()) && in_array($xmlError->level, array(LIBXML_ERR_ERROR, LIBXML_ERR_FATAL)) && $xmlError->file && ((int)$this->stepparams['api_page'] < 2 || is_object($xmlObj)))
		{
	...
	public function ApplyConversions($val, $arConv, $arItem, $field=false, $iblockFields=array())
	{
		...
		if(is_array($arConv))
		{
			...
			$subPattern = '#VAL#|#HASH#|#FILELINK#|#FILEDATE#|#DATETIME#|#DATETIME[\+\-]\d+#|#API_PAGE#|'.implode('|', $this->rcurrencies);
			$prefixPattern1 = '/(('.$subPattern.')|'.
				'\$\{[\'"]('.$subPattern.')[\'"]\})/';
			$prefixPattern2 = '/(\{([^\s\'"\{\}\$]+[\'"][^\'"\{\}]*[\'"])*[^\s\'"\{\};]+\}|'.
				'\$\{[\'"]([^\s\{\}]*[\'"][^\'"\{\}]*[\'"])*[^\s\'"\{\}]*[\'"]\})/';
			foreach($arConv as $k=>$v)
			{
				...
				$i = 0;
				while(++$i<10 && strlen($v['FROM']) > 0 && preg_match($prefixPattern1, $v['FROM']))
				{
					$v['FROM'] = preg_replace_callback($prefixPattern1, array($this, 'ConversionReplaceValues'), $v['FROM']);
				}
				while(++$i<10 && $v['WHEN']!='REGEXP' && strlen($v['FROM']) > 0 && preg_match($prefixPattern2, $v['FROM']))
				{
					$v['FROM'] = preg_replace_callback($prefixPattern2, array($this, 'ConversionReplaceValues'), $v['FROM']);
				}
				...
				{
					$i = 0;
					while(++$i<10 && strlen($v['TO']) > 0 && preg_match($prefixPattern1, $v['TO']))
					{
						$v['TO'] = preg_replace_callback($prefixPattern1, array($this, 'ConversionReplaceValues'), $v['TO']);
					}
					while(++$i<10 && strlen($v['TO']) > 0 && preg_match($prefixPattern2, $v['TO']))
					{
						$v['TO'] = preg_replace_callback($prefixPattern2, array($this, 'ConversionReplaceValues'), $v['TO']);
					}
					...
					elseif($v['THEN']=='MATH_MULTIPLY') $val = \Bitrix\EsolImportxml\Utils::GetFloatRoundVal($this->GetFloatVal($val) * $this->CalcFloatValue($v['TO'], true));
					elseif($v['THEN']=='MATH_DIVIDE') $val = ($this->GetFloatVal($v['TO'])==0 ? 0 : \Bitrix\EsolImportxml\Utils::GetFloatRoundVal($this->GetFloatVal($val) / $this->CalcFloatValue($v['TO'], true)));
					elseif($v['THEN']=='MATH_ADD') $val = \Bitrix\EsolImportxml\Utils::GetFloatRoundVal($this->GetFloatVal($val) + $this->CalcFloatValue($v['TO'], true));
					elseif($v['THEN']=='MATH_SUBTRACT') $val = \Bitrix\EsolImportxml\Utils::GetFloatRoundVal($this->GetFloatVal($val) - $this->CalcFloatValue($v['TO'], true));
					...
					elseif($v['THEN']=='MATH_SUBTRACT_PERCENT') (strlen($val) > 0 ? \Bitrix\EsolImportxml\Utils::GetFloatRoundVal($val = $this->GetFloatVal($val) * (1 - $this->GetFloatVal($v['TO'])/100)) : '');
					...
					elseif($v['THEN']=='TRANSLIT')
					{
						...
						if(is_array($val))
						{
							...
							{
								$val[$k2] = $this->Str2Url($v2, $arParams);
							}
						}
						else
						{
	...
	public function SaveStatusImport($end = false)
	{
		...
		if($this->stepparams['IMPORT_MODE']=='CRON')
		{
			if(($time = time()) < $this->timeSaveResult + 15) return;
			$this->GetBreakParams('cron_save');
		}
		...
		if($this->procfile)
		{
			...
			{
				if(stripos($k, 'cstm_')===0) unset($stepparams[$k]);
			}
			*/
			$writeParams = array_merge($stepparams, array(
				'xmlCurrentRow' => intval($this->xmlCurrentRow),
				'xmlSectionCurrentRow' => intval($this->xmlSectionCurrentRow),
				//'sectionIds' => $this->sectionIds
			));
			$writeParams['action'] = ($end ? 'finish' : 'continue');
			file_put_contents($this->procfile, \Bitrix\EsolImportxml\Utils::PhpToJSObject($writeParams));
		}
	}
	...
	public function GetFileArray($file, $arDef=array(), $arParams=array(), $oldId=0, $origVal=false)
	{
		...
		{
			try{
				$arFile = self::GetFileArrayDirect($file, $arDef, $arParams, $oldId);
				$attempts = 0;
				...
				if($attempts==0)
				{
					...
					if(!$this->checkDiskSpace && (strpos($mess, 'is not a valid file-handle resource')!==false || strpos($mess, 'cannot open the file')!==false))
					{
						...
						{
							$extraMess = Loc::getMessage("ESOL_IX_FILE_ERROR_CHECK_DISK_SPACE");
					...
					$this->errors[] = (strlen($extraMess) > 0 ? $extraMess.'<br>' : '').sprintf(Loc::getMessage("ESOL_IX_FILE_ERROR"), $file).'<br>['.$ex->getMessage().']';
				}
			}
		}
		...
		if($origVal===false && empty($arFile) && strlen($file) > 0 && $file!='-' && strpos($file, $this->params['ELEMENT_MULTIPLE_SEPARATOR'])===false)
		{
			...
			if($this->lastFileError)
			{
				...
				{
					$errorText = sprintf(Loc::getMessage("ESOL_IX_FILE_ERROR"), $file).' '.sprintf(Loc::getMessage("ESOL_IX_FILE_ERROR_STATUS_X"), $m[1]);
				}
				elseif(Loc::getMessage("ESOL_IX_FILE_ERROR_".$this->lastFileError))
				{
					$arUrl = parse_url($file);
					$domain = $arUrl['scheme'].'://'.$arUrl['host'];
					$errorText = sprintf(Loc::getMessage("ESOL_IX_FILE_ERROR"), $file).' '.sprintf(Loc::getMessage("ESOL_IX_FILE_ERROR_".$this->lastFileError), $domain);
				}
	...
	public function GetFileArrayDirect($file, $arDef=array(), $arParams=array(), $oldId=0)
	{
		$this->lastFileError = '';
		...
		elseif(preg_match('/http(s)?:\/\//', $file))
		{
			...
			if(class_exists('\Bitrix\Main\Web\HttpClient'))
			{
				...
				$ext = ToLower(\Bitrix\EsolImportxml\Utils::GetFileExtension($basename));
				$arOptions = array();
				...
				try{
					...
					while(strlen($loc) > 0 && $loop < 5)
					{
						$loop++;
						$locPrev = $loc;
						$ob = new \Bitrix\EsolImportxml\HttpClient($arOptions);
				...
				if(!$bCustomName && $hcd && (stripos($hcd, 'filename=')!==false || stripos($hcd, 'filename*=')!==false))
				{
					...
					if(count($hcdParts1) > 0)
					{
						...
						if(!\Bitrix\EsolImportxml\Utils::IsUtfMode()) $newFn = \Bitrix\Main\Text\Encoding::convertEncoding($newFn, "UTF-8", "Windows-1251");
					...
					{
						$file = \Bitrix\EsolImportxml\Utils::ReplaceFile($file, dirname($file).'/'.$newFn);
					}
				}
		...
		if(array_key_exists('name', $arFile))
		{
			...
			{
				if(\Bitrix\EsolImportxml\Utils::IsUtfMode() && $io->ValidateFilenameString(Utils::Win1251Utf8($arFile['name']))) $arFile['name'] = Utils::Win1251Utf8($arFile['name']);
			}
		}
		...
		if(is_array($arFile) && array_key_exists('type', $arFile))
		{
			if(strpos($arFile['type'], 'image/')===0)
			{
				...
				$arExts = array('tiff'=>'png', 'svg'=>'webp', 'avif'=>'webp');
				if(in_array($ext2, array_keys($arExts)) && !empty($fileTypes) && !in_array($ext2, $fileTypes) && in_array($arExts[$ext2], $fileTypes))
				{
					$gdCreateFuncName = 'imagecreatefrom'.$ext2;
					$gdSaveFuncName = 'image'.$arExts[$ext2];
					if(class_exists('\Imagick') && in_array(ToUpper($ext2), \Imagick::queryFormats()))
					{
						try{
							$tmpsubdir = $this->CreateTmpImageDir();
					...
					elseif(function_exists($gdCreateFuncName) && function_exists($gdSaveFuncName))
					{
						$tmpsubdir = $this->CreateTmpImageDir();
						$file = \Bitrix\Main\IO\Path::convertLogicalToPhysical($tmpsubdir.preg_replace('/\.[^\.]{2,5}\s*$/', '', $arFile['name']).'.'.$arExts[$ext2]);
						$img = call_user_func($gdCreateFuncName, $arFile['tmp_name']);
						if($arExts[$ext2]=='png'){imageinterlace($img, false); imagepng($img, $file, 9);}
						else call_user_func($gdSaveFuncName, $img, $file, 100);
						imagedestroy($img);
						$arFile = self::MakeFileArray($file);
						$ext = ToLower(str_replace('image/', '', $arFile['type']));
					}
		...
		if(is_array($arFile) && array_key_exists('type', $arFile))
		{
			...
			{
				$ext = ToLower(\Bitrix\EsolImportxml\Utils::GetFileExtension($arFile['name']));
			...
			if($ext1!=$ext2)
			{
				...
				if(!$convert)
				{
					...
					if(function_exists($imagecreateFunc) && function_exists($imageFunc) && ($img = call_user_func($imagecreateFunc, $arFile['tmp_name'])))
					{
						...
						{
							if($ext2=='webp' && !imageistruecolor($img)) {
								imagepalettetotruecolor($img);
							}
							call_user_func($imageFunc, $img, $file, $quality);
						}
						imagedestroy($img);
						$arFile = self::MakeFileArray($file);
						$convert = true;
	...
	public static function ResizePicture($arFile, $arResize)
	{
		...
		if(($width > 0 && $orig[0] > $width) || ($height > 0 && $orig[1] > $height))
		{
			...
			{
				if($arResize['FILLING']==='Y'/* && $image_type == Image::FORMAT_WEBP*/)
					imagefilledrectangle($image_p, 0, 0, $width, $height, imagecolorallocate($image_p, 255, 255, 255));
		...
		foreach($arFileIds as $fileId)
		{
			...
			if($fileId > 0)
			{
				if(!array_key_exists($fileId, $this->dbFileExtIds))
				{
					...
					if(($arFile = \Bitrix\EsolImportxml\Utils::GetFileArray($fileId)) && ($imgPath = $_SERVER['DOCUMENT_ROOT'].\Bitrix\Main\IO\Path::convertLogicalToPhysical($arFile['SRC'])) && file_exists($imgPath) && filesize($imgPath) > 0)
					{
	...
	public function GetCurrencyRates()
	{
		...
		{
			$arRates = \Bitrix\EsolImportxml\Utils::Unserialize(\Bitrix\Main\Config\Option::get(static::$moduleId, 'CURRENCY_RATES', ''));
	...
	public function ConversionReplaceValues($m)
	{
		...
		else
		{
			...
			{				
				...
				{
					$arr = \Bitrix\EsolImportxml\Utils::JsObjectToPhp($value);
	...
	public function GetValueByXpath($xpath, $simpleXmlObj=null, $singleVal=false)
	{
		...
		if(strlen($curXpath2) > 0)
		{
			...
			if(isset($this->parentXpath) && mb_strlen($this->parentXpath) > 0 && mb_strpos($curXpath, $this->parentXpath)===0)
			{
				...
				if(strlen($tmpXpath) > 0) $tmpXmlObj = $this->Xpath($currentXmlObj, $tmpXpath);
				else $tmpXmlObj = false;
				if(!empty($tmpXmlObj))
				{
	...
	public function GetNamedElementFields($arItem, $forStat = true)
	{
		$arFields = array();
		foreach($this->params['FIELDS'] as $key=>$fieldFull)
		{
			...
			if($forStat)
			{
				if(!empty($arItem['~'.$key])) $arFields[$field] = array('VALUE' => $arItem['~'.$key]);
			}
			else $arFields[$field] = $arItem['~'.$key];
		}
		return $arFields;
	}
	...
	public function GetRealXmlValue($val)
	{
		...
		if($this->params['HTML_ENTITY_DECODE']=='Y')
		{
			if(is_array($val))
			{
				foreach($val as $k=>$v)
				{
					$v = html_entity_decode($v, ENT_QUOTES | ENT_HTML5, $this->siteEncoding);
					$v = preg_replace_callback($emoji_pattern, array(__CLASS__, 'Replace2Entity'), $v);
					$val[$k] = $v;
				}
			}
			else
	...
	public static function Replace2Entity($matches)
	{
		...
		$unpacked = unpack('H*', mb_convert_encoding($matches[0], 'UTF-32', 'UTF-8'));
		if(isset($unpacked[1]))
		{
			return $entity = '&#x'.ltrim($unpacked[1], '0').';';
		}
