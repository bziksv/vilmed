...
class Importer extends ImporterData {
	function __construct($filename, $params, $fparams, $stepparams, $pid = false)
	{
		...
		//$this->fileEncoding = \Bitrix\EsolImportxml\Utils::GetXmlEncoding($this->GetFileName());
		$this->fileEncoding = 'utf-8';
		$this->siteEncoding = \Bitrix\EsolImportxml\Utils::getSiteEncoding();
		$this->xpathMulti = ($this->params['XPATHS_MULTI'] ? \Bitrix\EsolImportxml\Utils::Unserialize(base64_decode($this->params['XPATHS_MULTI'])) : array());
		...
		{
			$arSkuFields = $arSkuAddFields = $arSkuUidAddFields = $arSkuDuplicateFields = array();
			...
			foreach($arSkuAddFields as $key=>$field)
			{
				...
				elseif(is_array($this->params['ELEMENT_UID_SKU']) && in_array($field, $this->params['ELEMENT_UID_SKU']))
				{
					$arSkuUidAddFields[] = $field;
				}
			}
			$this->arSkuAddFields = array_keys($arSkuAddFields);
			$this->arSkuUidAddFields = $arSkuUidAddFields;
		...
		/*Section map*/
		$this->sectionMap = \Bitrix\EsolImportxml\Utils::Unserialize(base64_decode($this->params['SECTION_MAP']));
		...
		/*Property map*/
		$this->propertyMap = \Bitrix\EsolImportxml\Utils::Unserialize(base64_decode($this->params['PROPERTY_MAP']));
		...
		if(count($this->propertyMap['MAP']) > 0)
		{
			...
			foreach($this->propertyMap['MAP'] as $k=>$v)
			{
				...
				$this->fieldSettings[$v['ID']] = $this->fparams[$v['XML_ID'].'_'.$fieldKey] = $this->fparams[$v['XML_ID'].'-'.$fieldKey] = (is_array($v['EXTRA']) ? $v['EXTRA'] : \Bitrix\EsolImportxml\Utils::JsObjectToPhp($v['EXTRA']));
		...
		/*Offer property map*/
		$this->offerPropertyMap = \Bitrix\EsolImportxml\Utils::Unserialize(base64_decode($this->params['OFFPROPERTY_MAP']));
		...
		if(count($this->offerPropertyMap['MAP']) > 0)
		{
			...
			foreach($this->offerPropertyMap['MAP'] as $k=>$v)
			{
				...
				$arMap2[$v['XML_ID']][] = $v['ID'];
				$fieldKey = count($arMap2[$v['XML_ID']]) - 1;
				$this->fieldSettings[$v['ID']] = $this->fparams['OFFER_'.$v['XML_ID'].'_'.$fieldKey] = $this->fparams['OFFER_'.$v['XML_ID'].'-'.$fieldKey] = (is_array($v['EXTRA']) ? $v['EXTRA'] : \Bitrix\EsolImportxml\Utils::JsObjectToPhp($v['EXTRA']));
				if(!is_array($this->fieldSettings[$v['ID']])) $this->fieldSettings[$v['ID']] = $this->fparams['OFFER_'.$v['XML_ID'].'_'.$fieldKey] = array();
				if(strpos($v['ID'], '|')!==false)
				{
					list($field, $adata) = explode('|', $v['ID']);
					$this->fieldSettings[$field] = $this->fieldSettings[$v['ID']];
		...
		$this->logger = new \Bitrix\EsolImportxml\Logger($params, $pid);
		...
		$this->el = new \Bitrix\EsolImportxml\DataManager\IblockElementTable($params);
		...
		{
			$this->stepparams = array_merge($this->stepparams, \Bitrix\EsolImportxml\Utils::Unserialize(file_get_contents($this->tmpfile)));
		}
	...
	public function ImportStep($time = 0)
	{
	...
	public function EndOfLoading($time)
	{
		...
		if($bElemDeactivate || $bSetDefaultProps)
		{
			...
			while($this->stepparams['deactivate_element_first'] < $this->stepparams['deactivate_element_last'])
			{
				...
				$dbRes = \Bitrix\EsolImportxml\DataManager\IblockElementTable::GetListComp($arFields, array('ID'), array('ID'=>'ASC'));
				while($arr = $dbRes->Fetch())
				{
	...
	{
		$arVals = \Bitrix\EsolImportxml\Utils::Unserialize(base64_decode($vals));
	...
	public function MissingElementsUpdate($ID, $IBLOCK_ID, $isOffer = false)
	{
		...
		$dbRes = \Bitrix\EsolImportxml\DataManager\IblockElementTable::GetListComp($arFilter, $arKeys);
	...
	{		
		...
		if(strlen($fieldName) > 0)
		{
			...
			elseif(!empty($ffilter['NOT_UPLOAD_VALUES']))
			{
				...
				if(count($arSubFilter) > 1) $arFilter[] = array_merge(array('LOGIC'=>'AND'), $arSubFilter);
				else $arFilter = array_merge($arFilter, current($arSubFilter));
			}
		}
	}
	...
	public function GetMissingFilter($isOffer = false, $IBLOCK_ID = 0, $arUpdatedIds=array())
	{
		...
		if($IBLOCK_ID > 0 && !empty($arDefaults))
		{
			...
			if(!empty($arProductFields) && !empty($arUpdatedIds) && $IBLOCK_ID > 0)
			{
				...
				{
					$arProductFields = array(array_merge(array('LOGIC'=>'OR'), array_map(array('\Bitrix\EsolImportxml\Utils', 'ArrayCombine'), array_keys($arProductFields), $arProductFields)));
				}
	...
	public function InitImport($type = 'element')
	{
		...
		foreach($this->params['FIELDS'] as $k=>$fieldFull)
		{
			...
			if(isset($this->fparams[$k]['HLBL_FIELD']) && !is_array($this->fparams[$k]['HLBL_FIELD']) && strlen($this->fparams[$k]['HLBL_FIELD']) > 0)
			{
				$this->fieldSettings[$field.'|'.$this->fparams[$k]['HLBL_FIELD']] = $this->fparams[$k];
			}
	...
	public function CheckSkipLine($arItem, $type='element')
	{
		...
		if($load)
		{
			foreach($this->fparams as $k=>$v)
			{
				...
				if(!$load)
				{
					$this->skipReasonFieldName = $field;
					return true;
				}
			}
		}
	...
	public function GetNextRecord($time)
	{
				...
				&& isset($this->xmlElements[$this->xmlCurrentRow - $this->xmlRowDiff])))
		{
			...
			foreach($this->params['FIELDS'] as $key=>$field)
			{
				...
				$val = $this->GetRealXmlValue($val);
				if(!$this->logger->NeedSaveElementSkipped() && $this->PreCheckSkipLine($key, $val))
				{
					//$this->logger->SaveElementSkipped($fieldName, $val);
					$skipLine = true;
					break;
				}
			...
			if(!$skipLine)
			{
				...
				{
					if($this->logger->NeedSaveElementSkipped())
					{
						$this->logger->SaveElementSkipped('FILTER_FIELD', $this->skipReasonFieldName, $this->GetNamedElementFields($arItem), $this->params['IBLOCK_ID']);
					}
				}
				else
				{
	...
	public function GetPropertyType(&$arFields)
	{
		...
		if($arFields['PROPERTY_TYPE']==Loc::getMessage("ESOL_IX_PROP_STRING")) $arFields['PROPERTY_TYPE'] = 'S';
		elseif($arFields['PROPERTY_TYPE']==Loc::getMessage("ESOL_IX_PROP_NUMBER")) $arFields['PROPERTY_TYPE'] = 'N';
		elseif($arFields['PROPERTY_TYPE']==Loc::getMessage("ESOL_IX_PROP_LIST")) $arFields['PROPERTY_TYPE'] = 'L';
	...
	public function SaveSectionRecord($arItem, $parentSectionId=0, $isSub=false)
	{
		...
		if($this->sectionLoadMode || count($this->sectionMap['MAP']) > 0)
		{
			...
			if($this->sectionLoadMode=='MAPPED_CHILD' 
				&& !(array_key_exists($arItemFields['PARENT_TMP_ID'], $this->sectionIds) && (is_array($this->sectionIds[$arItemFields['PARENT_TMP_ID']]) || (int)$this->sectionIds[$arItemFields['PARENT_TMP_ID']] > 0)) 
				&& !($this->subSectionInSection && $parentSectionId)) $skip = true;
		...
		else
		{
			...
			if(isset($arFieldsSections['PARENT_TMP_ID']) && strlen($arFieldsSections['PARENT_TMP_ID']) > 0)
			{
	...
	public function SaveRecord($arItem, $sectionID=0, $isPacket=false)
	{
		...
		foreach($this->params['FIELDS'] as $key=>$fieldFull)
		{
			...
			if($field=='PROFILE_URL' && is_numeric($this->fparams[$key]['REL_PROFILE_ID']) && strlen($this->fparams[$key]['REL_PROFILE_ID']) > 0)
			{
				if(is_array($value))
				{
					...
					{
						$arRelProfiles[$key.'-'.$subkey] = array('LINK'=>$subvalue, 'PROFILE'=>$this->fparams[$key]['REL_PROFILE_ID']);
					}
				}
				else $arRelProfiles[$key] = array('LINK'=>$value, 'PROFILE'=>$this->fparams[$key]['REL_PROFILE_ID']);
			}
		}
		...
		foreach($this->params['FIELDS'] as $key=>$fieldFull)
		{
			...
			if(!empty($conversions))
			{
				...
				if($value===false || (is_array($value) && count(array_diff(array_map(array('\Bitrix\EsolImportxml\Utils', 'CompEmptyString'), $value), array(false)))==0)) continue;
			}
			...
			)
		{
			if($this->logger->NeedSaveElementSkipped())
			{
				$this->logger->SaveElementSkipped('FILTER_SECTION', '', $this->GetNamedElementFields($arItem), $IBLOCK_ID);
			}
		...
		if(!empty($emptyFields) || empty($arUid))
		{
			...
			if(!$res)
			{
				...
				if(is_array($errElemName)) $errElemName = reset($errElemName);
				if(strlen($errElemName)==0) $errElemName = $arFieldsElement['XML_ID'];
				if(strlen($errElemName)==0) $errElemName = '№'.$this->xmlCurrentRow;
				$this->errors[] = sprintf(Loc::getMessage("ESOL_IX_NOT_SET_FIELD"), implode(', ', $emptyFields), '').(strlen($errElemName) > 0 ? ' ('.$errElemName.')' : '');
		...
		$arKeys = array_merge($this->baseElemFields, array_keys($arFieldsElement));
		...
		$allowCreate = (bool)($allowCreate && \Bitrix\EsolImportxml\DataManager\IblockElementTable::SelectedRowsCount($dbRes)==0);
	...
	public function SaveRecordAfter($ID, $IBLOCK_ID, $arItem, $arFieldsElement=array(), $isChanges=true, $saveOffers=true)
	{
		...
		if($saveOffers && $this->params['ELEMENT_UID_SKU'] && ($this->params['SEARCH_OFFERS_WO_PRODUCTS']!='Y' || $ID==0 || $this->params['CREATE_NEW_OFFERS']=='Y'))
		{
			...
			{				
				...
				elseif(empty($this->arSkuDuplicateFields) && empty($this->arSkuUidAddFields)) $isSaved = true;
				else
				{
					foreach($this->arSkuDuplicateFields as $key=>$key2)
					{
		...
		if($ID > 0)
		{
			...
			{
				if($arElement = \CIblockElement::GetList(array(), array('ID'=>$ID, 'IBLOCK_ID'=>$IBLOCK_ID), false, false, array('DETAIL_PAGE_URL'))->GetNext())
				{
	...
	public function CheckIdForNewElement(&$arFieldsElement, $isOffer=false)
	{
		if(isset($arFieldsElement['ID']))
		{
			...
			if($error===false && !preg_match('/^[1-9]\d*$/', $ID)) $error = Loc::getMessage("ESOL_IX_ERROR_FORMAT_ID");
			if($error===false && $ID > $maxVal) $error = sprintf(Loc::getMessage("ESOL_IX_ERROR_OUTOFRANGE_ID"), $maxVal);
			if($error===false && \Bitrix\Iblock\ElementTable::getList(array('filter'=>array('ID'=>$ID), 'select'=>array('ID')))->Fetch()) $error = Loc::getMessage("ESOL_IX_ERROR_EXISTING_ID");
			if($error!==false)
			{
				$this->stepparams['error_line']++;
				$this->errors[] = sprintf(($isOffer ? Loc::getMessage("ESOL_IX_NEW_OFFER_WITH_ID") : Loc::getMessage("ESOL_IX_NEW_ELEMENT_WITH_ID")), $arFieldsElement['ID'], $error);
				return false;
			}
		...
		while($arProperty = $this->GetNextProperty($groupXpath, $groupName))
		{
			...
			foreach($arPropsInd as $propInd)
			{
					...
					if($isPropertyMap && array_key_exists('NAME', $arPropertyFieldsOrig))
					{
						...
						if(array_key_exists($fName, $propertyMap['MAP']))
						{
							...
							foreach($arFields as $fieldKey=>$field)
							{
								...
								$fPrefix = ($isOffer && $this->isOfferPropertyMap ? 'OFFER_' : '');
								$value = $origValue = $arPropertyFields['VALUE'];
								if(!$isOffer && ($fgKey = $fPrefix.$fName.'-'.$fieldKey) && (in_array($fgKey, $this->fieldsForSkuGen) || in_array($fgKey, $this->fieldsBindToGenSku)))
								{
									//sku generate
								...
								$fs = $this->fparams[$fPrefix.$fName.'_'.$fieldKey];
				...
				foreach($propIds as $propId)
				{
					...
					{
						if(!is_array($arProp) && $arPropertyFields['XML_ID']) $arProp = $this->GetIblockPropertyByXmlId($arPropertyFields['XML_ID'], $IBLOCK_ID, /*$allowCreate*/false, $paramNewProps, $arPropertyFields);
						if(!is_array($arProp) && $arPropertyFields['NAME']) $arProp = $this->GetIblockPropertyByName($arPropertyFields['NAME'], $IBLOCK_ID, /*$allowCreate*/false, $paramNewProps, $arPropertyFields, $checkPropXmlId);
						if(!is_array($arProp) && $arPropertyFields['CODE']) $arProp = $this->GetIblockPropertyByCode($arPropertyFields['CODE'], $IBLOCK_ID);
						
						if(!is_array($arProp) && $allowCreate) //create after searching for element
						{
							if(!isset($_p['new_props'])) $_p['new_props'] = array('check_xml_id'=>$checkPropXmlId, 'params'=>$paramNewProps, 'props'=>array());
							$_p['new_props']['props'][] = $arPropertyFields;
						}
	...
	public function PrepareElementPictures(&$arFieldsElement, $IBLOCK_ID, $fieldPrefix='', $arElement=array())
	{
		...
		foreach($arPictures as $picName)
		{
			if($arFieldsElement[$picName])
			{
				...
				if(empty($arFile) && (!is_array($val) || $val=current($val)) && preg_match('/[;,\|\s'.preg_quote($sep, '/').']/s', $val))
				{
					...
					{
						$arFile = $this->GetFileArray($newVal, $arDef, $arFileParams, $arElement[$picName], $val);
					}
				}
		...
		foreach($arTexts as $keyText)
		{
			if($arFieldsElement[$keyText])
			{
				...
				{
					$arFieldsElement[$keyText] = \Bitrix\EsolImportxml\Utils::DownloadTextTextByLink($arFieldsElement[$keyText]);
				}
				else
				{
		...
		foreach($arPictures as $picName)
		{
			if($arFields[$picName])
			{
				...
				if(empty($arFile) && strpos($val, $this->params['ELEMENT_MULTIPLE_SEPARATOR'])!==false)
				{
					...
					{
						$arFile = $this->GetFileArray($val, array(), array('FILETYPE'=>'IMAGE'), $arSection[$picName], $val);
					}
				}
	...
	public function SaveSKUWithGenerate($ID, $NAME, $IBLOCK_ID, $arItem, $arFParams=false)
	{
		...
		if(!$this->isPacket && $ret && $isChanges)
		{
			...
			/*\Bitrix\EsolImportxml\DataManager\IblockElementTable::updateElementIndex($IBLOCK_ID, $ID);*/
		}
	...
	public function SaveSKU($ID, $NAME, $IBLOCK_ID, $arItem, $convertedFields=array(), $arFParams=false)
	{
		...
		foreach($arFields as $key=>$fieldFull)
		{
			...
			if(strpos($field, 'OFFER_')!==0) continue;
			if(!isset($arFParams[$key]) || !is_array($arFParams[$key])) $arFParams[$key] = array();
			$conversions = $arFParams[$key]['CONVERSION'];
		...
		if(\Bitrix\EsolImportxml\DataManager\IblockElementTable::SelectedRowsCount($dbRes)==0)
		{
		...
		/*if($OFFER_ID && ($this->params['ELEMENT_NO_QUANTITY_DEACTIVATE']=='Y' || $this->params['ELEMENT_NO_PRICE_DEACTIVATE']=='Y' || ($this->params['ELEMENT_LOADING_ACTIVATE']=='Y' && !$ID)) && class_exists('\Bitrix\Catalog\ProductTable') && class_exists('\Bitrix\Catalog\PriceTable'))
		{
			foreach($arProductIds as $prodId)
			{
				...
				if(!empty($arOfferIds))
				{
					...
					if($active!==false)
					{
						$arElem = \Bitrix\EsolImportxml\DataManager\IblockElementTable::GetListComp(array('ID'=>$prodId, 'CHECK_PERMISSIONS' => 'N'), array('ACTIVE'))->Fetch();
	...
	public function GetPropField(&$arFieldsProps, &$arFieldsPropsOrig, $fieldSettingsExtra, $propDef, $fieldName, $value, $origValue, $arUids = array())
	{
		...
		if($propDef)
		{
			if($propDef['USER_TYPE']=='directory')
			{
				...
				//if(isset($fieldSettingsExtra['PICTURE_PROCESSING'])) $arFieldsPropsItem[$key2.'|SETTINGS'] = $fieldSettingsExtra;
	...
	public function GetPropList(&$arFieldsProps, &$arFieldsPropsOrig, $fieldSettingsExtra, $IBLOCK_ID, $value, $isOffer = false)
	{
		...
		foreach($arProps as $prop)
		{
			...
			$paramNewProps = array();
			if(strlen($fieldSettingsExtra['PROPLIST_NEWPROP_PREFIX']) > 0) $paramNewProps['CODE_PREFIX'] = $fieldSettingsExtra['PROPLIST_NEWPROP_PREFIX'];
			foreach($fieldSettingsExtra as $k=>$v)
			{
				if(strpos($k, 'PROPLIST_NEWPROP_FEATURE_')===0)
				{
					$paramNewProps[substr($k, 25)] = $v;
				}
			}
			
			$propDef = $this->GetIblockPropertyByName($arCurProp[0], $IBLOCK_ID, $createNew, $paramNewProps);
			if($propDef!==false)
			{
	...
	public function AfterElementAdd($IBLOCK_ID, $ID)
	{
		...
		if($this->IsFacetChanges()) \Bitrix\EsolImportxml\DataManager\IblockElementTable::updateElementIndex($IBLOCK_ID, $ID);
	}
	...
	public function AfterSectionSave($ID, $IBLOCK_ID, $arFields, $arSection=array())
	{
		...
		{
			if($arSection = \CIblockSection::GetList(array(), array('ID'=>$ID, 'IBLOCK_ID'=>$IBLOCK_ID), false, array('SECTION_PAGE_URL'))->GetNext())
			{
	...
	public function UpdateSection($ID, $IBLOCK_ID, $arFields, $arSection, $sectionUid=false)
	{
		...
		if(!empty($arFields))
		{
			...
			\Bitrix\EsolImportxml\DataManager\InterhitedpropertyValues::ClearSectionValues($IBLOCK_ID, $ID, $arFields);
		}
		if($sectionUid)
		{
	...
	public function CreateNewProp($arFields, $paramNewProps, $propFields)
	{
		...
		if(is_array($paramNewProps) && count($paramNewProps) > 0)
		{
			...
			{
				$arFields['CODE'] = $this->GetPropCodeByName($arFields['CODE_PREFIX'].$arFields['CODE']);
	...
	public function RemoveProperties($ID, $IBLOCK_ID)
	{
		...
		if(is_array($arIds) && !empty($arIds))
		{
			...
			foreach($arIds as $k=>$v)
			{
				...
				elseif(strpos($v, 'ICAT_PRICE')===0)
				{
					$arPrice = explode('_', substr($v, 10), 2);
			...
			if(!empty($arFieldsPrices) || !empty($arFieldsProductStores))
			{
	...
	public function GetNewProperties(&$arProps, $IBLOCK_ID)
	{
		if(!isset($arProps['new_props']) || !is_array($arProps['new_props'])) return;
		if(!isset($arProps['new_props']['props']) || !is_array($arProps['new_props']['props'])) return;
		
		$checkPropXmlId = (bool)$arProps['new_props']['check_xml_id'];
		$paramNewProps = (is_array($arProps['new_props']['params']) ? $arProps['new_props']['params'] : array());
		$arPropsOrig = array();
		foreach($arProps['new_props']['props'] as $arPropertyFields)
		{
			$arProp = false;
			if(!is_array($arProp) && $arPropertyFields['XML_ID']) $arProp = $this->GetIblockPropertyByXmlId($arPropertyFields['XML_ID'], $IBLOCK_ID, true, $paramNewProps, $arPropertyFields);
			if(!is_array($arProp) && $arPropertyFields['NAME']) $arProp = $this->GetIblockPropertyByName($arPropertyFields['NAME'], $IBLOCK_ID, true, $paramNewProps, $arPropertyFields, $checkPropXmlId);
			...
			if(is_array($arProp) && isset($arProp['ID']))
			{
				...
				if($value!==false)
				{					
					$this->GetPropField($arProps, $arPropsOrig, array(), $arProp, $fieldName, $value, $origValue, $this->params['ELEMENT_UID']);
					
					if(isset($arPropertyFields['DESCRIPTION']))
					{
						if(!isset($arProps[$fieldName.'_DESCRIPTION'])) $arProps[$fieldName.'_DESCRIPTION'] = $arPropertyFields['DESCRIPTION'];
						else
						{
							if(!is_array($arProps[$fieldName.'_DESCRIPTION'])) $arProps[$fieldName.'_DESCRIPTION'] = array($arProps[$fieldName.'_DESCRIPTION']);
							$arProps[$fieldName.'_DESCRIPTION'][] = $arPropertyFields['DESCRIPTION'];
						}
					}
				}
			}
		}
		unset($arProps['new_props']);
	}
	...
	public function SaveProperties($ID, $IBLOCK_ID, $arProps, $arOldVals=array(), $needUpdate = false, $parentId = 0)
	{
		...
		$this->GetNewProperties($arProps, $IBLOCK_ID);
		$propsDef = $this->GetIblockProperties($IBLOCK_ID);
		...
		if(!empty($arProps))
		{
			foreach($arOldProps as $pk=>$pv)
			{
				...
				elseif(in_array($propsDef[$pk]['PROPERTY_TYPE'], array('L', 'E', 'G')) && $propsDef[$pk]['MULTIPLE']=='Y' && is_array($arProps[$pk]) && is_array($pv) && !isset($pv['VALUE']) && (count($arProps[$pk])==count($pv) || (($arProps[$pk]=\Bitrix\EsolImportxml\Utils::ArrayUnique($arProps[$pk])) && count($arProps[$pk])==count($pv))))
				{
					$newVal1 = array();
					$newVal2 = array();
				...
				{
					if((!is_array($pv) && strlen($pv) > 0 && is_array($newVal2 = \Bitrix\EsolImportxml\Utils::Unserialize($pv))) || (is_array($pv) && ($newVal2 = $pv)))
					{
	...
	public function IsEqProps($v1, $v2, $propDef=array())
	{
		...
		if(is_array($v1) || is_array($v2))
		{
			...
			if(count($v1)==count($v2))
			{
				foreach($v1 as $k=>$v)
				{
					if(!array_key_exists($k, $v2) || !$this->IsEqProps($v, $v2[$k], $propDef)) $eq = false;
				}
			} else $eq = false;
		}
	...
	public function GetFilterProperties($IBLOCK_ID)
	{
		...
		if(!isset($this->arFilterProperties[$IBLOCK_ID]))
		{
			...
			if(class_exists('\Bitrix\Iblock\SectionPropertyTable'))
			{
				...
				if(($arOfferIblock = \Bitrix\EsolImportxml\Utils::GetOfferIblockByOfferIblock($IBLOCK_ID)) && isset($arOfferIblock['IBLOCK_ID']) && $arOfferIblock['IBLOCK_ID'] > 0)
				{
	...
	public function GetListPropertyValue($arProp, $val, $bField=false)
	{
		...
		if($val['VALUE']!==false && strlen($val['VALUE']) > 0)
		{
			...
			if(!isset($this->propVals[$arProp['ID']][$cacheVals]))
			{
				...
				while(($arPropEnum = $dbRes->Fetch()) && (array_key_exists('=VALUE', $arFilter) && ToLower(trim($arPropEnum['VALUE']))!=ToLower(trim($val['VALUE']))) /*check "²"*/){}
				if($arPropEnum)
				{
					$arPropFields = $val;
					if(isset($arPropFields['TMP_ID'])) unset($arPropFields['TMP_ID']);
					if($bField!=='XML_ID')
					{
	...
	public function GetDefaultElementFields(&$arElement, $iblockFields, $IBLOCK_ID=0, $arElementProps = array())
	{
		...
		if($IBLOCK_ID > 0 && !empty($arElementProps))
		{
			$SECTION_ID = 0;
			/*
			if(isset($arElement['IBLOCK_SECTION']) && is_array($arElement['IBLOCK_SECTION']) && count($arElement['IBLOCK_SECTION']) > 0)
			{
				$SECTION_ID = current($arElement['IBLOCK_SECTION']);
			}
			*/
			$propsKey = $IBLOCK_ID.'-'.$SECTION_ID;
			
			$arPropIds = array();
			if(!isset($this->propsElemForCreate)) $this->propsElemForCreate = array();
			if(!array_key_exists($propsKey, $this->propsElemForCreate) && is_callable(array('\Bitrix\Iblock\PropertyTable', 'getList')))
			{
				$arPropCodes = array();
				$arCheckTemplates = array('ELEMENT_PREVIEW_PICTURE_FILE_NAME', 'ELEMENT_DETAIL_PICTURE_FILE_NAME');
				if($SECTION_ID > 0)
				{
					$ipropValues = new \Bitrix\Iblock\InheritedProperty\SectionTemplates($IBLOCK_ID, $SECTION_ID);
					$arTemplates = $ipropValues->findTemplates();
				}
				else
				{
					$ipropValues = new \Bitrix\Iblock\InheritedProperty\IblockTemplates($IBLOCK_ID);
					$arTemplates = $ipropValues->findTemplates();
				}
				foreach($arCheckTemplates as $tname)
				{
					if(isset($arTemplates[$tname]) && strlen($arTemplates[$tname]['TEMPLATE']) > 0)
					{
						if(preg_match_all('/\.property\.([^\}]+)\}/Uis', $arTemplates[$tname]['TEMPLATE'], $m))
						{
							$arPropCodes = array_unique(array_merge($arPropCodes, $m[1]));
						}
					}
				}
				...
				{
					$arPropIds = array();
					$dbRes = \Bitrix\Iblock\PropertyTable::getList(array('filter'=>array('IBLOCK_ID'=>$IBLOCK_ID, array('LOGIC'=>'OR', '=CODE'=>$arPropCodes, '=ID'=>$arPropCodes)), 'select'=>array('ID')));
					while($arProp = $dbRes->Fetch())
					{
						$arPropIds[] = $arProp['ID'];
					}
				}
				$this->propsElemForCreate[$propsKey] = $arPropIds;
			}
			else
			{
				$arPropIds = $this->propsElemForCreate[$propsKey];
			}
			if(!empty($arPropIds))
			{
				if(!array_key_exists('PROPERTY_VALUES', $arElement)) $arElement['PROPERTY_VALUES'] = array();
				foreach($arPropIds as $propId)
				{
					if(array_key_exists($propId, $arElementProps)) $arElement['PROPERTY_VALUES'][$propId] = $arElementProps[$propId];
				}
			}
		}
	}
	...
	public function GetIblockElementValue($arProp, $val, $fsettings, $bAdd = false, $allowNF = false, $allowMultiple = false)
	{
		...
		if($relField && $relField!='IE_ID' && $arProp['LINK_IBLOCK_ID'])
		{
			...
			$dbRes = \Bitrix\EsolImportxml\DataManager\IblockElementTable::GetListComp($arFilter, array('ID', 'XML_ID'), array('ID'=>'ASC'), ($allowMultiple ? false : 1));
	...
	public function GetIblockElementValueEx($arProp, $val, $bAdd = false, $allowNF = false, $allowMultiple = false)
	{
		...
		$dbRes = \Bitrix\EsolImportxml\DataManager\IblockElementTable::GetListComp($arFilter, $arKeys, array('ID'=>'ASC'), ($allowMultiple ? false : 1));
		if($arElem = $dbRes->Fetch())
		{
			$val = $arElem['ID'];
			if($allowMultiple)
		...
		if($val && Loader::includeModule('highloadblock') && isset($arProp['USER_TYPE_SETTINGS']['TABLE_NAME']) && $arProp['USER_TYPE_SETTINGS']['TABLE_NAME'])
		{
			...
			foreach($arItems as $arFields)
			{
				...
				if(!isset($this->propVals[$arProp['ID']][$cacheKey]))
				{
					...
					if(count($arFields)==1)
					{
						$this->PrepareHighLoadBlockFields($arFields, $arHLFields, $arProp['ID']);
						//$arFilter = $arFields;
					...
					if($arr2 = $dbRes2->Fetch())
					{
						...
						{
							$this->PrepareHighLoadBlockFields($arFields, $arHLFields, $arProp['ID'], $arr2);
	...
	public function PrepareHighLoadBlockFields(&$arFields, $arHLFields, $propId, $arOldVals=array())
	{
		foreach($arFields as $k=>$v)
		{
			...
			$fKey = 'IP_PROP'.$propId.'|'.$k;
			$fs = (isset($this->fieldSettings['OFFER_'.$fKey]) ? $this->fieldSettings['OFFER_'.$fKey] : $this->fieldSettings[$fKey]);
			if($arHLFields[$k]['MULTIPLE']=='Y')
			{
				...
				foreach($v as $k2=>$v2)
				{
					$arFields[$k][$k2] = $this->GetHighLoadBlockFieldVal($v2, $type, $settings, $fs, $arOldVals[$k]);
				}
				if($type=='file' && count(array_diff($arFields[$k], array('')))==0) unset($arFields[$k]);
			}
			else
			{
				$arFields[$k] = $this->GetHighLoadBlockFieldVal($v, $type, $settings, $fs, $arOldVals[$k]);
				if($type=='file' && !is_array($arFields[$k])) unset($arFields[$k]);
			}
		}
	}
	
	public function GetHighLoadBlockFieldVal($v, $type, $settings, $fs, $oldVal='')
	{
		if($type=='file')
		{
			$arFile = $this->GetFileArray($v, (is_array($fs) && isset($fs['PICTURE_PROCESSING']) ? $fs['PICTURE_PROCESSING'] : array()), array(), $oldVal);
	...
	public function AfterSaveProduct(&$arFieldsElement, $ID, $IBLOCK_ID, $isUpdate=false, $isOffer=false)
	{
			...
			|| ($this->params['ELEMENT_NO_PRICE_DEACTIVATE']=='Y' && floatval($this->productor->GetProductPrice($ID, $IBLOCK_ID))<=0))
		{
			...
			if($isOffer && ($arOfferIblock = \Bitrix\EsolImportxml\Utils::GetOfferIblockByOfferIblock($IBLOCK_ID)))
			{
