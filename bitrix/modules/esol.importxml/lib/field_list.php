...
class FieldList {
	...
	public static function GetIblockElementFieldsForStat($IBLOCK_ID=0)
	{
		if(!isset(self::$arStatFields[$IBLOCK_ID]))
		{
			$arFields = self::GetIblockElementFields();
			$arFields["IE_MODIFIED_BY"] = array(
				"name" => Loc::getMessage("ESOL_IX_FI_MODIFIED_BY"),
			);
			if($IBLOCK_ID > 0 && ($arPropFields = self::GetIblockProperties($IBLOCK_ID)))
			{
				foreach($arPropFields as $arPropField)
				{
					$arFields[$arPropField['value']] = array('name' => $arPropField['name']);
				}
			}
			self::$arStatFields[$IBLOCK_ID] = $arFields;
		}
		
		return self::$arStatFields[$IBLOCK_ID];
	}
	...
	public static function GetCatalogFields($IBLOCK_ID, $offers=false)
	{
		...
		if(Loader::includeModule('catalog'))
		{
			...
			if($arCatalog || $IBLOCK_ID===true)
			{
				...
				if(!$offers || Option::get('catalog', 'use_offer_marking_code_group')=='Y')
				{
					$arCatalogFields[] = array(
						"value" => "ICAT_UF_PRODUCT_GROUP",
						"name" => Loc::getMessage("ESOL_IX_FI_UF_PRODUCT_GROUP"),
					);
				}
			}
		}
		...
		if(Loader::includeModule('iblock'))
		{
			...
			while($arr = $dbRes->Fetch())
			{
				...
				$arProperties[] = array(
					...
					"uid" => ($bUid ? "Y" : "N"),
					"general" => "Y",
					"wdesc" => (bool)($arr["WITH_DESCRIPTION"]=='Y' || ($arr['PROPERTY_TYPE']=='E' && $arr['USER_TYPE'] && !(in_array($arr['USER_TYPE'], array('SKU', 'EList', 'EAutocomplete')))) || in_array($arr['USER_TYPE'], array('CitrusArealtyMetroStation', 'maxyss_unit'))),
	...
	public function ShowSelectPropertyList($IBLOCK_ID, $fname, $val=false, $prefix='', $onlyProps=false)
	{
		$properties = $this->GetSelectPropertyList($IBLOCK_ID, $val, $prefix);
		if(!$onlyProps) $catalogFields = $this->GetSelectCatalogList($IBLOCK_ID, $val, $prefix);
	...
	public function GetSelectCatalogList($IBLOCK_ID, $val=false, $prefix='')
	{
		...
		if(is_array($arFields))
		{
			foreach($arFields as $k=>$v)
			{
				if(!preg_match('/^(ICAT_STORE\d+_AMOUNT|ICAT_PRICE\d+_PRICE)$/', $v['value'])) continue;
				$optval = ($prefix ? $prefix : '').htmlspecialcharsbx($v['value']);
				$options .= '<option value="'.$optval.'"'.(in_array($optval, $val) ? ' selected' : '').'>'.htmlspecialcharsbx($v['name']).'</option>';
			}
		}
