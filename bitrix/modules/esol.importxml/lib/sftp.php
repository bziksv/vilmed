...
class Sftp
{
	...
	{
		$path = trim($path);
		$ssl = preg_match("#^ftp(es|s)://#", $path);
		$ftpes = preg_match("#^ftpes://#", $path);
		$urlComponents = $this->ParseUrl($path);
		$filepath = $urlComponents['path'];
		$ftphost = $urlComponents['host']; 
		$ftpport = (isset($urlComponents['port']) ? $urlComponents['port'] : ($ssl && !$ftpes ? 990 : 21));
		...
		if(!isset($this->connects[$streamHash]))
		{
			...
			if($stream)
			{
				...
				if(ftp_login($stream, $ftpuser, $ftppassword))
				{
					...
					{
						$path = preg_replace('/(ftp(es|s)?:\/\/[^\/]+)\//Uis', '$1'.rtrim($rootPath, '/').'/', $path);
					}
	...
	public function ParseUrl($path)
	{
		...
		if(preg_match('/^(ftp(es|s)?:\/\/)([^:]*):(.*)@(.*\/.*)$/is', $path, $m))
		{
			$path = $m[1].$m[5];
			$urlComponents = parse_url($path);
			$urlComponents['user'] = $m[3];
			//$urlComponents['pass'] = rawurldecode($m[3]);
			$urlComponents['pass'] = $m[4];
		}
	...
	public function Upload($path, $fn)
	{
		...
		if((!preg_match("#^ftp://#i", $path) && function_exists('ftp_connect')
			&& !preg_match("#^ftp(es|s)://#i", $path) && function_exists('ftp_ssl_connect'))
	...
	public function GetListFiles($path)
	{
		...
		else
		{
			if((preg_match("#^ftp://#", $path) && function_exists('ftp_connect')
				|| preg_match("#^ftp(es|s)://#", $path) && function_exists('ftp_ssl_connect')))
			{
