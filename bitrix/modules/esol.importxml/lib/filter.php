...
class Filter {
	...
	public function ShowSelectFilterFields($fl, $iblockId, $arFilter)
	{
		...
		foreach($arFilter as $k=>$v)
		{
			if(isset($arFields[$v['FIELD']]) && isset($arFields[$v['FIELD']]['type']) && in_array($arFields[$v['FIELD']]['type'], array('section', 'list')))
			{
				...
				echo '<input type="hidden" name="FVALS_'.htmlspecialcharsbx($v['FIELD']).'" value="'.htmlspecialcharsbx(\Bitrix\EsolImportxml\Utils::PhpToJSObject($arValues)).'">';
			}
		}
	}
	...
	public function ShowFilterBlock($fid, $arFilter, $fl)
	{
		...
		<div class="kda-ee-sheet-cfilter" id="<?echo $fid;?>" data-type="<?echo $this->type;?>" <?if(!in_array($this->type, array('e', 'hl'))){echo ' style="display: none;"';}?>>
			<div class="kda-ee-sheet-cfilter-hidden">
				<input type="hidden" name="OLD_FILTER" value="<?echo (count($arFilter) > 0 ? htmlspecialcharsbx(\Bitrix\EsolImportxml\Utils::PhpToJSObject($arFilter)) : '');?>">
	...
	public function GetListValues($field, $arParams=array())
	{
		...
		if($iblockId > 0) $offerIblockId = \Bitrix\EsolImportxml\Utils::GetOfferIblock($iblockId);
		if($iblockId > 0) $parentIblockId = \Bitrix\EsolImportxml\Utils::GetParentIblock($iblockId);
		...
		else
		{
			...
			elseif($field=='IE_IBLOCK_OFFER')
			{
				...
				if($cnt > $maxValuesCnt)
				{
					$arOfferIblock = \Bitrix\EsolImportxml\Utils::GetOfferIblock($IBLOCK_ID, true);
