...
class Conversion
{
	...
	public function UpdateElementFields(&$arFieldsElement, $ID)
	{
		...
		foreach($arFieldsElement as $fieldKey=>$fieldVal)
		{
			...
			$fKey = $fKey2 = 'IE_'.$fieldKey;
			if($fKey2=='IE_IBLOCK_SECTION') $fKey2 = 'IE_IBLOCK_SECTION_TMP_ID';
			$arFieldsElement[$fieldKey] = $this->UpdateElementField($fKey2, $fieldVal, $ID);
	...
	public function UpdateProduct(&$arProduct, &$arPrices, &$arStores, $ID)
	{
		...
		foreach($arPrices as $gid=>$arFieldsPrice)
		{
			foreach($arFieldsPrice as $priceKey=>$priceVal)
			{
				...
				$fKey = 'ICAT_PRICE'.$gid.'_'.(strpos($priceKey, 'PRICE|')===0 ? explode('|', $priceKey)[0] : $priceKey);
	...
	public function ApplyExtraConversions($fKey, $val, $ID)
	{
		...
		if(count($arConv) > 0)
		{
			...
			if($this->isSection)
			{
				...
				$prefixPattern2 = '/(#(('.implode('|', $this->fieldSectPrefixes).')[A-Z0-9_]+)#|\$\{[\'"]#(('.implode('|', $this->fieldSectPrefixes).')[A-Z0-9_]+)#[\'"]\}|#VAL#|#DATETIME#|#DATETIME[\+\-]\d+#|'.implode('|', $this->ie->rcurrencies).')/';
			}
			else
			{
				$prefixPattern = '/^(PARENT_)?('.implode('|', $this->fieldPrefixes).')[A-Z0-9_]+$/';
				$prefixPattern2 = '/(#(PARENT_)?(('.implode('|', $this->fieldPrefixes).')[A-Z0-9_]+)#|\$\{[\'"]#(PARENT_)?(('.implode('|', $this->fieldPrefixes).')[A-Z0-9_]+)#[\'"]\}|#VAL#|#DATETIME#|#DATETIME[\+\-]\d+#|'.implode('|', $this->ie->rcurrencies).')/';
			}
	...
	public function ConversionExtraReplaceValues($m)
	{
		...
		if(preg_match('/^DATETIME([\+\-]\d+)?$/', $key, $m2))
		{
	...
	{
		return $this->ie->GetFloatVal($val, $precision, $allowEmpty);
	}
