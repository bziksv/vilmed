...
if(array_key_exists('POSTEXTRA', $_POST))
{
	...
	if($arFieldParams) $arFieldParams = \Bitrix\EsolImportxml\Utils::JsObjectToPhp($arFieldParams);
	if(!$arFieldParams) $arFieldParams = array();
	/*if(!\Bitrix\EsolImportxml\Utils::IsUtfMode())
	{
...
elseif($_POST['action']=='import_conv_csv')
{
	...
	ob_end_clean();
	echo \Bitrix\EsolImportxml\Utils::PhpToJSObject(array('CONV'=>$arConv, 'EXTRA_CONV'=>$arExtraConv));
	die();*/
}
...
elseif($_POST['action']=='save' && is_array($_POST['EXTRASETTINGS']))
{
	...
	$returnJson = (empty($PEXTRASETTINGS[$keys[1][0]]) ? '""' : \Bitrix\EsolImportxml\Utils::PhpToJSObject($PEXTRASETTINGS[$keys[1][0]]));
	...
	if($arProp = $dbRes->Fetch())
	{
		...
		elseif($arProp['USER_TYPE']=='directory' && $arProp['USER_TYPE_SETTINGS']['TABLE_NAME'] && CModule::IncludeModule('highloadblock'))
		{
			...
			while($arHLField = $dbRes->Fetch())
			{
				...
				if($val==$arHLField['FIELD_NAME'] && $arHLField['USER_TYPE_ID']=='file')
				{
					$bPicture = true;
				}
			}
...
//$arStuct = \Bitrix\EsolImportxml\Utils::JsObjectToPhp($_POST['POSTSTRUCT']);
$arStuct = \Bitrix\EsolImportxml\Utils::Unserialize(base64_decode($_POST['POSTSTRUCT']));
...
?>
<form action="" method="post" enctype="multipart/form-data" name="field_settings" class="esol_ix_settings_form"<?if(!$isAdmin){echo ' data-notadmin="1"';}?>>
	...
	<table width="100%">
		...
		<?if($bPropList){?>
			...
			<tr class="subparams" <?if(!$createNewProps){echo 'style="display: none;"';}?>>
				<td class="adm-detail-content-cell-l"><?echo GetMessage("ESOL_IX_SETTINGS_PROPLIST_NEWPROP_PREFIX");?>:</td>
				<td class="adm-detail-content-cell-r">
					<?
					...
					echo '<div class="esol-ix-settings-conversion" '.(!$showCondition ? 'style="display: none;"' : '').'>'.
							...
							' <select class="field_then" name="'.$fName.'[THEN][]">'.
								...
								'<optgroup label="'.GetMessage("ESOL_IX_SETTINGS_CONVERSION_THEN_GROUP_MATH").'">'.
									...
									'<option value="MATH_FORMULA" '.($v['THEN']=='MATH_FORMULA' ? 'selected' : '').'>'.GetMessage("ESOL_IX_SETTINGS_CONVERSION_THEN_MATH_FORMULA").'</option>'.
								'</optgroup>'.
								...
								'</optgroup>'.
							'</select> '.
							'<input class="field_to" type="text" name="'.$fName.'[TO][]" value="'.htmlspecialcharsbx($v['TO']).'"'.($isDisabled ? ' readonly' : '').'>'.
				...
				<div id="esol_picprocessing_wrap" <?if($bElemPicture && !$bElemPictureIncProcessing){echo 'style="display: none;"';}?>>
					...
					>
						...
						<div class="adm-list-label">
							...
							><?echo GetMessage("ESOL_IX_PICTURE_FILLING")?></label>
						</div>
					</div>
					<div class="adm-list-item">
						<div class="adm-list-control">
					...
					<div class="adm-list-item">
						...
						<div class="adm-list-label">
							...
							><?echo GetMessage("ESOL_IX_PICTURE_CHANGE_EXTENSION")?></label>
						</div>
					</div>
					<div class="adm-list-item"
						...
						?>"
					>
						<?echo GetMessage("ESOL_IX_PICTURE_NEW_EXTENSION")?>:&nbsp;<select
		...
		<?if(!$bOtherField || $bProfileUrl){?>
			...
			<?if($bMultipleProp && $bIblockElement && strlen($propertyName) > 0 && (!$iblockElementIblock || $iblockElementIblock==$IBLOCK_ID)){?>
				<tr>
					<td class="adm-detail-content-cell-l"><?echo GetMessage("ESOL_IX_SETTINGS_EXCLUDE_CURRENT_ELEMENT");?>:</td>
					<td class="adm-detail-content-cell-r">
						<?
			...
			<tr>
				<td class="esol-ix-settings-margin-container" colspan="2" id="esol-ix-conv-wrap1">
					...
					foreach($arVals as $k=>$v)
					{
						$isDisabled = (bool)($v['THEN']=='EXPRESSION' && !$isAdmin);
						$cellsOptions = '<option value="">'.sprintf(GetMessage("ESOL_IX_SETTINGS_CONVERSION_CELL_CURRENT"), $i).'</option>';
						foreach($arSFields as $k2=>$arGroup)
						{
						...
						echo '<div class="esol-ix-settings-conversion" '.(!$showCondition ? 'style="display: none;"' : '').'>'.
								...
								'</select> '.
								'<input class="field_to" type="text" name="'.$fName.'[TO][]" value="'.htmlspecialcharsbx($v['TO']).'"'.($isDisabled ? ' readonly' : '').'>'.
								'<input class="choose_val" value="..." type="button" onclick="ESettings.ShowExtraChooseVal(this, '.$countCols.')"'.($isDisabled ? ' disabled' : '').'>'.
								'<a href="javascript:void(0)" onclick="ESettings.ConversionUp(this)" title="'.GetMessage("ESOL_IX_SETTINGS_UP").'" class="up"></a>'.
								'<a href="javascript:void(0)" onclick="ESettings.ConversionDown(this)" title="'.GetMessage("ESOL_IX_SETTINGS_DOWN").'" class="down"></a>'.
								'<a href="javascript:void(0)" onclick="ESettings.RemoveConversion(this)" title="'.GetMessage("ESOL_IX_SETTINGS_DELETE").'" class="delete"></a>'.
								(!$isAdmin ? '<input type="hidden" name="'.$fName.'[INDEX][]" value="'.htmlspecialcharsbx(strlen($v['INDEX']) > 0 ? $v['INDEX'] : $k).'">' : '').
...
var admKDASettingMessages = {
	'CURRENT_VALUE': '<?echo htmlspecialcharsex(GetMessage("ESOL_IX_SETTINGS_LANG_CURRENT_VALUE"));?>',
	...
	'RATES': <?echo (is_array($arRates) && count($arRates) > 0 ? \Bitrix\EsolImportxml\Utils::PhpToJSObject($arRates) : "''");?>,
	...
	'IFILEDATE': '<?echo htmlspecialcharsex(GetMessage("ESOL_IX_SETTINGS_LANG_IFILEDATE"));?>',
	'IDATETIME': '<?echo htmlspecialcharsbx(GetMessage("ESOL_IX_SETTINGS_LANG_IDATETIME"));?>',
	'EXTRAFIELDS': <?echo \Bitrix\EsolImportxml\Utils::PhpToJSObject($arSFields)?>,
	'AVAILABLE_TAGS': <?echo \Bitrix\EsolImportxml\Utils::PhpToJSObject($availableTags)?>,
	'VALUES': <?echo (is_array($arPropVals) && count($arPropVals) > 0 ? \Bitrix\EsolImportxml\Utils::PhpToJSObject($arPropVals) : "''");?>
};
</script>
