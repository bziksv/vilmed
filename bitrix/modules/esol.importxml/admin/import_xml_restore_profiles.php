...
if($_POST['action']=='save')
{
	...
	$oProfile = new \Bitrix\EsolImportxml\Profile(($_REQUEST['suffix'] ? $_REQUEST['suffix'] : ''));
	...
	if($_GET['return_data'])
	{
		$returnJson = (empty($PEXTRASETTINGS[$keys[1][0]][$keys[1][1]]) ? '""' : \Bitrix\EsolImportxml\Utils::PhpToJSObject($PEXTRASETTINGS[$keys[1][0]][$keys[1][1]]));
...
<form action="" method="post" enctype="multipart/form-data" name="restore_profiles" id="restore_profiles">
	...
	<input type="hidden" name="suffix" value="<?echo $_REQUEST['suffix']?>">
	<?/*if($error){
		ShowError($error);
		?><script>
	...
	<table width="100%">
		...
		<tr id="esol_restore_profile_list_row" style="display: none;">
			<td class="adm-detail-content-cell-l" valign="top"><?echo GetMessage("ESOL_IX_RESTORE_PROFILE_CHOOSE");?>:</td>
			<td class="adm-detail-content-cell-r" valign="top">
				<div id="esol_restore_profile_list"></div>
			</td>
		</tr>
	</table>
</form>
