...
if(strlen($PROFILE_ID) > 0 && $PROFILE_ID!=='new')
{
	...
	if($EXTRASETTINGS)
	{
		foreach($EXTRASETTINGS as $k=>$v)
		{
			if($v && !is_array($v))
			{
				$EXTRASETTINGS[$k] = \Bitrix\EsolImportxml\Utils::JsObjectToPhp($v);
			}
		}
	}
...
{
	$oProfile->CheckUserSettings($SETTINGS_DEFAULT);
}
...
if ($REQUEST_METHOD == "POST" && $MODE=='AJAX')
{
	...
	{
		$oFilter = new \Bitrix\EsolImportxml\Filter($_POST['IBLOCK_ID']);
		...
		$APPLICATION->RestartBuffer();
		ob_end_clean();
		echo \Bitrix\EsolImportxml\Utils::PhpToJSObject($arValues);
		die();
	}
	...
	if($ACTION=='GET_PROCESS_PARAMS')
	{
		...
		if(ob_get_contents()) ob_end_clean();
		$oProfile = new \Bitrix\EsolImportxml\Profile();
		echo \Bitrix\EsolImportxml\Utils::PhpToJSObject($oProfile->GetProccessParams($PROCCESS_PROFILE_ID));
		die();
	}
	...
	if($ACTION=='COPY_PROFILE')
	{
		...
		echo \Bitrix\EsolImportxml\Utils::PhpToJSObject(array('id'=>$id));
		die();
	}
	...
	if($ACTION=='GET_XPATH_VALUES')
	{
		...
		$xmlViewer = new \Bitrix\EsolImportxml\XMLViewer($arFile['SRC'], $SETTINGS_DEFAULT, $_POST['PROFILE_ID']);
		$arVals = $xmlViewer->GetXpathVals($_POST['XPATH'], $_POST['PARENT_XPATH'], $_POST['EXTRASETTINGS'], $SETTINGS_DEFAULT);
		
		$APPLICATION->RestartBuffer();
		if(ob_get_contents()) ob_end_clean();
		echo \Bitrix\EsolImportxml\Utils::PhpToJSObject($arVals);
		die();
	}
...
if ($REQUEST_METHOD == "POST" && $STEP > 1 && check_bitrix_sessid())
{
	...
	if ($STEP > 1)
	{
		...
		if (strlen($strError) <= 0 && (!$notRewriteFile || $_POST['FORCE_UPDATE_FILE']=='Y'))
		{
			if($STEP==2 || ($STEP==3 && $_POST['FORCE_UPDATE_FILE']=='Y'))
			{
				...
				{
					if($_POST["EXT_DATA_FILE"])
					{
						$oProfile = \Bitrix\EsolImportxml\Profile::getInstance();
						$oProfile->CheckFileSettings($_POST["EXT_DATA_FILE"], $PROFILE_ID);
					...
					{
						$fileId = \Bitrix\EsolImportxml\SMail::GetNewFile($SETTINGS_DEFAULT['EMAIL_DATA_FILE'], 0, 'esol_importxml_'.$PROFILE_ID);
						if($fileId > 0)
						{
			...
			if((isset($_FILES["DATA_FILE"]) && $_FILES["DATA_FILE"]["tmp_name"]) || (isset($_POST['DATA_FILE']) && $_POST['DATA_FILE'] && !is_numeric($_POST['DATA_FILE'])))
			{
				...
				elseif(isset($_POST['DATA_FILE']) && strlen($_POST['DATA_FILE']) > 0)
				{
					...
					if(strpos($_POST['DATA_FILE'], '/')===0) 
					{
						...
						if(!file_exists($filepath))
						{
							if(\Bitrix\EsolImportxml\Utils::IsUtfMode()) $filepath2 = $APPLICATION->ConvertCharsetArray($filepath, LANG_CHARSET, 'CP1251');
					...
					else
					{
						...
						if($filepath && $_POST['OLD_DATA_FILE'])
						{
							...
							$newFileSize = 0;
							$lastModified = '';
							if(stripos(trim($filepath), 'http')===0 && strpos($filepath, '{API_')===false && !\Bitrix\EsolImportxml\Utils::IsApiService($filepath) && is_callable(array($client, 'head')) && ($headers = $client->head($filepath)) && $client->getStatus()!=404)
							{
					...
					if(!$fid)
					{
						...
						if($arFile['name'])
						{
							...
							$arFile['external_id'] = 'esol_importxml_'.$PROFILE_ID;
							$arFile['del_old'] = 'Y';
							if($fid = \Bitrix\EsolImportxml\Utils::SaveFile($arFile, $moduleId))
							{
				...
				if(!$fid)
				{
					...
					{
						$strError.= GetMessage("ESOL_IX_FTP_EXTENSION")."<br>";
					}
					if($extFile)
					{
		...
		if (strlen($strError) <= 0)
		{
			...
			if(strlen($strError)==0 && (!$DATA_FILE_NAME = \Bitrix\EsolImportxml\Utils::GetFileName($DATA_FILE_NAME)))
			{
	...
	if($ACTION == 'SHOW_REVIEW_LIST')
	{
		...
		$error = '';
		$fl = new \Bitrix\EsolImportxml\FieldList($SETTINGS_DEFAULT);
		$xmlViewer = new \Bitrix\EsolImportxml\XMLViewer($DATA_FILE_NAME, $SETTINGS_DEFAULT, $PROFILE_ID);
		try{
		...
		echo '<div class="esol_ix_section_section"><span>'.GetMessage("ESOL_IX_SECTION").'</span><span id="hint_SECTION_ID"></span><script>BX.hint_replace(BX("hint_SECTION_ID"), "'.GetMessage("ESOL_IX_SECTION_HINT").'");</script>';
		...
		echo '<input type="hidden" name="defaultsettings_json" value="'.htmlspecialcharsbx(\Bitrix\EsolImportxml\Utils::PhpToJSObject($SETTINGS_DEFAULT)).'">';
		echo '<input type="hidden" name="settings_json" value="'.htmlspecialcharsbx(\Bitrix\EsolImportxml\Utils::PhpToJSObject($SETTINGS)).'">';
		echo '<input type="hidden" name="extrasettings_json" value="'.htmlspecialcharsbx(\Bitrix\EsolImportxml\Utils::PhpToJSObject($EXTRASETTINGS)).'">';
		//echo '<input type="hidden" name="struct_json" value="'.htmlspecialcharsbx(\Bitrix\EsolImportxml\Utils::PhpToJSObject($arStruct)).'">';
		...
		{
			echo '<div class="esol_ix_xml_struct_warning">';
			echo BeginNote();
			echo '<b>'.GetMessage("ESOL_IX_AUTOFIELDS_NOTE").'</b> <a href="javascript:void(0)" onclick="EIXPreview.UnsetAutoSettings(this)">'.GetMessage("ESOL_IX_AUTOFIELDS_CANCEL").'</a>';
			echo EndNote();
			echo '</div>';
		}
	...
	if($ACTION == 'DO_IMPORT')
	{
		...
		$APPLICATION->RestartBuffer();
		if(ob_get_contents()) ob_end_clean();
		echo '<!--module_return_data-->'.\Bitrix\EsolImportxml\Utils::PhpToJSObject($arResult).'<!--/module_return_data-->';
...
$arSubMenu[] = array(
	"TEXT" => GetMessage("ESOL_IX_TOOLS_IMG_LOADER"),
	"TITLE" => GetMessage("ESOL_IX_TOOLS_IMG_LOADER"),
...
$aMenu = array(
	array(
		...
		"ICON" => "",*/
		"HTML" => '<a href="https://esolutions.su/solutions/'.$moduleId.'/?tab=video" target="blank" class="adm-btn" title="'.GetMessage("ESOL_IX_MENU_VIDEO").'">'.GetMessage("ESOL_IX_MENU_VIDEO").'</a>'
	),
	array(
		...
		"ICON" => "",*/
		"HTML" => '<a href="https://esolutions.su/solutions/'.$moduleId.'/?tab=faq" target="blank" class="adm-btn" title="'.GetMessage("ESOL_IX_MENU_FAQ").'">'.GetMessage("ESOL_IX_MENU_FAQ").'</a>'
	),
	array(
...
<form method="POST" action="<?echo $sDocPath ?>?<?if(strlen($PROFILE_ID) > 0){echo 'PROFILE_ID='.htmlspecialcharsbx($PROFILE_ID).'&';}?>lang=<?echo LANG ?>" ENCTYPE="multipart/form-data" name="dataload" id="dataload" class="esol-ix-s1-form">
	...
	?>
		<tr>
			<td class="esol-ix-settings-margin-container" colspan="2" align="center">
				<a class="esol-ix-grey" href="javascript:void(0)" onclick="ESettings.AddProfileDescription(this)"><?echo GetMessage("ESOL_IX_PROFILE_DESCRIPTION_ADD");?></a>
			</td>
		</tr>
		<?
		}
		...
		<tr <?if(!$isDescription){echo ' style="display: none;"';}?>>
			<td><?echo GetMessage("ESOL_IX_PROFILE_DESCRIPTION"); ?>:</td>
			<td>
		...
		<tr class="esol-extra-mode-chbs-wrap<?if($sepMode){echo ' esol-extra-mode-chbs-wrap-active';}?>">
			<td valign="top"><?echo GetMessage("ESOL_IX_ONLY_UPDATE_MODE"); ?>: <span id="hint_ONLY_UPDATE_MODE_ELEMENT"></span><script>BX.hint_replace(BX('hint_ONLY_UPDATE_MODE_ELEMENT'), '<?echo GetMessage("ESOL_IX_ONLY_UPDATE_MODE_HINT"); ?>');</script></td>
			<td>			
				...
				<a href="javascript:void(0)" onclick="EProfile.ShowExtraModeChbs(this)" class="esol-extra-mode-link" title="<?echo GetMessage("ESOL_IX_EXTRA_MODE_LINK"); ?>" <?if(!$OFFERS_IBLOCK_ID){echo 'style="display: none;"';}?>></a>
				<div class="esol-extra-mode-chbs">
					<div>
						...
						<label for="only_update_mode_product"><?echo GetMessage("ESOL_IX_ONLY_UPDATE_MODE_PRODUCT"); ?></label>
					</div>
					<div>
						...
						<label for="only_update_mode_offer"><?echo GetMessage("ESOL_IX_ONLY_UPDATE_MODE_OFFERS"); ?></label>
					</div>
				</div>
			</td>
		</tr>
		...
		<tr class="esol-extra-mode-chbs-wrap<?if($sepMode){echo ' esol-extra-mode-chbs-wrap-active';}?>">
			<td valign="top"><?echo GetMessage("ESOL_IX_ONLY_CREATE_MODE"); ?>: <span id="hint_ONLY_CREATE_MODE_ELEMENT"></span><script>BX.hint_replace(BX('hint_ONLY_CREATE_MODE_ELEMENT'), '<?echo GetMessage("ESOL_IX_ONLY_CREATE_MODE_HINT"); ?>');</script></td>
			<td>
				...
				<div class="esol-extra-mode-chbs">
					<div>
						...
						<label for="only_create_mode_product"><?echo GetMessage("ESOL_IX_ONLY_CREATE_MODE_PRODUCT"); ?></label>
					</div>
					<div>
						...
						<label for="only_create_mode_offer"><?echo GetMessage("ESOL_IX_ONLY_CREATE_MODE_OFFERS"); ?></label>
					</div>
				</div>
			</td>
		</tr>
		...
		<tr>
			<td><?echo GetMessage("ESOL_IX_ONLY_DELETE_MODE"); ?>: <span id="hint_ONLY_DELETE_MODE"></span><script>BX.hint_replace(BX('hint_ONLY_DELETE_MODE'), '<?echo GetMessage("ESOL_IX_ONLY_DELETE_MODE_HINT"); ?>');</script></td>			<td>
				<input type="checkbox" name="SETTINGS_DEFAULT[ONLY_DELETE_MODE]" value="Y" <?if($SETTINGS_DEFAULT['ONLY_DELETE_MODE']=='Y'){echo 'checked';}?> onchange="EProfile.RadioChb(this, ['SETTINGS_DEFAULT[ONLY_UPDATE_MODE_ELEMENT]', 'SETTINGS_DEFAULT[ONLY_CREATE_MODE_ELEMENT]'], '<?echo htmlspecialcharsbx(GetMessage("ESOL_IX_ONLY_DELETE_MODE_CONFIRM")); ?>')">
			</td>
		</tr>
		...
		<tr>
			<td colspan="2" align="center">
				...
				<a href="javascript:void(0)" onclick="EProfile.OpenMissignElementFields(this)" class="esol-ix-link2window"><?echo GetMessage("ESOL_IX_ELEMENT_MISSING_SET_FIELDS"); ?></a>
			</td>
		</tr>
		...
		<tr>
			<td colspan="2" align="center">
				...
				<a href="javascript:void(0)" onclick="EProfile.OpenMissignElementFilter(this)" class="esol-ix-link2window"><?echo GetMessage("ESOL_IX_ELEMENT_MISSING_SET_FILTER"); ?></a>
			</td>
		</tr>
		...
		<tr>
			<td><?echo GetMessage("ESOL_IX_SECTION_EMPTY_REMOVE"); ?>:</td>
			<td>
		...
		<?if($bCatalog){?>
			...
			<tr>
				<td><?echo GetMessage("ESOL_IX_CALCULATE_PRICE"); ?>:</td>
				<td>
					...
					&nbsp;
					(<a href="javascript:void(0)" onclick="EProfile.OpenCalcPriceForm(this)" class="esol-ix-link2window"><?echo GetMessage("ESOL_IX_CALCULATE_PRICE_WINDOW"); ?></a>)
				</td>
			</tr>
		<?}?>
		
		<tr class="heading">
			<td colspan="2"><?echo GetMessage("ESOL_IX_SETTINGS_SPEED"); ?> <a href="javascript:void(0)" onclick="EProfile.ToggleAdditionalSettings(this)" class="esol_ix_head_more show"><?echo GetMessage("ESOL_IX_SETTINGS_ADDITONAL_SHOW_HIDE"); ?></a></td>
		</tr>
		...
		<tr>
			<td><?echo GetMessage("ESOL_IX_CHECK_CHANGES"); ?>: <span id="hint_CHECK_CHANGES"></span><script>BX.hint_replace(BX('hint_CHECK_CHANGES'), '<?echo GetMessage("ESOL_IX_CHECK_CHANGES_HINT"); ?>');</script></td>
			<td>
				<input type="hidden" name="SETTINGS_DEFAULT[CHECK_CHANGES]" value="N">
				<input type="checkbox" name="SETTINGS_DEFAULT[CHECK_CHANGES]" value="Y" <?if($SETTINGS_DEFAULT['CHECK_CHANGES']!='N' && $SETTINGS_DEFAULT['ELEMENT_IMAGES_FORCE_UPDATE']!='Y'){echo 'checked';}?> data-confirm-disable="<?echo GetMessage("ESOL_IX_DISABLE_IS_LOW_SPEED"); ?>">
			</td>
		</tr>
		...
		<tr>
			...
			<td>
				<input type="checkbox" name="SETTINGS_DEFAULT[ELEMENT_NOT_UPDATE_WO_CHANGES]" value="Y" <?if($SETTINGS_DEFAULT['ELEMENT_NOT_UPDATE_WO_CHANGES']=='Y' || ($PROFILE_ID=='new' && strlen($strError)==0)){echo 'checked';}?> data-confirm-disable="<?echo GetMessage("ESOL_IX_DISABLE_IS_LOW_SPEED"); ?>">
			</td>
		</tr>
		
		<tr>
			<td><?echo GetMessage("ESOL_IX_ELEMENT_DISABLE_EVENTS"); ?>: <span id="hint_ELEMENT_DISABLE_EVENTS"></span><script>BX.hint_replace(BX('hint_ELEMENT_DISABLE_EVENTS'), '<?echo GetMessage("ESOL_IX_ELEMENT_DISABLE_EVENTS_HINT"); ?>');</script></td>
			<td>
		...
		<tr>
			<td valign="top"><?echo GetMessage("ESOL_IX_PACKET_IMPORT"); ?>: <span id="hint_PACKET_IMPORT"></span><script>BX.hint_replace(BX('hint_PACKET_IMPORT'), '<?echo GetMessage("ESOL_IX_PACKET_IMPORT_HINT"); ?>');</script></td>
			<td valign="top">
				<??>
				<table cellspacing="0"><tr>
				...
				<td id="packet_size"<?if($SETTINGS_DEFAULT['PACKET_IMPORT']!='Y'){echo ' style="display: none;"';}?>>
					<?echo GetMessage("ESOL_IX_PACKET_SIZE"); ?>:
		...
		<tr>
			<td><?echo GetMessage("ESOL_IX_IMAGES_CHECK_PARAMS"); ?>: <span id="hint_EPARAMS_FOR_IMAGES_CHECK"></span><script>BX.hint_replace(BX('hint_EPARAMS_FOR_IMAGES_CHECK'), '<?echo GetMessage("ESOL_IX_IMAGES_CHECK_PARAMS_HINT"); ?>');</script></td>
			<td>
				<select name="SETTINGS_DEFAULT[IMAGES_CHECK_PARAMS]">
					<option value="PATH"<?if($SETTINGS_DEFAULT['IMAGES_CHECK_PARAMS']=='PATH' || $PROFILE_ID=='new'){echo ' selected';}?>><?echo GetMessage("ESOL_IX_IMAGES_CHECK_PATH").' ('.GetMessage("ESOL_IX_IMAGES_CHECK_PARAMS_FAST").')';?></option>
					<option value="MD5"<?if($SETTINGS_DEFAULT['IMAGES_CHECK_PARAMS']=='MD5'){echo ' selected';}?>><?echo GetMessage("ESOL_IX_IMAGES_CHECK_MD5").' ('.GetMessage("ESOL_IX_IMAGES_CHECK_PARAMS_SLOW_EXACTLY").')';?></option>
					<option value=""<?if(!$SETTINGS_DEFAULT['IMAGES_CHECK_PARAMS'] && $PROFILE_ID!='new'){echo ' selected';}?>><?echo GetMessage("ESOL_IX_IMAGES_CHECK_DEFAULT").' ('.GetMessage("ESOL_IX_IMAGES_CHECK_PARAMS_SLOW").')';?></option>
					<option value="WO_NAME"<?if($SETTINGS_DEFAULT['IMAGES_CHECK_PARAMS']=='WO_NAME' || $SETTINGS_DEFAULT['ELEMENT_NOT_CHECK_NAME_IMAGES']=='Y'){echo ' selected';}?>><?echo GetMessage("ESOL_IX_IMAGES_CHECK_WO_NAME").' ('.GetMessage("ESOL_IX_IMAGES_CHECK_PARAMS_SLOW").')';?></option>
					<option value="WO_SIZE"<?if($SETTINGS_DEFAULT['IMAGES_CHECK_PARAMS']=='WO_SIZE'){echo ' selected';}?>><?echo GetMessage("ESOL_IX_IMAGES_CHECK_WO_SIZE").' ('.GetMessage("ESOL_IX_IMAGES_CHECK_PARAMS_SLOW").')';?></option>
					<option value="PATH_SIZES"<?if($SETTINGS_DEFAULT['IMAGES_CHECK_PARAMS']=='PATH_SIZES'){echo ' selected';}?>><?echo GetMessage("ESOL_IX_IMAGES_CHECK_PATH_SIZES").' ('.GetMessage("ESOL_IX_IMAGES_CHECK_PARAMS_SLOW").')';?></option>
				</select>
			</td>
		</tr>
		...
		<?
		$arItems = array(
			GetMessage("ESOL_IX_SPEED_NOTE_ACC_UPDATE"),
			GetMessage("ESOL_IX_SPEED_NOTE_PICTURES"),
			GetMessage("ESOL_IX_SPEED_NOTE_ACC_CREATE")
		);
		if(class_exists('\Bitrix\Iblock\ElementTable'))
		{
			...
			{
				array_unshift($arItems, GetMessage("ESOL_IX_SPEED_NOTE_INDEX_NAME", array(
		...
		if($SETTINGS_DEFAULT['IBLOCK_ID'] && is_array($SETTINGS_DEFAULT['ELEMENT_UID']) && count($SETTINGS_DEFAULT['ELEMENT_UID']) > 0 && count($SETTINGS_DEFAULT['ELEMENT_UID'])==count(preg_grep('/^IP_PROP\d+$/', $SETTINGS_DEFAULT['ELEMENT_UID'])) && class_exists('\Bitrix\Iblock\PropertyTable'))
		{
			...
			if(count($arPropIds)==\Bitrix\Iblock\PropertyTable::getCount(array('ID'=>$arPropIds, 'VERSION'=>2, 'MULTIPLE'=>'N')))
			{
				...
				if(is_callable(array($conn, 'isTableExists')) && $conn->isTableExists($tblName))
				{
					...
					{
						array_unshift($arItems, GetMessage("ESOL_IX_SPEED_NOTE_INDEX_PROPS", array(
							"#LINK#" => '/bitrix/admin/sql.php?lang='.LANGUAGE_ID,
		...
		{
			array_unshift($arItems, GetMessage("ESOL_IX_SPEED_NOTE_XMLREADER"));
		}
		if(count($arItems) > 0)
		{
		?>
			...
			<?
			echo BeginNote();
			echo '<p align="center"><b>'.GetMessage("ESOL_IX_SPEED_NOTES").'</b></p>';
			echo '<ul>';
		...
		<tr class="heading">
			<td colspan="2"><?echo GetMessage("ESOL_IX_SETTINGS_STATISTIC"); ?> <a href="javascript:void(0)" onclick="EProfile.ToggleAdditionalSettings(this)" class="esol_ix_head_more show"><?echo GetMessage("ESOL_IX_SETTINGS_ADDITONAL_SHOW_HIDE"); ?></a></td>
		</tr>
		...
		<tr>
			<td><?echo GetMessage("ESOL_IX_STAT_SAVE_NOT_FOUND_ELEMENTS"); ?>:</td>
			<td>
				<input type="checkbox" name="SETTINGS_DEFAULT[STAT_SAVE_NOT_FOUND_ELEMENTS]" value="Y" <?if($SETTINGS_DEFAULT['STAT_SAVE_NOT_FOUND_ELEMENTS']=='Y'){echo 'checked';}?>>
			</td>
		</tr>
		
		<tr>
			<td><?echo GetMessage("ESOL_IX_STAT_SAVE_SKIPPED_ELEMENTS"); ?>:</td>
			<td>
				<input type="checkbox" name="SETTINGS_DEFAULT[STAT_SAVE_SKIPPED_ELEMENTS]" value="Y" <?if($SETTINGS_DEFAULT['STAT_SAVE_SKIPPED_ELEMENTS']=='Y'){echo 'checked';}?>>
			</td>
		</tr>
		
		<tr>
			<td><?echo GetMessage("ESOL_IX_STAT_SAVE_LAST_N"); ?>:</td>
			<td>
		...
		<tr>
			<td colspan="2" align="center">
				<?
				echo BeginNote();
				echo sprintf(GetMessage("ESOL_IX_STAT_NOTE"), '/bitrix/admin/'.$moduleFilePrefix.'_profile_list.php?lang='.LANGUAGE_ID);
				echo EndNote();
				?>
			</td>
		</tr>
		...
		<tr class="heading">
			<td colspan="2"><?echo GetMessage("ESOL_IX_SETTINGS_FILE_READING"); ?> <a href="javascript:void(0)" onclick="EProfile.ToggleAdditionalSettings(this)" class="esol_ix_head_more show" id="kda-head-more-link"><?echo GetMessage("ESOL_IX_SETTINGS_ADDITONAL_SHOW_HIDE"); ?></a></td>
		</tr>
		...
		<tr>
			...
			<td>
				...
				<td id="fix_xml_errors_params"<?if($SETTINGS_DEFAULT['AUTO_FIX_XML_ERRORS']!='Y'){echo ' style="display: none;"';}?>>
					...
					<div style="margin-top: 7px;">
						<input type="checkbox" name="SETTINGS_DEFAULT[AUTO_FIX_XML_NAMESPACES]" value="Y" <?if($SETTINGS_DEFAULT['AUTO_FIX_XML_NAMESPACES']=='Y'){echo 'checked';}?> id="esol_ix_auto_fix_xml_namespaces">
						<label for="esol_ix_auto_fix_xml_namespaces"><?echo GetMessage("ESOL_IX_AUTO_FIX_XML_ERRORS_NAMESPACES"); ?></label>
					</div>
				</td>
				</tr></table>
			</td>
		...
		<tr>
			<td><?echo GetMessage("ESOL_IX_MAX_READ_FILE_TIME"); ?>: <span id="hint_MAX_READ_FILE_TIME"></span><script>BX.hint_replace(BX('hint_MAX_READ_FILE_TIME'), '<?echo GetMessage("ESOL_IX_MAX_READ_FILE_TIME_HINT"); ?>');</script></td>
			<td>
				<select name="SETTINGS_DEFAULT[MAX_READ_FILE_TIME]">
					<?
					for($ii=10;$ii<=60;$ii=$ii+10)
					{
						echo '<option value="'.$ii.'"'.($SETTINGS_DEFAULT['MAX_READ_FILE_TIME']==$ii ? ' selected' : '').'>'.$ii.' '.GetMessage("ESOL_IX_MAX_READ_FILE_TIME_SEC").'</option>';
					}
					echo '<option value="0"'.((string)$SETTINGS_DEFAULT['MAX_READ_FILE_TIME']==='0' ? ' selected' : '').'>'.GetMessage("ESOL_IX_MAX_READ_FILE_TIME_NOLIMIT").'</option>';
					?>
				</select>
			</td>
		</tr>
		...
		/*if(isset($SETTINGS_DEFAULT['IBLOCK_ID']) && is_numeric($SETTINGS_DEFAULT['IBLOCK_ID']) && class_exists('\Bitrix\Iblock\IblockTable'))
		{
			...
			if($arIblock['WORKFLOW']=='Y' && CModule::IncludeModule('workflow'))
			{
				...
				?>
			<tr>
				<td><?echo GetMessage("ESOL_IX_USE_WORKFLOW"); ?>:</td>
				<td>
					<table cellspacing="0"><tr>
					...
					<td id="workflow_status"<?if($SETTINGS_DEFAULT['USE_WORKFLOW']!='Y'){echo ' style="display: none;"';}?>>
						<?echo GetMessage("ESOL_IX_WORKFLOW_STATUS"); ?>:<br>
		...
		<?if($bCatalog){?>
			<tr>
				<td valign="top"><?echo GetMessage("ESOL_IX_SEARCH_OFFERS_WO_PRODUCTS"); ?>: <span id="hint_SEARCH_OFFERS_WO_PRODUCTS"></span><script>BX.hint_replace(BX('hint_SEARCH_OFFERS_WO_PRODUCTS'), '<?echo GetMessage("ESOL_IX_SEARCH_OFFERS_WO_PRODUCTS_HINT"); ?>');</script></td>
				<td valign="top">
					...
					<div id="create_new_offers_wrap" style="margin-top: 7px;<?if($SETTINGS_DEFAULT['SEARCH_OFFERS_WO_PRODUCTS']!='Y'){echo 'display: none;';}?>">
						...
						<label for="create_new_offers_chb"><?echo GetMessage("ESOL_IX_CREATE_NEW_OFFERS"); ?></label>
					</div>
				</td>
			</tr>
		<?}?>
		...
		<tr>
			<td><?echo GetMessage("ESOL_IX_BIND_PROPERTIES_TO_SECTIONS"); ?>: <span id="hint_BIND_PROPERTIES_TO_SECTIONS"></span><script>BX.hint_replace(BX('hint_BIND_PROPERTIES_TO_SECTIONS'), '<?echo GetMessage("ESOL_IX_BIND_PROPERTIES_TO_SECTIONS_HINT"); ?>');</script></td>
			<td>
				<table cellspacing="0"><tr>
				...
				<?
				echo '<div style="padding-bottom: 2px;">'.GetMessage("ESOL_IX_BIND_PROPERTIES_TO_SECTIONS_EXCLUDE").'</div>';
		...
		<tr>
			<td class="esol-ix-settings-margin-container" colspan="2" align="center">
				...
				<div class="esol-ix-settings-phpexpression" style="display: none;">
					...
					<textarea name="SETTINGS_DEFAULT[ONAFTERSAVE_HANDLER]"<?if(!$USER->IsAdmin()){echo ' readonly';}?>><?echo $SETTINGS_DEFAULT['ONAFTERSAVE_HANDLER']?></textarea>
				</div>
			</td>
		</tr>
	...
	<tr>
		<td id="resblock" class="esol-ix-result">
		 <table width="100%"><tr><td width="50%">
			...
			<div class="detail_status" id="esol_ix_result_wrap">
				<?echo CAdminMessage::ShowMessage(array(
					"TYPE" => "PROGRESS",
					...
					"DETAILS" =>
					'<div class="esol-ix-result-block">'
						.'<span>'.GetMessage("ESOL_IX_SU_ALL").' <b id="total_line">0</b></span>'
						.'<span>'.GetMessage("ESOL_IX_SU_CORR").' <b id="correct_line">0</b></span>'
						.'<span>'.GetMessage("ESOL_IX_SU_ER").' <b id="error_line">0</b></span>'
					.'</div>'
					.'<div class="esol-ix-result-block">'
						.'<span class="esol-ix-result-item-green">'.GetMessage("ESOL_IX_SU_ELEMENT_ADDED").' <b id="element_added_line">0</b></span>'
						.'<span>'.GetMessage("ESOL_IX_SU_ELEMENT_UPDATED").' <b id="element_updated_line">0</b></span>'
						.'<span>'.GetMessage("ESOL_IX_SU_ELEMENT_CHANGED").' <b id="element_changed_line">0</b></span>'
						//.($SETTINGS_DEFAULT['STAT_SAVE']=='Y' && $SETTINGS_DEFAULT['STAT_SAVE_SKIPPED_ELEMENTS']=='Y' ? '<span>'.GetMessage("ESOL_IX_SU_ELEMENT_SKIPPED").' <b id="element_skipped_line">0</b></span>' : '')
						.($SETTINGS_DEFAULT['ONLY_DELETE_MODE']=='Y' ? ('<span class="esol-ix-result-item-red">'.GetMessage("ESOL_IX_SU_ELEMENT_DELETED").' <b id="element_removed_line">0</b></span>') : '')
						.($SETTINGS_DEFAULT['CELEMENT_MISSING_DEACTIVATE']=='Y' ? ('<span class="esol-ix-result-item-red">'.GetMessage("ESOL_IX_SU_HIDED").' <b id="killed_line">0</b></span>') : '')
						.($SETTINGS_DEFAULT['CELEMENT_MISSING_TO_ZERO']=='Y' ? ('<span>'.GetMessage("ESOL_IX_SU_ZERO_STOCK").' <b id="zero_stock_line">0</b></span>') : '')
						.($SETTINGS_DEFAULT['CELEMENT_MISSING_REMOVE_ELEMENT']=='Y' ? ('<span class="esol-ix-result-item-red">'.GetMessage("ESOL_IX_SU_REMOVE_ELEMENT").' <b id="old_removed_line">0</b></span>') : '')
					.'</div>'
					.'<div class="esol-ix-result-block">'
						.(!empty($SETTINGS_DEFAULT['ELEMENT_UID_SKU']) ? ('<span class="esol-ix-result-item-green">'.GetMessage("ESOL_IX_SU_SKU_ADDED").' <b id="sku_added_line">0</b></span>') : '')
						.(!empty($SETTINGS_DEFAULT['ELEMENT_UID_SKU']) ? ('<span>'.GetMessage("ESOL_IX_SU_SKU_UPDATED").' <b id="sku_updated_line">0</b></span>') : '')
						.(!empty($SETTINGS_DEFAULT['ELEMENT_UID_SKU']) ? ('<span>'.GetMessage("ESOL_IX_SU_SKU_CHANGED").' <b id="sku_changed_line">0</b></span>') : '')
						.($SETTINGS_DEFAULT['OFFER_MISSING_DEACTIVATE']=='Y' ? ('<span class="esol-ix-result-item-red">'.GetMessage("ESOL_IX_SU_OFFER_HIDED").' <b id="offer_killed_line">0</b></span>') : '')
						.($SETTINGS_DEFAULT['OFFER_MISSING_TO_ZERO']=='Y' ? ('<span>'.GetMessage("ESOL_IX_SU_OFFER_ZERO_STOCK").' <b id="offer_zero_stock_line">0</b></span>') : '')
						.($SETTINGS_DEFAULT['OFFER_MISSING_REMOVE_ELEMENT']=='Y' ? ('<span class="esol-ix-result-item-red">'.GetMessage("ESOL_IX_SU_OFFER_REMOVE_ELEMENT").' <b id="offer_old_removed_line">0</b></span>') : '')
					.'</div>'
					.'<div class="esol-ix-result-block">'
						.'<span class="esol-ix-result-item-green">'.GetMessage("ESOL_IX_SU_SECTION_ADDED").' <b id="section_added_line">0</b></span>'
						.'<span>'.GetMessage("ESOL_IX_SU_SECTION_UPDATED").' <b id="section_updated_line">0</b></span>'
						.($SETTINGS_DEFAULT['SECTION_EMPTY_DEACTIVATE']=='Y' ? ('<span class="esol-ix-result-item-red">'.GetMessage("ESOL_IX_SU_SECTION_EMPTY_DEACTIVATE").' <b id="section_deactivate_line">0</b></span>') : '')
						.($SETTINGS_DEFAULT['SECTION_EMPTY_REMOVE']=='Y' ? ('<span class="esol-ix-result-item-red">'.GetMessage("ESOL_IX_SU_SECTION_EMPTY_REMOVE").' <b id="section_remove_line">0</b></span>') : '')
					.'</div>'
					.'<div>'.GetMessage("ESOL_IX_EXECUTION_TIME").' <b id="execution_time"></b></div>'
					.($SETTINGS_DEFAULT['STAT_SAVE']=='Y' ? ('<b style="display: none; margin-top: 5px;"><a target="_blank" href="/bitrix/admin/'.$moduleFilePrefix.'_event_log.php?lang='.LANGUAGE_ID.'&find_profile_id='.($PROFILE_ID + 1).'&find_exec_id=" id="esol_ix_stat_profile_link">'.GetMessage("ESOL_IX_STATISTIC_LINK").'</a></b>') : '')
					.($SETTINGS_DEFAULT['STAT_SAVE']=='Y' ? ('<b style="display: none; margin-top: 5px;"><a target="_blank" href="/bitrix/admin/'.$moduleFilePrefix.'_rollback.php?lang='.LANGUAGE_ID.'&PROFILE_ID='.$PROFILE_ID.'&PROFILE_EXEC_ID=" id="esol_ix_rollback_profile_link">'.GetMessage("ESOL_IX_ROLLBACk_LINK").'</a></b>') : ''),
					"HTML" => true,
				))?>
			</div>
		 </td></tr></table>
...
{
	echo BeginNote();
	echo GetMessage("ESOL_IX_XMLREADER");
	echo EndNote();
}
if($STEP == 2)
{
	...
	if($arFile['path'])
	{
		...
		function EsolSetFilePath(l)
		{
			if($('#bx_file_data_file_cont .adm-input-file-name').length==0)
			{
				...
				else if(l < 50) setTimeout(function(){EsolSetFilePath(l+1);}, 50);
			}
...
if($_POST['PROCESS_CONTINUE']=='Y'){
	$oProfile = new \Bitrix\EsolImportxml\Profile();
?>
	EImport.Init(<?=\Bitrix\EsolImportxml\Utils::PhpToJSObject($arPost);?>, <?=\Bitrix\EsolImportxml\Utils::PhpToJSObject($oProfile->GetProccessParams($_POST['PROFILE_ID']));?>);
<?}else{?>
	EImport.Init(<?=\Bitrix\EsolImportxml\Utils::PhpToJSObject($arPost);?>);
<?}?>
<?endif; ?>
//-->
</script>
