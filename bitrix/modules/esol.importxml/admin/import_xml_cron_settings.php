403 Forbidden<?php # 403 Forbidden ?>
<?php // 403 Forbidden ?>
<?php /* 403 Forbidden */ ?>
...
function GetPhpPathForCron($checkPath = false)
{
	...
	if(is_array($arLines))
	{
		foreach($arLines as $line)
		{
			...
			if(isset($arLineParts[5]) && !in_array($arLineParts[5], $arPaths) && stripos($arLineParts[5], 'php')!==false && stripos($line, '.php')!==false)
			{
	...
	if($checkPath && count($arPaths) > 1)
	{
		...
		{				
			...
			if(is_array($arPhpLines) && isset($arPhpLines[0]) && preg_match('/PHP\s*([\d\.]+)/i', $arPhpLines[0], $m) && !array_key_exists($m[1], $arVersions))
			{
				$res = $m[1];
	...
	if(stripos($phpPath, 'php')!==false && strlen($phpPath) < 100) return $phpPath;
	...
	if (strlen($strErrorMessage)<=0 && $_REQUEST["subaction"]=='add')
	{
		...
		if (strlen($PROFILE_ID) > 0)
		{
			...
			else
			{
				...
				{
					$strSuccessMessage .= GetMessage("ESOL_IX_CRON_SAVE_ONLY_FILE");
					if($_SERVER['SERVER_ADMIN']=='info@timeweb.ru')
					{
						$strSuccessMessage .= GetMessage("ESOL_IX_CRON_SAVE_IN_PANEL_TIMEWEB")."<br><br><i>".str_replace(' '.$phpParams, '', mb_substr($cfg_subdata, mb_strlen($strTime))).'</i><br><br>';
					}
					else
					{
						$strSuccessMessage .= GetMessage("ESOL_IX_CRON_SAVE_IN_PANEL")."<br><br><i>".mb_substr($cfg_subdata, mb_strlen($strTime)).'</i><br><br>';
					}
					$strSuccessMessage .= GetMessage("ESOL_IX_CRON_WHERE_IS")."<br>";
					$strSuccessMessage .= '<i>'.$agent_php_path.'</i> - '.GetMessage("ESOL_IX_CRON_PHP_PATH_COMMENT")."<br>";
					$strSuccessMessage .= '<i>'.$execFile.'</i> - '.GetMessage("ESOL_IX_CRON_EXEC_FILE_COMMENT")."<br>";
...
if(($res = trim($ob->post($selfLink, array('action'=>'getphpversion')))) && stripos($res, 'php')!==false && strlen($res) < 100) $phpPath = $res;
else $phpPath = GetPhpPathForCron();
*/
$phpPath = GetPhpPathForCron();
/*/Define php path*/
