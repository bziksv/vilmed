...
if(isset($_POST['MAP']))
{
	...
	{
		$arParams = \Bitrix\EsolImportxml\Utils::Unserialize(base64_decode($_POST['MAP']));
  ...
  <div id="group_property_form">
	...
	<div style="display: none;">
		<select name="section">
			...
			<option value="NOT_LOAD"><?echo GetMessage("ESOL_IX_NOT_LOAD_PROPERTY");?></option><?
			$arFields = array('NOT_LOAD'=>GetMessage("ESOL_IX_NOT_LOAD_PROPERTY"));
				  ...
				  <div class="esol-ix-select-mapping-wrap" data-nc-message="<?echo GetMessage("ESOL_IX_NOT_CHOOSE")?>" data-xpath="<?echo htmlspecialcharsbx($curFieldXpath);?>">
					...
					if(array_key_exists($xmlId, $arMap2))
					{
						...
						foreach($arMap2[$xmlId] as $val)
						{
							...
							{
								echo '<div class="esol-ix-select-mapping'.($val['ID']!='NOT_LOAD' ? ' esol-ix-select-mapping-full' : '').'" data-xml-id="'.htmlspecialcharsbx($xmlId).'">'.
