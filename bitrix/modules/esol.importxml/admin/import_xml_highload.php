...
if ($REQUEST_METHOD == "POST" && $MODE=='AJAX')
{
	...
	if($ACTION=='RENAME_PROFILE')
	{
		...
		if(!\Bitrix\EsolImportxml\Utils::IsUtfMode()) $newName = $APPLICATION->ConvertCharset($newName, 'UTF-8', 'CP1251');
		$fl = new \Bitrix\EsolImportxml\Profile('highload');
		...
		if (strlen($strError) <= 0)
		{
			if($STEP==2 && !$notRewriteFile)
			{
				if((!isset($_FILES["DATA_FILE"]) || !$_FILES["DATA_FILE"]["tmp_name"]) && (!isset($_POST['DATA_FILE']) || is_numeric($_POST['DATA_FILE'])))
				{
					...
					{
						$fileId = \Bitrix\EsolImportxml\SMail::GetNewFile($SETTINGS_DEFAULT['EMAIL_DATA_FILE'], 0, 'esol_importxml_hl'.$PROFILE_ID);
						if($fileId > 0)
						{
			...
			if((isset($_FILES["DATA_FILE"]) && $_FILES["DATA_FILE"]["tmp_name"]) || (isset($_POST['DATA_FILE']) && $_POST['DATA_FILE'] && !is_numeric($_POST['DATA_FILE'])))
			{
				...
				elseif(isset($_POST['DATA_FILE']) && strlen($_POST['DATA_FILE']) > 0)
				{
					...
					if(strpos($_POST['DATA_FILE'], '/')===0) 
					{
						...
						if(!file_exists($filepath))
						{
							if(\Bitrix\EsolImportxml\Utils::IsUtfMode()) $filepath = $APPLICATION->ConvertCharsetArray($filepath, LANG_CHARSET, 'CP1251');
...
$arSubMenu[] = array(
	...
	"LINK" => "/bitrix/admin/".$moduleFilePrefix."_profile_list_highload.php?lang=".LANG,
);
$arSubMenu[] = array(
	...
	?>
		...
		<tr>
			...
			<td width="60%" class="esol-ix-file-choose">
				...
				if($arFile['SRC'])
				{
					...
					{
						if(\Bitrix\EsolImportxml\Utils::IsUtfMode()) $arFile['SRC'] = $APPLICATION->ConvertCharsetArray($arFile['SRC'], LANG_CHARSET, 'CP1251');
		...
		<tr>
			<td colspan="2" align="center">
				<input type="hidden" id="ELEMENT_MISSING_DEFAULTS" name="SETTINGS_DEFAULT[ELEMENT_MISSING_DEFAULTS]" value="<?echo htmlspecialcharsbx($SETTINGS_DEFAULT['ELEMENT_MISSING_DEFAULTS']);?>">
