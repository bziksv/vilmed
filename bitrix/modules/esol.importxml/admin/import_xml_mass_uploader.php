...
if($_POST['action']=='save')
{
	...
	$dir = trim(htmlspecialcharsbx($oRequest->get('folder')), '/');
	if(strlen($dir) > 0) 
	{
		if(CUtil::DetectUTF8($dir)) $dir = \Bitrix\Main\Text\Encoding::convertEncoding($dir, "UTF-8", "Windows-1251");
		...
		{
			//if(CUtil::DetectUTF8($arImage['name'])) $arImage['name'] = \Bitrix\Main\Text\Encoding::convertEncoding($arImage['name'], "UTF-8", "Windows-1251");
	...
	{
		$error = GetMessage("ESOL_IX_MASS_UPLOAD_ERROR_EMPTY_DIR");
	}
}
...
if($success)
{
	CAdminMessage::ShowMessage(array(
		'TYPE' => 'OK',
		'MESSAGE' => GetMessage("ESOL_IX_MASS_UPLOAD_SUCCESS")
	));
}
...
elseif(COption::GetOptionString($moduleId, 'IMAGES_PATH')) $folder = COption::GetOptionString($moduleId, 'IMAGES_PATH');
?>
<form action="<?=$_SERVER['REQUEST_URI']?>" method="post" enctype="multipart/form-data" name="field_settings">
	...
	<table width="100%">
		...
		<col width="50%">
		<tr>
			<td class="adm-detail-content-cell-l"><?echo GetMessage("ESOL_IX_MASS_UPLOAD_FOLDER");?>:</td>
