...
if($_POST && $_POST['action']=='save' /*(isset($_POST['FILTER']) || isset($_POST['EFILTER']))*/)
{
	...
	if(is_array($arFilter) && count($arFilter) > 0) echo base64_encode(serialize($arFilter));
	die();
}

if($OLDFILTER) $FILTER = \Bitrix\EsolImportxml\Utils::Unserialize(base64_decode($OLDFILTER));
...
if(count($FILTER) > 0 && count(preg_grep('/^find_/', array_keys($FILTER))) > 0)
{
	...
	\Bitrix\EsolImportxml\Utils::AddFilter($arFilter, $OLDFILTER);
...
<form action="" method="post" enctype="multipart/form-data" name="filter_form" id="esol-ix-filter" class="esol-ix-filter">
	...
	{
		$fl = new \Bitrix\EsolImportxml\FieldList(array('IBLOCK_ID'=>$IBLOCK_ID));
		$eFilter = new \Bitrix\EsolImportxml\Filter($IBLOCK_ID);
