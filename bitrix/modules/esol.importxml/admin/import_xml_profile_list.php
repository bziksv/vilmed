...
$profileType = \Bitrix\EsolImportxml\ProfileGroupTable::TYPE_IMPORT_IBLOCK;
...
if($_POST['action']=='get_profile_run_form' && strlen($_POST['profile_id']) > 0)
{
	...
	$arStatus = $oProfile->GetGadgetStatus($pid, true);
	...
	echo '<form method="post" action="/bitrix/admin/'.$moduleFilePrefix.'.php?PROFILE_ID='.$pid.'&lang='.LANGUAGE_ID.'">'.
		...
		'<input type="submit" value="'.GetMessage('ESOL_IX_RUN_IMPORT').'">'.
	'</form>';
	die();
}
...
if ($usePageNavigation)
{
	...
	$countQuery = new Query(\Bitrix\EsolImportxml\ProfileTable::getEntity());
	$countQuery->addSelect(new ExpressionField('CNT', 'COUNT(1)'));
...
$rsData = new CAdminResult(\Bitrix\EsolImportxml\ProfileTable::getList($getListParams), $sTableID);
if ($usePageNavigation)
{
...
$lAdmin->AddHeaders(array(
	...
	array("id"=>"RUN_BTN", "content"=>GetMessage("ESOL_IX_RUN_BTN"), "default"=>true),
));
...
{
	$row =& $lAdmin->AddRow('G'.$f_ID, $arGroup, $moduleFilePrefix."_profile_list.php?set_filter=Y&filter_group_id=".$f_ID."&lang=".LANG, GetMessage("ESOL_IX_TO_GROUP"));
...
{
	//$arProfileParams = \Bitrix\EsolImportxml\Profile::DecodeProfileParams($arProfile['PARAMS']);
	...
	$row->AddField("RUN_BTN", '<a href="javascript:void(0)" class="adm-nav-page esol_ix_run_btn" onclick="EProfileList.RunProfileImport(\''.$f_ID.'\');">'.GetMessage('ESOL_IX_RUN_IMPORT').'</a>');
