...
{
	$_POST['AUTH_SETTINGS'] = $AUTH_SETTINGS = \Bitrix\EsolImportxml\Utils::JsObjectToPhp($_POST['AUTH_SETTINGS']);
}
...
if(is_array($_POST['AUTH_SETTINGS']) && !\Bitrix\EsolImportxml\Utils::IsUtfMode()) 
{
...
if($_POST['action']=='checkconnect')
{
	...
	unset($_POST['AUTH_SETTINGS']['HANDLER_FOR_LINK']);
	$fileParams = \Bitrix\EsolImportxml\Utils::PhpToJSObject($_POST['AUTH_SETTINGS']);
	$oProfile = \Bitrix\EsolImportxml\Profile::getInstance($_REQUEST['TYPE']=='HL' ? 'highload' : 'iblock');
	$oProfile->CheckFileSettings($fileParams, $_REQUEST['PROFILE_ID']);
	$arFile = \Bitrix\EsolImportxml\Utils::MakeFileArray($fileParams);
	$res = ($arFile['size'] > 0 && $arFile['type']!='text/html');
	$arResult = array('result'=>($res ? 'success' : 'fail'), 'file'=>$arFile);
	echo \Bitrix\EsolImportxml\Utils::PhpToJSObject($arResult);
	die();
}
elseif($_POST['action']=='loadparams')
{
	...
	if(strlen($authLink) > 0)
	{
		...
		$className = \Bitrix\EsolImportxml\Utils::GetVendorClassName($arUrl['host']);
		...
		elseif(in_array($status, array(400, 405)))
		{
			...
			if(strpos($res, '{')===0 && ($arAnswer = \Bitrix\EsolImportxml\Utils::JsObjectToPhp($res)) && is_array($arAnswer) && 
	...
	echo \Bitrix\EsolImportxml\Utils::PhpToJSObject(array('VARS'=>$arVars, 'LOC'=>$formAction));
	die();
}
...
if($_POST['action']=='save' && $_POST['AUTH_SETTINGS'])
{
	...
	echo '<script>';
	echo 'EProfile.SetLinkAuthParams('.\Bitrix\EsolImportxml\Utils::PhpToJSObject($_POST['AUTH_SETTINGS']).');';
	echo '</script>';
	die();
}
...
<form action="<?echo $APPLICATION->GetCurUri();?>" method="post" enctype="multipart/form-data" name="field_settings">
	...
	<?
	echo BeginNote();
	echo GetMessage("ESOL_IX_AUTH_LINK_VIDEO").' <a target="_blank" href="https://www.youtube.com/watch?v=GIH8VboVSNA">https://www.youtube.com/watch?v=GIH8VboVSNA</a>';
	echo EndNote();
	?>
	<table width="100%" class="esol-ix-list-settings">
		...
		<tr>
			<td colspan="2" class="esol-ix-linkauth-handler">
				...
				?>
				<textarea name="AUTH_SETTINGS[HANDLER_FOR_LINK]"<?if(!$USER->IsAdmin()){echo ' readonly';}?>><?echo $AUTH_SETTINGS['HANDLER_FOR_LINK']?></textarea>
			</td>
		</tr>
	</table>
</form>
