...
| Подключение | `header.php` → на карточке товара (`isProductDetail()`) и в каталоге (`isCatalogDir()`) |
| Эта документация | `bitrix/templates/elektro_flat/css/VMD-DESCRIPTION.md` |

Подключается автоматически на странице товара и на страницах каталога
(`/catalog/...`). Никаких JS не требуется (FAQ работает на нативном `<details>`).

SEO-текст категории кладётся в **DESCRIPTION раздела** (админка → раздел →
«Описание») тем же контейнером `<article class="vmd-desc">…</article>`.
Промпт генерации в модуле Titlo: «Пример: со стилями (Vilmed)» для типа
`category`.

На страницах каталога скрипт `js/vmd-desc-collapse.js` оставляет видимыми
заголовок описания (`<h2>` первым ребёнком, legacy `<h1>`) и
`<p class="vmd-subtitle">`, остальное — за кнопкой «Подробнее» / «Свернуть».
**Не используйте `<h1>` в DESCRIPTION** — page H1 уже в `#pagetitle`.
На карточке товара поведение не меняется.
...
### SVG в админке (не вырезаются)

Ядро Bitrix (`fileman` html_editor) по умолчанию удаляет `<svg>` при
переключении в визуальный режим / сохранении (`svg: {remove:1}`).

Патч: `local/js/vilmed/admin-allow-svg.js` + подключение из
`local/php_interface/include/vilmed_admin_svg.php` (через `init.php`).
После выката можно править описание и в визуальном режиме — иконки
сохраняются. Без патча SVG пропадают, остаются пустые `<div class="ic">`.
...
| `vmd-note--warn` про изменение характеристик | не нужен отдельно — достаточно `vmd-manager` |
| `vmd-faq` | опционально (если есть типовые вопросы) |
| `vmd-cta` | не использовать в генерации |
| `vmd-manager` | обязательно один раз в конце (дисклеймер + контакт) |

---

## 6. Готовый промт для генерации описаний

Полный текст промта — ниже, в разделе **«PROMPT»** (этот же файл). Скопируй
блок целиком, подставь данные по товару (ТТХ, назначение) и отдай нейросети.
