...
if($obCache->InitCache($arParams["CACHE_TIME"], serialize($arFilter), "/catalog/element")) {
	...
	if($ob = $rsElement->GetNextElement()) {
		...
		$arCurElement = array(
			...
			"TITLE_RECOMMEND" => $arElement["PROPERTY_TITLE_RECOMMEND_VALUE"],
			"RECOMMEND" => $arProps["RECOMMEND"]["VALUE"],
		);
	}
...
if(!empty($arCurElement)) {
    ...
    if($arCurElement['RECOMMEND']){
        ...
        $APPLICATION->IncludeComponent("bitrix:catalog.section", "filtered",
            array(
                ...
                "PRODUCT_QUANTITY_VARIABLE" => $arParams["PRODUCT_QUANTITY_VARIABLE"],
                "PRODUCT_PROPS_VARIABLE" => $arParams["PRODUCT_PROPS_VARIABLE"],
                "FILTER_NAME" => "arCustomRecommendFilter",
                "CACHE_TYPE" => $arParams["CACHE_TYPE"],
                "CACHE_TIME" => $arParams["CACHE_TIME"],
	...
	//ALSO_BUY// VILMED: «С этим товаром покупают» — встроенная аналитика заказов (b_sale_product2product)
	if(!$arCurElement["IS_COLLECTION"] && Loader::includeModule("sale")) {
		$arAlsoBuyIds = array();
		$rsAlsoBuy = \Bitrix\Sale\Internals\Product2ProductTable::getList(array(
			"select" => array("PRODUCT_ID"),
			"filter" => array("=PARENT_PRODUCT_ID" => (int)$ElementID),
			"order"  => array("CNT" => "DESC"),
			"limit"  => 8,
		));
		while($arAB = $rsAlsoBuy->fetch()) {
			$abId = (int)$arAB["PRODUCT_ID"];
			if($abId > 0 && $abId != (int)$ElementID) {
				$arAlsoBuyIds[] = $abId;
			}
		}
		$arAlsoBuyIds = array_values(array_unique($arAlsoBuyIds));
		if(!empty($arAlsoBuyIds)) {
			global $arAlsoBuyFilter;
			$arAlsoBuyFilter = array("ID" => $arAlsoBuyIds, "!ID" => $ElementID);
			$APPLICATION->IncludeComponent("bitrix:catalog.section", "filtered",
				array(
					"IBLOCK_TYPE" => $arParams["IBLOCK_TYPE"],
					"IBLOCK_ID" => $arParams["IBLOCK_ID"],
