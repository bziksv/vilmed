	...
	public static function groups(): array
	{
		return [
			...
			'names' => [
				...
				'hint' => 'Укорочение NAME до ключевой фразы. Стандартные промпты ≈50 символов; свой промпт — свои правила и длина до 255. Перегенерация по новому промпту без жёсткого лимита «50».',
				'types' => [self::TYPE_PHRASE, self::TYPE_PHRASE_BATCH],
			],
			'section_names' => [
				'title' => 'Короткие названия категорий',
				'hint' => 'Фраза раздела. Стандарт ≈50 + «купить» в конце при возможности. Свой промпт — расширенные требования и длина до 255; можно перегенерировать по новому промпту.',
...
- НЕ используй <h1>: на странице товара уже есть H1 с названием карточки
  (совпадение с NAME даёт дубль в SEO и вёрстке). Начинай сразу с
  <p class="vmd-subtitle"> (одно предложение-лид), затем 1–2 вводных <p>.
  Если нужен заголовок смыслового блока — только <h2>/<h3>.
- Вводные <p>: главное помечай <strong>, один-два ключевых факта — <mark>.
...
- FAQ (если есть вопросы): <div class="vmd-faq">
  ...
  Обязательно оборачивай каждый вопрос в <details>…</details>, не оставляй
  голые <summary> без details.
...
ИКОНКИ (.ic): только полный инлайновый <svg>…</svg> в line-стиле Lucide
(не оставляй path/circle без обёртки <svg>):
...
- Слова «лизинг», «рассрочка», «кредит», «акции», «скидки», «в наличии на складе»,
  гарантированные сроки поставки — пиши ТОЛЬКО если эти слова ДОСЛОВНО есть
  в данных товара / на странице {link} (в тексте, свойствах, таблице).
  Если на странице их нет — не упоминай вообще, даже «нейтрально» и «по запросу».
- Не создавай FAQ вида «Есть ли лизинг/рассрочка?» и не отвечай «да» / «доступны».
- Если коммерческие условия на странице не упомянуты — максимум одна нейтральная
  фраза без лизинга/рассрочки/кредита:
  «Для получения КП по запросу, уточнения наличия и условий поставки напишите на
  <a href="mailto:info@vilmed.ru">info@vilmed.ru</a>.»
- В блоке vmd-manager не упоминай лизинг/рассрочку/кредит — только характеристики/
...
ПРАВИЛА РАЗМЕТКИ (используй только эти классы, ничего не выдумывай):
- Корень: <article class="vmd-desc"> … </article>.
- НЕ используй <h1>: на странице раздела уже есть H1 с названием категории.
  Начинай с <p class="vmd-subtitle"> (одно предложение-лид: что за раздел и для кого),
  затем 1–2 вводных <p>. Заголовки смысловых блоков — только <h2>/<h3>.
  Важно: vmd-subtitle должен идти первым — на сайте лид остаётся видимым,
  а остальной текст раскрывается по кнопке «Подробнее».
- Вводные <p>: главное помечай <strong>, 1–2 ключевых факта — <mark>.
...
- FAQ (2–4 вопроса): <div class="vmd-faq">
  ...
  Каждый вопрос — внутри <details>…</details>, не голый <summary>.
...
ЗАПРЕТ НА ВЫДУМАННЫЕ КОММЕРЧЕСКИЕ УСЛОВИЯ (жёстко):
- Слова «лизинг», «рассрочка», «кредит», «акции», «скидки», «в наличии на складе» —
  только если ДОСЛОВНО есть на странице {link}. Иначе не упоминай вообще.
- Не создавай FAQ вида «Есть ли лизинг/рассрочка?» с ответом «Да…».
- Если на странице коммерческих условий нет — максимум одна нейтральная фраза
  без лизинга/рассрочки/кредита:
  «Для получения КП по запросу, уточнения наличия и условий поставки напишите на
  <a href="mailto:info@vilmed.ru">info@vilmed.ru</a>.»
...
- подтянуть плотность и покрытие фразы;
- расширить текст осмысленно, не ломая читаемость.
...
- НЕ выкидывай и не схлопывай блоки: vmd-subtitle, вводные, h2/h3,
  .vmd-features, .vmd-list, .vmd-table-wrap/.vmd-spec, .vmd-faq (с <details>),
  .vmd-note, .vmd-manager, полные <svg> в .ic.
  НЕ используй и не добавляй <h1> (на странице уже есть H1 с названием).
...
ДОРАБОТАЙ уже размещённый HTML раздела {link}. Это редактура, не новый текст с нуля.
Сохрани деловой тон первого прохода: без рекламной воды и коммерческих клише.
Объём текста ДОЛЖЕН вырасти: допиши абзацы/пункты под недостающие слова TLP
(без off-topic воды). Верни только <article class="vmd-desc">…</article>.

ОБЯЗАТЕЛЬНО сохрани / восстанови:
- .vmd-subtitle первым (без <h1> — на странице уже есть H1), затем вводные,
  .vmd-features, .vmd-list, .vmd-note;
- FAQ: <div class="vmd-faq"><details><summary>…</summary><div class="vmd-faq__a">…</div></details>…
  без атрибута open (пункты свёрнуты); не оставляй голые <summary>;
- в конце .vmd-manager с контактом
  <a href="mailto:info@vilmed.ru">info@vilmed.ru</a>;
- любой email — только mailto-ссылка;
- vmd-cta не добавляй; иконки — полный <svg>…</svg> Lucide в .ic.
- лизинг/рассрочку/кредит не добавляй, если их нет на странице.

<strong>/<mark>: максимум 1–3 смысловых акцента. ЗАПРЕЩЕНО жирнить каждое слово из TLP.
Вплетай TLP обычным текстом. Не выдумывай ассортимент и коммерческие обещания.
TXT;
	}
