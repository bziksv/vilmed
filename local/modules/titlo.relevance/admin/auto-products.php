...
<div class="titlo-names titlo-auto-products">
		...
		<div class="titlo-howto__body">
			<ul>
				...
				<li>
					...
					<strong>Отбор товаров</strong> (кого берём) и
					<strong>Параметры проработки</strong> (что с ними делать).
				</li>
				<li>
					<strong>Отбор</strong>:
					<b>Ветка каталога</b> — поиск по частям слов (<code>kawe</code>),
					«Выбрать все» по видимым, «Применить».
					Несколько веток сразу; пустой выбор = весь каталог.
					Далее фильтр/сортировка списка, затем <b>Поиск товара</b>
					(части слов в любом порядке, например <code>ото kaw</code>) —
					ищет уже внутри выбранной ветки и фильтра.
				</li>
				<li>
					<strong>Параметры проработки</strong> —
					режим, промпт, политика «если описание уже есть», лимиты TLP,
					кнопка «В очередь».
				</li>
				<li>
					Режимы очереди:
					<em>только анализ</em> (балл, без генерации и без даты автопроработки),
					<em>полная проработка</em> (анализ → TLP → описание → сохранение → повторный анализ → дата),
					<em>повторная доработка</em> (без первого анализа, по промпту доработки).
				</li>
				<li>
					<strong>Полное описание</strong> — детальный текст карточки в Bitrix.
					Колонка «Описание» — есть ли оно и сколько символов.
					Колонка <strong>Балл</strong> — последний анализ; глазик — сводка, TF-IDF и TLP.
				</li>
				<li>
					<strong>Верхняя таблица</strong> — товары. <strong>Внизу</strong> — журнал заданий.
					«Текст не писали» = анализ был, описание не меняли.
					«Автопроработка» — автомат закрыл карточку, а не «есть ли текст».
				</li>
				<li>
					Сценарий: ветка (поиск <code>kawe</code> → Выбрать все → Применить) →
					фильтр «Ещё без автопроработки» → поиск «ото» → отметили →
					полная проработка → в очередь.
				</li>
				<li>
					<strong>Прогнать уже отмеченный снова</strong>: фильтр «Прошли автопроработку»
					или поиск по ID → галка → «Переписать описание заново» → «В очередь».
				</li>
				<li>
	...
	<section class="titlo-panel titlo-panel--select" aria-labelledby="titlo-panel-select-title">
		<header class="titlo-panel__head">
			<h3 class="titlo-panel__title" id="titlo-panel-select-title">Отбор товаров</h3>
		...
		<div class="titlo-panel__body">
			...
			<div class="titlo-auto-search-bar">
				<label class="titlo-auto-search" for="titlo-auto-q">Поиск товара</label>
				<input type="search" id="titlo-auto-q" class="adm-input titlo-auto-search__input"
					placeholder="части слов в любом порядке, напр. ото kaw"
				...
				<span class="titlo-auto-search__hint">Ищет внутри выбранной ветки и фильтра · все слова обязательны · порядок не важен</span>
			</div>
		</div>
	</section>
	...
	<section class="titlo-panel titlo-panel--run" aria-labelledby="titlo-panel-run-title">
		<header class="titlo-panel__head">
			...
			<span class="titlo-panel__hint">Режим, промпт и постановка выбранных товаров в очередь</span>
		</header>
